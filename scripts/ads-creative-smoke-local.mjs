#!/usr/bin/env node
// E2B on disposable workerd/D1/R2, never an account or persistent binding.
...
const temp = mkdtempSync(join(tmpdir(), 'bloomops-e2b-smoke-'));
let mf;
try {
  ...
  await build({ entryPoints: [new URL('./ads-creative-smoke-worker.mjs', import.meta.url).pathname], bundle: true, platform: 'node', format: 'esm', outfile: entry,
    ...
    external: ['node:*'], define: { E2B_MIGRATIONS: JSON.stringify(migrations) }, logLevel: 'silent' });
  ...
  writeFileSync(config, JSON.stringify({ name: 'bloomops-e2b-disposable', main: entry, compatibility_date: '2025-05-01', compatibility_flags: ['nodejs_compat'] }));
  ...
  mf = new Miniflare(convertV4MiniflareOptions({ name: 'bloomops-e2b-disposable', modules: true, script: readFileSync(join(temp, 'bundle/entry.js'), 'utf8'),
    compatibilityDate: '2025-05-01', compatibilityFlags: ['nodejs_compat'], cf: false,
    d1Databases: { DB: 'bloomops-e2b-disposable-local' }, r2Buckets: { FILES: 'bloomops-e2b-disposable-local' },
    resourcePersistencePath: temp, bindings: { E2B_DISPOSABLE: 'local-only' } }));
  const response = await mf.dispatchFetch('http://localhost/e2b-disposable'), result = await response.json();
  for (const message of result.messages || []) console.log(`ok   ${message}`);
  assert.equal(response.status, 200, result.error);
  console.log(`E2B disposable workerd/D1/R2: ${result.checks} checks passed.`);

...
for (let i = 0; i < 12; i++) {
  ...
  for (const dept of ['systems', 'social']) {
    ...
    if (dept === 'social') for (let j = 0; j < 4; j++) {
...
try {
  for (const role of ['owner', 'admin', 'team_member', 'client']) {
    ...
    const res = await context.request.post(base + '/api/auth/sign-in/magic-link', { headers: { origin: base }, data: { email, callbackURL: '/' } });
  ...
  writeFileSync(join(out, 'fixture.json'), JSON.stringify({ ws, members, clients, projects, content }, null, 2), { mode: 0o600 });
  console.log('Synthetic fixture ready: 12 Clients, 24 Projects, 72 Milestones, 240 Actions, 72 Deliverables, 48 Content Items; four issued role sessions.');
} catch {

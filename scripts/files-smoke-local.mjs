#!/usr/bin/env node
// Run B5 inside the real Worker runtime with disposable D1 and R2. No account,
...
const temp = mkdtempSync(join(tmpdir(), 'bloomops-b5-smoke-'));
let mf;
try {
  ...
  const bundle = await build({ entryPoints: [new URL('./files-smoke-worker.mjs', import.meta.url).pathname], bundle: true, platform: 'node', format: 'esm', write: false,
    external: ['node:*'], define: { B5_MIGRATIONS: JSON.stringify(migrations) }, logLevel: 'silent' });
  mf = new Miniflare(convertV4MiniflareOptions({ name: 'bloomops-b5-disposable', modules: true, script: bundle.outputFiles[0].text,
    compatibilityDate: '2025-05-01', compatibilityFlags: ['nodejs_compat'], cf: false,
    d1Databases: { DB: 'bloomops-b5-disposable-local' }, r2Buckets: { FILES: 'bloomops-b5-disposable-local' },
    resourcePersistencePath: temp, bindings: { B5_DISPOSABLE: 'local-only' } }));
  // Dispatch into the Worker itself; no Node-side R2 proxy is used.
  const response = await mf.dispatchFetch('http://localhost/b5-disposable');
  const result = await response.json();
  for (const message of result.messages || []) console.log(`ok   ${message}`);
  assert.equal(response.status, 200, result.error);
  console.log(`B5 disposable workerd/D1/R2: ${result.checks} checks passed.`);

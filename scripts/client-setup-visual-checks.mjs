...
export async function clientSetupVisualChecks({page,ctx,base,clientId,check,out,workspaceId}){
 ...
 await page.locator('.bo-sheet-toolbar').waitFor();const view=page.locator('.bo-sheet-toolbar>.bo-sheet-menu-anchor>.bo-btn');await view.focus();await page.keyboard.press('Enter');check('Prospect view opens through the keyboard',await page.locator('.bo-sheet-menu').isVisible());await page.keyboard.press('Escape');

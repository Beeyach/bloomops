...
export async function clientSetupVisualChecks({page,ctx,base,clientId,check,out,workspaceId}){
 ...
 check('Metric labels share the same grid baseline',await metric.evaluate(e=>{const labels=e.querySelectorAll('.bo-report-grid>label');return Math.abs(labels[0].getBoundingClientRect().top-labels[1].getBoundingClientRect().top)<1;}));
 ...
 await page.locator('.bo-sheet-toolbar').waitFor();const view=page.getByRole('button',{name:'All prospects',exact:true});await view.focus();await page.keyboard.press('Enter');check('Prospect view opens through the keyboard',await page.locator('.bo-sheet-menu').isVisible());await page.keyboard.press('Escape');await page.locator('.bo-sheet-menu').waitFor({state:'hidden'});check('Prospect view Escape closes and returns focus',await view.evaluate(e=>e===document.activeElement));

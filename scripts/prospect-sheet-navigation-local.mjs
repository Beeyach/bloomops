...
const out=resolve(arg('--out','/tmp/prospecting-navigation')),base=arg('--base','http://localhost:8788');
...
try{
 ...
 writeFileSync(join(out,'measurement.json'),JSON.stringify({method:'Local built Worker, identical isolated fixture and zero artificial D1 latency; seven rounds, two warmups; full destination plus two frames. Cold JS decoded bytes measured separately.',summary,bundles,samples},null,2)+'\n');console.log(JSON.stringify(summary));

// Test-only entry bundled by ads-smoke-local; never deployed.
...
export default { async fetch(request, env) {
  if (env.D1_ADS_DISPOSABLE !== 'local-only' || new URL(request.url).hostname !== 'localhost') return new Response(null, { status: 403 });
  ...
  try {
    ...
    for (const migration of D1_ADS_MIGRATIONS) await run(migration);
    ...
    for (let i=0; i<240; i++) {
      const id = `p-${String(i).padStart(3,'0')}`, service = i % 2 ? 'social' : 'ads';
      await env.DB.batch([
    ...
    const read=(who=owner, filters={})=>adsProjection(db,who,filters,{now});
    ...
    check('240 assignments yield 50 current Ads Projects and a readable overflow',first.projects.items.length===50&&first.projects.hasMore);
    check('all Projects follow current Ads engagement truth',first.projects.items.every(p=>Number(p.id.slice(2))%2===0));
    ...
    check('Ads issues five metadata statements in one native D1 batch without R2',queryCount===5&&batchCount===1&&!env.FILES);
    ...
    check('all 120 Ads Projects are reachable in deterministic pages',second.projects.items.length===50&&third.projects.items.length===20&&!third.projects.hasMore&&new Set([...first.projects.items,...second.projects.items,...third.projects.items].map(p=>p.id)).size===120);
    check('bounded delivery and facet options exclude non-Ads Services',first.deliverables.items.length===6&&first.deliverables.hasMore&&first.options.services.items.length===1&&first.options.services.items[0].id==='ads');
    ...
    for(const who of [owner,pm]) check(`${who.role} sees current Ads work`,(await read(who)).projects.items.length===50);
    ...
    check('Client actors cannot use internal Ads read model',!(await read(await actor('client'))).ok);
    check('foreign actor sees no local Ads Projects',!(await read(await actor('foreign'))).projects.items.length);
    for(const filters of [{clientId:'foreign'},{serviceEngagementId:'social'},{serviceEngagementId:'missing'},{page:'0'}]) check('foreign/non-Ads/invalid filters fail safely',!(await read(owner,filters)).ok);
    ...
    await run("UPDATE service_types SET name='Renamed',department_id='social' WHERE id='ads'");
    ...
    await run("UPDATE service_types SET department_id='ads' WHERE id='ads'");
    await run("DELETE FROM project_assignments WHERE membership_id='m-team'");
    check('issued actor loses all Ads work after Project unassignment',!(await read(team)).projects.items.length);

#!/usr/bin/env node
// C4 on disposable workerd/D1/R2, never an account or persistent binding.
...
const temp = mkdtempSync(join(tmpdir(), 'bloomops-c4-smoke-'));
let mf;
try {
  const journal = JSON.parse(readFileSync(new URL('../drizzle/meta/_journal.json', import.meta.url)));
  assert.equal(journal.entries[16].tag, '0016_c4_content_assets');
  ...
  await build({ entryPoints: [new URL('./content-files-smoke-worker.mjs', import.meta.url).pathname], bundle: true, platform: 'node', format: 'esm', outfile: entry,
    ...
    external: ['node:*'], define: { C4_MIGRATIONS: JSON.stringify(migrations) }, logLevel: 'silent' });
  ...
  writeFileSync(config, JSON.stringify({ name: 'bloomops-c4-disposable', main: entry, compatibility_date: '2025-05-01', compatibility_flags: ['nodejs_compat'] }));
  ...
  mf = new Miniflare(convertV4MiniflareOptions({ name: 'bloomops-c4-disposable', modules: true, script: readFileSync(join(temp, 'bundle/entry.js'), 'utf8'),
    compatibilityDate: '2025-05-01', compatibilityFlags: ['nodejs_compat'], cf: false,
    d1Databases: { DB: 'bloomops-c4-disposable-local' }, r2Buckets: { FILES: 'bloomops-c4-disposable-local' },
    resourcePersistencePath: temp, bindings: { C4_DISPOSABLE: 'local-only' } }));
  const response = await mf.dispatchFetch('http://localhost/c4-disposable'), result = await response.json();
  for (const message of result.messages || []) console.log(`ok   ${message}`);
  assert.equal(response.status, 200, result.error);
  console.log(`C4 disposable workerd/D1/R2: ${result.checks} checks passed.`);

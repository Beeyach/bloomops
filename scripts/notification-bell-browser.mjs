...
export async function checkNotificationBell({page,ctx,base,inbox,command,check,out}){
 ...
 await page.keyboard.press('Escape');await page.setViewportSize({width:1440,height:1000});await bell.click();await panel.getByText(item.title,{exact:true}).click();await page.waitForURL(u=>u.pathname.startsWith('/discussions/'));check('bell item uses authorized open and marks only that item read',(await inbox(ctx)).items.find(n=>n.id===item.id).readAt!==null);

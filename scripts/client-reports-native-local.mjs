...
try{
 const binding=await mf.getD1Database('DB'),fresh=await mf.getD1Database('FRESH');const files=migrationFiles();assert.equal(files.at(-1).tag,'0048_clumsy_nekra');
 ...
 for(const table of tables)for(const row of all(t.raw,`SELECT * FROM ${table}`)){if(table==='client_report_metrics'){delete row.source_kind;delete row.import_id;}await binding.prepare(`INSERT INTO ${table}(${Object.keys(row).join(',')}) VALUES(${Object.keys(row).map(()=>'?').join(',')})`).bind(...Object.values(row)).run();}

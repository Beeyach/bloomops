...
import {checkReportComparisons} from '../tests/_client-report-comparisons.mjs';
...
try{
 ...
 await migrate(binding,files.slice(-3));await migrate(fresh,files);
 ...
 const upgrade=await mf.getD1Database('ARCHIVE');await migrate(upgrade,files.slice(0,-2));
 ...
 await migrate(upgrade,files.slice(-2));
 ...
 const compareUpgrade=await mf.getD1Database('COMPARE');await migrate(compareUpgrade,files.slice(0,-1));
 await changeReportArchive(t.db,t.owner,'james',legacy.id,archiveInput());
 for(const table of upgradeTables)for(const row of all(t.raw,`SELECT * FROM ${table}`))await compareUpgrade.prepare(`INSERT INTO ${table}(${Object.keys(row).join(',')}) VALUES(${Object.keys(row).map(()=>'?').join(',')})`).bind(...Object.values(row)).run();
 const comparisonBefore=await Promise.all(upgradeTables.map(async table=>(await compareUpgrade.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all()).results));
 assert.ok(comparisonBefore[upgradeTables.indexOf('client_report_drafts')].some(r=>r.archived_at&&r.publication_floor>0),'archived published fixture exists before comparison upgrade');
 await migrate(compareUpgrade,files.slice(-1));
 const comparisonAfter=await Promise.all(upgradeTables.map(async table=>(await compareUpgrade.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all()).results));
 check('comparison migration preserves populated archives, observations and publication snapshots',JSON.stringify(comparisonBefore)===JSON.stringify(comparisonAfter));
 check('comparison upgrade starts with no invented selections',(await compareUpgrade.prepare('SELECT count(*) n FROM client_report_comparisons').first()).n===0);
 check('comparison upgrade and fresh schema match',JSON.stringify(await schema(compareUpgrade))===JSON.stringify(await schema(fresh)));
 for(const template of ['ghl_campaign','social']){await checkReportComparisons(db,t.owner,await t.actor('james'),template);check(template+' native persisted comparison, immutable publication and source withdrawal',true);}
 await assert.rejects(binding.prepare("INSERT INTO client_report_comparisons(workspace_id,report_id,publication_id) VALUES('b',?,?)").bind(id,releases.firstId).run(),/FOREIGN KEY/);check('native comparison parent scope rejects cross-workspace references',true);

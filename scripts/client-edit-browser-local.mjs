...
try{
 ...
 const open=async p=>{await p.bringToFront();await p.goto(url,{waitUntil:'networkidle'});await p.getByRole('button',{name:'Edit details',exact:true}).click();await p.getByLabel('Client or company name',{exact:true}).waitFor();await p.waitForFunction(()=>!document.getElementById('edit-name')?.closest('fieldset').disabled);};
 const name=p=>p.getByLabel('Client or company name',{exact:true}),website=p=>p.getByLabel(/^Website(?:\s*\(optional\))?$/),dialog=p=>p.getByRole('dialog',{name:'Edit client details',exact:true});
 ...
 await page.reload({waitUntil:'networkidle'});await open(page);check('reopening uses current saved Client until explicit restoration',await name(page).inputValue()==='Client Recovery Synthetic Client');

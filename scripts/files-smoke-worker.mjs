// Test-only entry bundled by files-smoke-local. Never part of the app Worker.
...
export default { async fetch(request, env) {
  if (env.B5_DISPOSABLE !== 'local-only' || new URL(request.url).hostname !== 'localhost') return new Response(null, { status: 403 });
  ...
  try {
    ...
    for (const sql of B5_MIGRATIONS) await run(sql);
    ...
    for (const failure of ['reserve', 'finalize', 'activity', 'put', 'cleanup']) {
      ...
      if (failure === 'reserve') await run("CREATE TRIGGER fail_b5 BEFORE INSERT ON assets BEGIN SELECT RAISE(ABORT,'reserve'); END");
      if (failure === 'finalize') await run("CREATE TRIGGER fail_b5 BEFORE UPDATE ON assets WHEN NEW.status='ready' BEGIN SELECT RAISE(ABORT,'finalize'); END");
      if (failure === 'activity') await run("CREATE TRIGGER fail_b5 BEFORE INSERT ON activity_events WHEN NEW.subject_type='file' BEGIN SELECT RAISE(ABORT,'activity'); END");
      ...
      if (['reserve', 'finalize', 'activity'].includes(failure)) await run('DROP TRIGGER fail_b5');
      if (file) {

#!/usr/bin/env node
// C5 on disposable workerd/D1/R2, never an account or persistent binding.
...
const temp = mkdtempSync(join(tmpdir(), 'bloomops-c5-smoke-'));
let mf;
try {
  ...
  await build({ entryPoints: [new URL('./content-approvals-smoke-worker.mjs', import.meta.url).pathname], bundle: true, platform: 'node', format: 'esm', outfile: entry,
    ...
    external: ['node:*'], define: { C5_MIGRATIONS: JSON.stringify(migrations) }, logLevel: 'silent' });
  ...
  writeFileSync(config, JSON.stringify({ name: 'bloomops-c5-disposable', main: entry, compatibility_date: '2025-05-01', compatibility_flags: ['nodejs_compat'] }));
  ...
  mf = new Miniflare(convertV4MiniflareOptions({ name: 'bloomops-c5-disposable', modules: true, script: readFileSync(join(temp, 'bundle/entry.js'), 'utf8'),
    compatibilityDate: '2025-05-01', compatibilityFlags: ['nodejs_compat'], cf: false,
    d1Databases: { DB: 'bloomops-c5-disposable-local' }, r2Buckets: { FILES: 'bloomops-c5-disposable-local' },
    resourcePersistencePath: temp, bindings: { C5_DISPOSABLE: 'local-only' } }));
  const response = await mf.dispatchFetch('http://localhost/c5-disposable'), result = await response.json();
  for (const message of result.messages || []) console.log(`ok   ${message}`);
  assert.equal(response.status, 200, result.error);
  console.log(`C5 disposable workerd/D1/R2: ${result.checks} checks passed.`);

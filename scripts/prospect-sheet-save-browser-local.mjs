...
const pending=()=>page.locator('.bo-sheet-cell-recovery');
...
try{
 ...
 await choice('Fit').selectOption('skip');await pending().waitFor();check('Failed direct save retains selected value beside its cell for retry',await choice('Fit').inputValue()==='skip'&&(await read()).fit==='strong');check('Failure visibly says not saved',await page.locator('.bo-sheet-save-status.error').innerText()==='Fit not saved');
 await page.unroute('**/api/bloomops/prospecting/sheet-edit');await pending().getByRole('button',{name:'Retry',exact:true}).click();await pending().waitFor({state:'hidden'});await settled();check('Retry commits retained choice and clears stale failure',(await read()).fit==='skip'&&await page.locator('.bo-sheet-save-status.error').count()===0);
 ...
 await choice('Fit').selectOption('hold');await pending().waitFor();const revision=(await read()).revision,count=writes.length;check('Lost reply retains inline recovery even though server committed',(await read()).fit==='hold');
 await page.unroute('**/api/bloomops/prospecting/sheet-edit');await pending().getByRole('button',{name:'Retry',exact:true}).click();await pending().waitFor({state:'hidden'});await settled();check('Lost reply retry confirms without another write',(await read()).revision===revision&&writes.length===count);
 ...
 await choice('Fit').selectOption('skip');await pending().waitFor();check('Concurrent field change is protected',(await read()).fit==='strong'&&(await pending().innerText()).includes('Saved: Strong'));await pending().getByRole('button',{name:'Keep saved',exact:true}).click();await settled();
 ...
 await page.route('**/api/bloomops/prospecting/sheet-edit',r=>r.abort('failed'));await choice('Platform').selectOption('WordPress');await pending().waitFor();acceptDialogs=true;await page.reload({waitUntil:'networkidle'});acceptDialogs=false;await page.unroute('**/api/bloomops/prospecting/sheet-edit');await ready();await page.locator('.bo-sheet-recovery summary').click();await page.getByRole('button',{name:'Restore edits',exact:true}).click();await pending().waitFor();check('Failed direct choice survives reload for inline recovery',await choice('Platform').inputValue()==='WordPress');await pending().getByRole('button',{name:'Save',exact:true}).click();await pending().waitFor({state:'hidden'});await settled();check('Recovered choice saves canonical value',(await read()).platform==='WordPress');

#!/usr/bin/env node
// D1 Ads acceptance against the actual built Worker. Loopback and development
...
const base = arg("--url", "http://localhost:8787"),
  out = resolve(arg("--out", "/tmp/bloomops-e1-review"));
...
try {
  ...
  await reference.getByRole('heading',{name:'Palette',exact:true}).waitFor();
  await reference.screenshot({path:join(out,'design-reference.png'),fullPage:true}); screenshots++; await reference.close();
  const suffix=randomUUID().slice(0,8),ws=`e1-review-${suffix}`,members={};
  ...
  const types={};
  for(const [key,department,name] of [['build','ads','Website delivery'],['course','ads','Learning delivery'],['social','social','Social media']]) {
  ...
  const owner=await login(members.owner.email,'/ads'),team=await login(members.team.email,'/ads'),pm=await login(members.pm.email,'/ads');
  const errors=[]; for(const who of [owner,team,pm])who.page.on('pageerror',e=>errors.push(e.message));
  await owner.page.goto(base+'/ads', {waitUntil:'networkidle'});
  check('empty Ads is a real work destination',await owner.page.getByRole('heading',{name:'No campaign work in this view'}).isVisible());
  for(const width of widths)await layout('ads-empty',owner.page,width);
  const clients=[];
  ...
  const projects={};
  const longName='A considered campaign launch · '+ 'A'.repeat(80);
  for(const [key,client,name,service,visibility] of [['build',clients[0],longName,services.build,'client'],['social',clients[0],'NOT_ADS_GHL_KAJABI',services.social,'internal'],['course',clients[1],'Retargeting creative preparation',services.course,'client'],['unlinked',clients[0],'UNLINKED_ADS_PROJECT',null,'internal'],['secret',clients[1],'RESTRICTED_PROJECT_SECRET',services.course,'restricted']]) {
    projects[key]=(await api(owner.context,`/api/bloomops/clients/${client}/projects`,{name,serviceEngagementId:service,visibility,clientLabel:key==='build'?'Your campaign':null},201)).projectId;
  }
  ...
  const deliverable=(await api(owner.context,projectPath+'/deliverables',{title:'Campaign delivery brief',clientLabel:'Your welcome guide',visibility:'client',requestId:randomUUID()},201)).deliverableId;
  let revision=1;
  ...
  const bytes=Buffer.from('Synthetic Ads handoff.\n');
  const metadata={requestId:randomUUID(),filename:'Ads handoff.txt',mimeType:'text/plain',byteSize:bytes.length,visibility:'client',deliverableId:deliverable};
  ...
  await owner.page.goto(base+'/ads', {waitUntil:'networkidle'});
  check('Ads lists relational projects and omits free-text impostors',await owner.page.locator('[data-project-id]').count()===3&&!/NOT_ADS_GHL_KAJABI|UNLINKED_ADS_PROJECT/.test(await owner.page.locator('main').innerText()));
  check('Ads HTTP is protected and uncached',noStore(await owner.context.request.get(base+'/ads')));
  const summary=owner.page.getByRole('list',{name:`Summary for ${longName}`,exact:true});
  check('canonical summary presents progress, waiting Action, review Deliverable and ready File',/1 of 1 milestones finished/.test(await summary.innerText())&&/100%/.test(await summary.innerText())&&/1 open Action/.test(await summary.innerText())&&/1 waiting/.test(await summary.innerText())&&/1 in Client Review/.test(await summary.innerText())&&/1 Ready file/.test(await summary.innerText()));
  check('Ads operational summaries use distinct badges without middle-dot separators', !(await summary.innerText()).includes('·') && await summary.locator('.bo-status').count() >= 2);
  ...
  for(const width of widths)await layout('ads-populated-long-name',owner.page,width);
  ...
  check('Client filter narrows Projects and Service choices',await owner.page.locator('[data-project-id]').count()===1&&await owner.page.locator('#ads-service option').count()===2);
  ...
  await owner.page.goto(base+'/ads?status=forged', {waitUntil:'networkidle'});
  check('invalid filters retain heading, announced error and no data',await owner.page.getByRole('heading',{name:'Ads',exact:true}).isVisible()&&await owner.page.locator('main [role="alert"]').count()===1&&await owner.page.locator('[data-project-id]').count()===0);
  for(const width of widths)await layout('ads-invalid-filters',owner.page,width);
  await keyboardActivate(owner.page,owner.page.getByRole('link',{name:'Reset filters',exact:true}));await owner.page.waitForURL(base+'/ads');await owner.page.waitForLoadState('networkidle');
  ...
  await owner.page.goto(base+'/ads', {waitUntil:'networkidle'});await keyboardActivate(owner.page,owner.page.getByRole('link',{name:'1 waiting',exact:true}));await owner.page.waitForURL(/view=waiting/);await owner.page.waitForLoadState('networkidle');
  check('summary Action link opens canonical filtered Work',await owner.page.locator(`[data-action-id="${action}"]`).count()===1);
  await team.page.goto(base+'/ads', {waitUntil:'networkidle'});
  check('Action-only Team gets empty Ads with no parent or sibling summaries',await team.page.locator('[data-project-id],[data-deliverable-id]').count()===0&&!/Garden Studio|Learning Studio/.test(await team.page.locator('main').innerText()));
  for(const width of widths)await layout('ads-action-only',team.page,width);
  ...
  check('explicit Project assignment grants exactly its Ads tree',await team.page.locator('[data-project-id]').count()===1&&await team.page.locator(`[data-project-id="${projects.build}"]`).count()===1);
  for(const width of widths)await layout('ads-assigned-team',team.page,width);
  await pm.page.goto(base+'/ads', {waitUntil:'networkidle'});
  ...
  check('restricting Deliverable removes it and its File from PM summary',await pm.page.locator('[data-deliverable-id]').count()===0&&!(await pm.page.locator('main').innerText()).includes('Ready file')&&!(await pm.page.content()).includes('Campaign delivery brief'));
  for(const width of widths)await layout('ads-pm-restricted-child',pm.page,width);
  await client.page.goto(base+'/ads', {waitUntil:'networkidle'});await client.page.waitForURL(/\/portal/);await client.page.waitForLoadState('networkidle');
  check('Client route redirects to portal without Ads navigation or internal work',!(await client.page.content()).includes('Confirm the access invitation')&&await client.page.locator('a[href="/ads"],a[href="/portal/ads"]').count()===0);
  const nojs=await browser.newContext({javaScriptEnabled:false,storageState:await owner.context.storageState()}),plain=await nojs.newPage();await plain.goto(base+'/ads', {waitUntil:'networkidle'});
  ...
  const nojsView=async(label,expectedProjects)=>{
    for(const width of widths){
      ...
      check(`${label} ${width}px completed heading and filters are visible without JavaScript`,
        await plain.locator('h1').count()===1&&await plain.getByRole('heading',{name:'Ads',level:1,exact:true}).isVisible()
        &&(await Promise.all(['Client','Service','Project status'].map(name=>plain.getByLabel(name,{exact:true}).isVisible()))).every(Boolean)
        &&!(await plain.locator('main').innerText()).includes('Loading Ads projects…'));
  ...
  await nojsView('ads-nojs',[projects.build,projects.course,projects.secret]);
  ...
  check('no-JavaScript native filter submits GET and preserves the selected query',filteredResponse?.status()===200
    &&filteredResponse.request().method()==='GET'&&filteredURL.pathname==='/ads'
  ...
  await nojsView('ads-nojs-filtered',[projects.build]);
  check('no-JavaScript filtered result retains Client selection and narrows Service choices',
    await plain.getByLabel('Client',{exact:true}).inputValue()===clients[0]&&await plain.locator('#ads-service option').count()===2);
  await nojs.close();
  ...
  for(const width of widths)await layout('ads-last-page',owner.page,width);
  ...
  sql(`UPDATE workspace_memberships SET status='suspended' WHERE id=${lit(members.team.membership)}`);await team.page.goto(base+'/ads', {waitUntil:'networkidle'});await team.page.waitForURL(/\/sign-in/);await team.page.waitForLoadState('networkidle');
  check('suspension revokes the existing Ads session',!await team.page.getByRole('heading',{name:'Ads',exact:true}).count());
  await owner.page.goto(base+'/ads?clientId='+clients[1], {waitUntil:'networkidle'});await owner.page.emulateMedia({reducedMotion:'reduce'});await layout('ads-reduced-motion',owner.page,320);
  ...
  const touch=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true,storageState:await owner.context.storageState()}),touchPage=await touch.newPage();await touchPage.goto(base+'/ads', {waitUntil:'networkidle'});
  ...
  console.log(`D1 Ads browser/HTTP: ${checks} checks passed; ${screenshots} screenshots at ${widths.join(', ')}px.`);
} catch(error) {
  ...
  console.error('D1 Ads browser acceptance failed:',String(error?.message||error).split('\n').filter(line=>!/cookie:|authorization:|token=/i.test(line)).join('\n'));process.exitCode=1;

...
try{
 ...
 for(const [template,service,channel,title] of [['ghl_campaign',ghl,'email','N3A GHL persisted draft'],['social',social,'instagram','N3A Social persisted draft']]){
  ...
  check(template+' persisted context keeps distinct Client and Service names',persisted.data.report.clientName==='N3A Synthetic Reports'&&persisted.data.report.serviceName===(template==='ghl_campaign'?'GHL':'Social Media Management')&&persisted.data.report.packageName==='Synthetic QA service');
  ...
  check(template+' saved preview shows its actual Client',await owner.page.locator('.bo-report-context > div').filter({has:owner.page.locator('dt').getByText('Client',{exact:true})}).locator('dd').innerText()==='N3A Synthetic Reports');
 ...
 for(const reportId of created){
  ...
  check('publication snapshot preserves canonical Client and Service context '+reportId,entry.snapshot.clientName==='N3A Synthetic Reports'&&entry.snapshot.serviceName===(entry.snapshot.templateId==='ghl_campaign'?'GHL':'Social Media Management')&&entry.snapshot.packageName==='Synthetic QA service');

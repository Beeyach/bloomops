...
const tmp=mkdtempSync('/tmp/bloomops-n3a-browser-'),configPath=join(tmp,'wrangler.jsonc');
...
try{
 ...
 const makeClient=async name=>{const r=await post(owner.ctx,'/api/bloomops/clients',{name,contactName:'Synthetic contact',contactEmail:'contact@example.test',timezone:'UTC',requestId:randomUUID(),userId:'ellen',workspaceId:'a'});assert.equal(r.status,201);return r.data.client.id;};
 ...
 const addService=async(client,slug)=>{const r=await post(owner.ctx,`/api/bloomops/clients/${client}/services`,{serviceTypeId:types.find(t=>t.slug===slug).id,packageName:'Synthetic QA service'});assert.equal(r.status,201);return r.data.service.id;};
 ...
 const metric=async(page,label,value)=>{await page.getByLabel(label+' availability',{exact:true}).selectOption('value');await page.getByLabel(label+' count',{exact:true}).fill(String(value));};
 const created=[];
 for(const [template,service,channel,title] of [['ghl_campaign',ghl,'email','N3A GHL persisted draft'],['social',social,'instagram','N3A Social persisted draft']]){
  await owner.page.goto(base+path+'/new',{waitUntil:'networkidle'});await owner.page.getByLabel('Purchased service',{exact:true}).selectOption(service);await owner.page.getByLabel('Report template',{exact:true}).selectOption(template);
  await owner.page.getByLabel('Report title',{exact:true}).fill(title);await owner.page.getByLabel('Channel',{exact:true}).selectOption(channel);await owner.page.getByLabel('Period start',{exact:true}).fill('2026-08-01');await owner.page.getByLabel('Period end',{exact:true}).fill('2026-08-31');await owner.page.getByLabel('Timezone',{exact:true}).fill('Australia/Sydney');await owner.page.getByLabel('Account',{exact:true}).fill('Synthetic source account');await owner.page.getByLabel('Campaign or content scope',{exact:true}).fill('One synthetic campaign');
  await owner.page.getByLabel('Report commentary',{exact:true}).fill('Synthetic narrative <script>window.n3aUnsafe=true</script>');
  if(template==='ghl_campaign'){await metric(owner.page,'Sent messages',32);await metric(owner.page,'Delivered messages',1);await owner.page.getByLabel('Sent messages source note',{exact:true}).fill('<img src=x onerror=alert(1)> synthetic source');}else{await metric(owner.page,'Followers at period start',100);await metric(owner.page,'Followers at period end',95);await metric(owner.page,'Views',0);}
  ...
  check(template+' saved form reopens',await owner.page.getByLabel('Report title',{exact:true}).inputValue()===title);
  await owner.page.getByLabel('Report commentary',{exact:true}).fill('Updated synthetic narrative <script>window.n3aUnsafe=true</script>');await owner.page.getByRole('button',{name:'Save draft',exact:true}).click();await owner.page.waitForLoadState('networkidle');await owner.page.getByRole('link',{name:'Preview saved draft',exact:true}).waitFor();
 ...
 await admin.page.goto(base+path+'/'+id,{waitUntil:'networkidle'});await admin.page.getByLabel('Report commentary',{exact:true}).fill('Do not lose this conflicting edit');
 ...
 await admin.page.getByRole('button',{name:'Save draft',exact:true}).click();await admin.page.getByRole('alert').filter({hasText:'saved draft changed'}).waitFor();check('concurrent browser edit reports conflict and preserves input',await admin.page.getByLabel('Report commentary',{exact:true}).inputValue()==='Do not lose this conflicting edit');
 ...
 admin.page.once('dialog',d=>d.accept());await admin.page.getByRole('link',{name:'Reopen saved draft',exact:true}).click();await admin.page.waitForLoadState('networkidle');check('explicit conflict recovery loads saved draft',await admin.page.getByLabel('Report commentary',{exact:true}).inputValue()==='Winning saved version');
 await owner.page.goto(base+path+'/'+id,{waitUntil:'networkidle'});await owner.page.getByLabel('Report commentary',{exact:true}).fill('Recovered network edit');let fail=true;await owner.page.route('**/api/bloomops/clients/*/reports/*',route=>{if(route.request().method()==='PUT'&&fail){fail=false;return route.fulfill({status:503,contentType:'application/json',body:'{"error":"Synthetic temporary failure"}'});}return route.continue();});
 await owner.page.getByRole('button',{name:'Save draft',exact:true}).click();await owner.page.getByRole('alert').filter({hasText:'Synthetic temporary failure'}).waitFor();check('failed save retains attempted input',await owner.page.getByLabel('Report commentary',{exact:true}).inputValue()==='Recovered network edit');await owner.page.getByRole('button',{name:'Retry save',exact:true}).click();await owner.page.waitForLoadState('networkidle');await owner.page.unroute('**/api/bloomops/clients/*/reports/*');check('exact retry saves after network error',(await get(owner.ctx,api+'/'+id)).data.report.commentary==='Recovered network edit');

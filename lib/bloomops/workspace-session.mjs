  ...
  query.session = new Proxy(session, { get(target, key) {
    ...
    return async options => {
      ...
      const result = await target.findFirst({ ...options, extras: session => ({
        boWorkspace: sql`(${workspaceAccessQuery(db, session.userId, { selection,
...
export function readWorkspaceIdentity(db, work) {
  return reads.run({ binding: db.$client, capture: null }, async () => {

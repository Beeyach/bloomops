import { loadInternalContentResource } from './content-internal-access.mjs';
...
export const contentAccess = (req,id,action='content.view') => workQueryAccess(req,requireAuthorized(req,{action,resource:access=>action.startsWith('content.approval.')?loadContentResource(access.db,access.actor,id):loadInternalContentResource(access.db,access.actor,id)}));
...
export async function contentBody(req,create=false) {
  const body=await readBody(req), allowed=[...CONTENT_DETAIL_FIELDS,...create?['platforms']:[],create?'requestId':'expectedRevision'];

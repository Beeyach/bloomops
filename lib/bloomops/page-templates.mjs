...
export async function createPageFromTemplate(db,actor,input){
 ...
 const result=await createTreePage(db,actor,{workspaceId:actor.workspaceId,requestId:pageRequestId},{snapshot:{title:source.title,body:source.body,condition,statements:id=>[
  ...
  complete(db,actor,sql`EXISTS(SELECT 1 FROM bloomops_pages WHERE workspace_id=${actor.workspaceId} AND id=${id})`,sql`EXISTS(SELECT 1 FROM bloomops_page_template_creations WHERE workspace_id=${actor.workspaceId} AND page_id=${id}) AND EXISTS(SELECT 1 FROM bloomops_page_settings WHERE workspace_id=${actor.workspaceId} AND page_id=${id}) AND EXISTS(SELECT 1 FROM bloomops_page_locations WHERE workspace_id=${actor.workspaceId} AND page_id=${id}) AND EXISTS(SELECT 1 FROM activity_events WHERE workspace_id=${actor.workspaceId} AND subject_id=${id} AND subject_type='page' AND event_type='PAGE_CREATED')`),
 ]}});

import { adsContentParentCondition, adsParentCondition, internalContentReadCondition, internalContentWriteCondition, loadAdsParentResource, loadInternalContentResource } from './content-internal-access.mjs';
...
export async function getInternalContent(db, actor, contentId) {
  const [row] = await joined(db,{...fields,...contextFields,productionArea:c.productionArea,adsProjectId:c.adsProjectId,platforms:contentPlatformSelection()})
    .where(and(internalContentReadCondition(actor),eq(c.id,String(contentId)))).limit(1);
  if (!row) return null;
  if (row.productionArea === 'social') { const {productionArea,adsProjectId,...social}=row; return social; }
  return row;
}
...
export const createContent = (db,options) => createInContext(db,{...options,adsProjectId:null});
export const createAdsContent = (db,{projectId,...options}) => typeof projectId==='string' && projectId.length>0 ? createInContext(db,{...options,adsProjectId:projectId}) : Promise.resolve(missing());
async function createInContext(db,{actor,clientId,serviceEngagementId=null,adsProjectId=null,input,requestId,now=new Date()}) {
  const ads=adsProjectId !== null;
  const resource=await (ads?loadAdsParentResource(db,actor,adsProjectId):loadContentParentResource(db,actor,clientId,serviceEngagementId));
  if (ads && resource) { clientId=resource.clientId; serviceEngagementId=resource.serviceEngagementId; }
  ...
  if (ads && (Object.keys(CONTENT_FLAG_DEFAULTS).some(k=>has(details,k)) || details.visibility==='client')) return invalid({form:'Use internal Ads creative details.'});
  const normalized=normalizeContent(ads?{type:'ad_creative',...details}:details); if (!normalized.ok) return normalized;
  ...
  const retry=async()=>{
    const [row]=await db.select({id:c.id,metadata:a.metadataJson}).from(c).innerJoin(a,and(eq(a.workspaceId,c.workspaceId),eq(a.subjectId,c.id),eq(a.subjectType,'content'),eq(a.eventType,'CONTENT_CREATED')))
      .where(and(ads?internalContentWriteCondition(actor):contentReadCondition(actor),eq(c.clientId,String(clientId)),eq(c.creationRequestId,requestId))).limit(1);
    if (!row) return null;
    const initial=JSON.parse(row.metadata);
    return (initial.productionArea || 'social')===(ads?'ads':'social') && (initial.adsProjectId || null)===adsProjectId && initial.serviceEngagementId===serviceEngagementId && CONTENT_DETAIL_FIELDS.every(k=>initial.details[k]===patch[k]) && samePlatforms(initial.platforms||[],platforms)?success(row.id,true):conflict();
  };
  const previous=await retry(); if (previous) return previous;
  const parentGuard=()=>ads?sql`EXISTS (SELECT 1 FROM projects WHERE ${schema.projects.id}=${adsProjectId} AND ${schema.projects.clientId}=${String(clientId)} AND ${schema.projects.serviceEngagementId}=${serviceEngagementId} AND ${adsParentCondition(actor,patch.visibility)})`:contentParentCondition(actor,String(clientId),serviceEngagementId,patch.visibility);
  const [eligible]=await db.select({id:schema.clients.id}).from(schema.clients).where(and(eq(schema.clients.id,String(clientId)),parentGuard(),ownerValid(actor,patch.ownerMembershipId))).limit(1);
  if (!eligible) return invalid({form:'Choose an available parent, owner and visibility.'});
  const id=newId(),iso=now.toISOString(), values={id,workspaceId:actor.workspaceId,clientId:String(clientId),serviceEngagementId,productionArea:ads?'ads':'social',adsProjectId,creationRequestId:requestId,...patch,stage:'idea',stageContext:null,publishedAt:null,revision:1,createdAt:iso,updatedAt:iso};
  ...
  const results=await db.batch([
    // Avoid attempting an insert against a request in any production area. This
    // is checked in the write statement, including collisions after the retry read.
    db.insert(c).select(db.select(selected).from(schema.clients).where(and(eq(schema.clients.id,String(clientId)),parentGuard(),ownerValid(actor,patch.ownerMembershipId),
      sql`NOT EXISTS (SELECT 1 FROM content_items prior WHERE prior.workspace_id=${actor.workspaceId} AND prior.client_id=${String(clientId)} AND prior.creation_request_id=${requestId})`)))
    ...
    activityForMutation(db,c,eq(c.id,id),event(actor,values,'CONTENT_CREATED',{details:patch,serviceEngagementId,platforms,...ads?{productionArea:'ads',adsProjectId}:{}},now)),
  ]);
...
export async function updateContent(db,{actor,contentId,input,expectedRevision,now=new Date()}) {
  const resource=await loadInternalContentResource(db,actor,contentId,{write:true}); if (!resource) return missing();
  const decision=evaluate(actor,{action:'content.manage',resource}); if (!decision.allowed) return {ok:false,reason:decision.outcome};
  const current=await getInternalContent(db,actor,contentId); if (!current) return missing();
  if (!Number.isSafeInteger(expectedRevision)||expectedRevision<1) return invalid({form:'Refresh this Content before saving.'});
  if (current.productionArea==='ads' && (!object(input) || Object.keys(CONTENT_FLAG_DEFAULTS).some(k=>has(input,k)) || input.visibility==='client')) return invalid({form:'Use internal Ads creative details.'});
  ...
  const desired=current.productionArea==='ads'?adsContentParentCondition(actor,patch.visibility||current.visibility):contentParentCondition(actor,c.clientId,c.serviceEngagementId,patch.visibility||current.visibility);
  const reviewGuard=Object.keys(patch).some(k=>REVIEW_FROZEN_FIELDS.includes(k))?noRequestedApproval():sql`1`;
  const condition=and(internalContentWriteCondition(actor),eq(c.id,String(contentId)),eq(c.revision,expectedRevision),desired,ownerGuard,reviewGuard);
  ...
  if (!eligible) {
    ...
    const live=await getInternalContent(db,actor,contentId);
    if (!live || live.revision!==expectedRevision) return conflict();
    if(Object.keys(patch).some(k=>REVIEW_FROZEN_FIELDS.includes(k))){
      const [editable]=await db.select({id:c.id}).from(c).where(and(internalContentWriteCondition(actor),eq(c.id,String(contentId)),noRequestedApproval())).limit(1);
...
export async function contentOptions(db,actor,{query=''}={}) {
  ...
  const [clients,services,members,platforms]=await Promise.all([clientChoices,serviceChoices,

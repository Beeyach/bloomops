...
export async function createContent(db,{actor,clientId,serviceEngagementId=null,input,requestId,now=new Date()}) {
  ...
  const id=newId(),iso=now.toISOString(), values={id,workspaceId:actor.workspaceId,clientId:String(clientId),serviceEngagementId,productionArea:'social',adsProjectId:null,creationRequestId:requestId,...patch,stage:'idea',stageContext:null,publishedAt:null,revision:1,createdAt:iso,updatedAt:iso};

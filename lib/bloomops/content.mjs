...
export async function contentOptions(db,actor,{query=''}={}) {
  ...
  const [clients,services]=await Promise.all([
    db.select({clientId:cl.id,clientName:cl.name,canRestrict:contentAssignmentCondition(actor,cl.id,null)}).from(cl)
      .where(and(contentParentCondition(actor,cl.id,null),match)).orderBy(asc(cl.name),asc(cl.id)).limit(CONTENT_PAGE_SIZE+1),
    db.select({clientId:cl.id,clientName:cl.name,serviceEngagementId:se.id,serviceName:st.name,packageName:se.packageName,canRestrict:contentAssignmentCondition(actor,cl.id,se.id)}).from(se)

...
import { CONTENT_DETAIL_FIELDS, CONTENT_FLAG_DEFAULTS, CONTENT_PAGE_SIZE, CONTENT_QUERY_FIELDS, CONTENT_TEXT_LIMITS, CONTENT_TYPES } from './content-values.mjs';
...
const fields = Object.fromEntries(['id','clientId','serviceEngagementId',...CONTENT_DETAIL_FIELDS,'stage','publishedAt','revision','createdAt','updatedAt'].map(k => [k,c[k]]));
...
export async function getContent(db, actor, contentId) {
  const [row] = await joined(db,{...fields,...contextFields}).where(and(contentReadCondition(actor),eq(c.id,String(contentId)))).limit(1);
  return row || null;
}
...
export async function createContent(db,{actor,clientId,serviceEngagementId=null,input,requestId,now=new Date()}) {
  ...
  const normalized=normalizeContent(input); if (!normalized.ok) return normalized;
  ...
  const retry=async()=>{
    ...
    return initial.serviceEngagementId===serviceEngagementId && CONTENT_DETAIL_FIELDS.every(k=>initial.details[k]===patch[k])?success(row.id,true):conflict();
  };
  ...
  const id=newId(),iso=now.toISOString(), values={id,workspaceId:actor.workspaceId,clientId:String(clientId),serviceEngagementId,creationRequestId:requestId,...patch,stage:'idea',publishedAt:null,revision:1,createdAt:iso,updatedAt:iso};
  ...
  const results=await db.batch([
    ...
    activityForMutation(db,c,eq(c.id,id),event(actor,values,'CONTENT_CREATED',{details:patch,serviceEngagementId},now)),
  ]);
...
export async function updateContent(db,{actor,contentId,input,expectedRevision,now=new Date()}) {
  ...
  const condition=and(contentReadCondition(actor),eq(c.id,String(contentId)),eq(c.revision,expectedRevision),desired,ownerGuard);
  const [eligible]=await db.select({id:c.id}).from(c).where(condition).limit(1);
  if (!eligible) return invalid({form:'Choose an available owner and visibility.'});
  const results=await db.batch([
...
export function contentFilters(query={}) {
  ...
  for (const k of CONTENT_QUERY_FIELDS.filter(k=>k!=='page')) {
    if (query[k]!=null && (typeof query[k]!=='string'||query[k].length>100||/[\x00-\x1f\x7f]/.test(query[k]))) return invalid({form:'Choose a valid filter.'});
...
export async function listContent(db,actor,query={}) {
  ...
  for (const k of CONTENT_QUERY_FIELDS.filter(k=>k!=='page')) if(f[k])where.push(eq(c[k],f[k]));
  const selected={id:c.id,title:c.title,type:c.type,pillar:c.pillar,stage:c.stage,targetPublishDate:c.targetPublishDate,visibility:c.visibility,clientId:c.clientId,serviceEngagementId:c.serviceEngagementId,ownerMembershipId:c.ownerMembershipId,...contextFields};
...
export async function contentOptions(db,actor,{query=''}={}) {
  ...
  const clients=await db.select({clientId:cl.id,clientName:cl.name,canRestrict:contentAssignmentCondition(actor,cl.id,null)}).from(cl)
    .where(and(contentParentCondition(actor,cl.id,null),match)).orderBy(asc(cl.name),asc(cl.id)).limit(CONTENT_PAGE_SIZE+1);
  const services=await db.select({clientId:cl.id,clientName:cl.name,serviceEngagementId:se.id,serviceName:st.name,packageName:se.packageName,canRestrict:contentAssignmentCondition(actor,cl.id,se.id)}).from(se)
  ...
  const members=clients.length||services.length ? await db.select({membershipId:m.id,name:schema.user.name}).from(m).innerJoin(schema.user,eq(schema.user.id,m.userId))

...
import { CONTENT_STAGE_LABELS } from './content-values.mjs';
...
export function describeEvent(eventType, metadata = {}) {
  switch (eventType) {
    case 'RECORD_COMMENT_ADDED': return {title:'Comment posted',detail:null};
    case 'RECORD_COMMENT_EDITED': return {title:'Comment edited',detail:null};
    case 'RECORD_COMMENT_REMOVED': return {title:'Comment removed',detail:null};
    case 'RECORD_DISCUSSION_RESOLVED': return {title:'Discussion resolved',detail:null};
    case 'RECORD_DISCUSSION_REOPENED': return {title:'Discussion reopened',detail:null};
    case 'CONTENT_APPROVAL_REQUESTED': return {title:'Content approval requested',detail:metadata.contentTitle || null};
    case 'CONTENT_APPROVAL_APPROVED': return {title:'Content approved by Client',detail:metadata.contentTitle || null};
    case 'CONTENT_APPROVAL_CHANGES_REQUESTED': return {title:'Client requested Content changes',detail:metadata.contentTitle || null};
    case 'CONTENT_APPROVAL_WITHDRAWN': return {title:'Content approval withdrawn',detail:metadata.contentTitle || null};
    case 'CONTENT_STAGE_CHANGED': return { title: 'Content stage changed', detail: `${metadata.contentTitle || 'Content'}: ${CONTENT_STAGE_LABELS[metadata.from] || 'previous stage'} → ${CONTENT_STAGE_LABELS[metadata.to] || 'updated stage'}.` };
    case 'CONTENT_PLATFORMS_CHANGED': return { title: 'Content platforms changed', detail: metadata.contentTitle || null };
    ...
    case 'PROJECT_BLUEPRINT_GENERATED': return { title: 'Systems work generated', detail: metadata.projectName || null };
    ...
    case 'ONBOARDING_GUIDANCE_UPDATED': return { title: 'Onboarding instructions updated', detail: metadata.title || null };
...
export async function clientActivity(db, workspaceId, clientId, { limit = 60, actor = null } = {}) {
  ...
  const rows = await db
    .select({
      ...
      order: sql`activity_events.rowid`,
    })
    .from(a)
    ...
    .limit(limit);
  const contentFiles = actor?.workspaceId === workspaceId ? await contentFileActivityRows(db, actor, { clientId, limit }) : [];
  return [...rows, ...contentFiles].sort((a, b) => b.occurredAt.localeCompare(a.occurredAt) || b.order - a.order).slice(0, limit).map((row) => {

...
export async function inspectGoogleReplyBackfill(accessToken,context,{fetcher=fetch}={}){
 ...
 try{
  const listed=new Map(),tokens=new Set();let token,complete=false;
  ...
  return {status:'collected',hold:true,coverage:'unverified',from:new Date(after*1000).toISOString(),
   enumerationComplete:true,historyCaughtUp:false,observations:result.observations,unassigned:result.unassigned,nextHistoryId:null};

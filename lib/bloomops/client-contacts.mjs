...
export async function updateContact(db, { workspaceId, clientId, contactId, input, actorMembershipId = null, actorUserId = null, now = new Date() }) {
  ...
  const unchanged = sql`EXISTS (SELECT 1 FROM client_contacts WHERE workspace_id=${workspaceId} AND client_id=${clientId} AND id=${existing.id}
    AND name IS ${existing.name} AND email IS ${existing.email} AND phone IS ${existing.phone} AND title IS ${existing.title}
    AND is_primary=${existing.isPrimary ? 1 : 0} AND user_id IS ${existing.userId})`;
  // Lock and validate the contact snapshot before its detail/primary writes.
  const writes = [db.update(schema.clients).set({ health: sql`CASE WHEN ${unchanged} THEN health ELSE 'a11_contact_conflict' END` })
    .where(and(eq(schema.clients.workspaceId, workspaceId), eq(schema.clients.id, clientId)))];
  if (changed.length > 0) {
    writes.push(
      db
        .update(c)
  ...
  try {
    ...
    if (constraintMatches(err, /bloomops_clients_health_chk/)) return { ok: false, reason: 'conflict' };
    if (isEmailConflict(err)) return { ok: false, reason: 'duplicate_email', errors: { email: DUPLICATE_EMAIL } };
    throw err;
  }
...
export async function removeContact(db, { workspaceId, clientId, contactId, actorMembershipId = null, actorUserId = null, now = new Date() }) {
  ...
  const c = schema.clientContacts;
  try {
    const condition = and(eq(c.workspaceId, workspaceId), eq(c.clientId, clientId), eq(c.id, existing.id), sql`${c.userId} IS NULL`);
    const [, rows] = await db.batch([
      activityForMutation(db, c, condition, {
        workspaceId, clientId, subjectType: 'client_contact', subjectId: existing.id,
        eventType: ACTIVITY.CLIENT_CONTACT_REMOVED, actorMembershipId, actorUserId,
        metadata: { name: existing.name, wasPrimary: Boolean(existing.isPrimary) }, occurredAt: iso,
      }),
      db.delete(c).where(condition).returning({ id: c.id }),
    ]);

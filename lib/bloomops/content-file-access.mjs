...
export function contentFileVisibilityCondition(actor, visibility = f.visibility) {
  // Clients can never read Ads assets. Avoid expanding the entire internal Ads
  // authority tree inside the already-nested portal file projection (D1 depth).
  if (actor?.role === 'client') return sql`(${c.productionArea}='social' AND ${visibility}='client')`;

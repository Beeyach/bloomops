...
export async function getProspectReplies(db,actor,env,id){
 ...
 const [resolution]=await db.select({at:v.occurredAt,recordedAt:v.createdAt,note:sql`json_extract(${v.metadataJson},'$.note')`}).from(v).where(and(eq(v.workspaceId,actor.workspaceId),eq(v.subjectType,'prospect'),eq(v.subjectId,id),eq(v.eventType,'PROSPECT_REPLY_RESOLVED'),auth(actor))).orderBy(desc(v.createdAt),desc(v.id)).limit(1);
 ...
 return {workspaceId:actor.workspaceId,prospectId:id,deliveryId:row.receipt.id,businessName:row.businessName,website:row.website,
  ...
  resolution:resolution?{...resolution,current:state?.holdState==='held'&&Date.parse(resolution.recordedAt)>=Date.parse(state.updatedAt)}:null,
...
const resolutionRecordedAt=actor=>sql`(SELECT rv.created_at FROM activity_events rv WHERE rv.workspace_id=${actor.workspaceId} AND rv.subject_type='prospect' AND rv.subject_id=${r.prospectId} AND rv.event_type='PROSPECT_REPLY_RESOLVED' ORDER BY rv.created_at DESC,rv.id DESC LIMIT 1)`;
const resolutionAt=actor=>sql`(SELECT rv.occurred_at FROM activity_events rv WHERE rv.workspace_id=${actor.workspaceId} AND rv.subject_type='prospect' AND rv.subject_id=${r.prospectId} AND rv.event_type='PROSPECT_REPLY_RESOLVED' ORDER BY rv.created_at DESC,rv.id DESC LIMIT 1)`;
const resolutionNote=actor=>sql`(SELECT json_extract(rv.metadata_json,'$.note') FROM activity_events rv WHERE rv.workspace_id=${actor.workspaceId} AND rv.subject_type='prospect' AND rv.subject_id=${r.prospectId} AND rv.event_type='PROSPECT_REPLY_RESOLVED' ORDER BY rv.created_at DESC,rv.id DESC LIMIT 1)`;
const unresolvedDecision=actor=>sql`(${r.holdState}='stopped' OR ${resolutionRecordedAt(actor)} IS NULL OR julianday(${resolutionRecordedAt(actor)})<julianday(${r.updatedAt}))`;
export async function prospectReplyOverview(db,actor,page=1,handledPage=1){
 if(!allowed(actor)||![page,handledPage].every(value=>Number.isInteger(value)&&value>=1&&value<=10000))return null;
 const base=and(eq(d.workspaceId,actor.workspaceId),eq(d.state,'accepted'),inArray(r.holdState,['held','stopped']),auth(actor)),pending=and(base,unresolvedDecision(actor)),handled=and(base,eq(r.holdState,'held'),sql`NOT (${unresolvedDecision(actor)})`);
 const [rows,counts,handledRows,handledCount]=await readTogether(db,db=>Promise.all([
  db.select({id:p.id,businessName:p.businessName,website:p.website,holdState:r.holdState,checkStatus:r.checkStatus,stopReason:r.stopReason,stopNote:r.stopNote})
   ...
   .where(pending).orderBy(desc(r.updatedAt),desc(d.acceptedAt),desc(d.id)).limit(21).offset((page-1)*20),
  db.select({holdState:r.holdState,checkStatus:r.checkStatus,stopReason:r.stopReason,count:sql`count(*)`})
   ...
   .where(pending).groupBy(r.holdState,r.checkStatus,r.stopReason),
  db.select({id:p.id,businessName:p.businessName,website:p.website,holdState:r.holdState,checkStatus:r.checkStatus,resolutionAt:resolutionAt(actor),resolutionNote:resolutionNote(actor)})
   .from(d).innerJoin(p,and(eq(p.workspaceId,d.workspaceId),eq(p.id,d.prospectId))).innerJoin(r,and(eq(r.workspaceId,d.workspaceId),eq(r.prospectId,d.prospectId),eq(r.deliveryId,d.id)))
   .where(handled).orderBy(desc(resolutionRecordedAt(actor)),desc(d.id)).limit(21).offset((handledPage-1)*20),
  db.select({count:sql`count(*)`}).from(d).innerJoin(r,and(eq(r.workspaceId,d.workspaceId),eq(r.prospectId,d.prospectId),eq(r.deliveryId,d.id))).where(handled),
 ]));
 return {rows:rows.slice(0,20),groups:counts.map(row=>({...row,count:Number(row.count)})),page,more:rows.length>20,total:counts.reduce((sum,row)=>sum+Number(row.count),0),handledRows:handledRows.slice(0,20),handledPage,handledMore:handledRows.length>20,handledTotal:Number(handledCount[0]?.count||0)};
}

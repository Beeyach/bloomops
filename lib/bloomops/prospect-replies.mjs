...
export async function prospectReplyOverview(db,actor){
 if(!allowed(actor))return null;
 const rows=await db.select({id:p.id,businessName:p.businessName,website:p.website,holdState:r.holdState,checkStatus:r.checkStatus})
  .from(d).innerJoin(p,and(eq(p.workspaceId,d.workspaceId),eq(p.id,d.prospectId)))
  .leftJoin(r,and(eq(r.workspaceId,d.workspaceId),eq(r.prospectId,d.prospectId),eq(r.deliveryId,d.id)))
  .where(and(eq(d.workspaceId,actor.workspaceId),eq(d.state,'accepted'),auth(actor))).orderBy(desc(d.acceptedAt),desc(d.id)).limit(21);

...
export function normalizeMultiHandoff(raw,workspaceId){
 ...
 for(const item of raw.services){
  ...
  const checked=normalizeConversionInput({workspaceId:raw.workspaceId,mode:raw.mode,clientId:raw.clientId,...item},workspaceId);if(!checked.ok)return {...checked,errors:{...checked.errors,services:{[item.serviceTypeId]:checked.errors}}};inputs.push(checked.input);
 }

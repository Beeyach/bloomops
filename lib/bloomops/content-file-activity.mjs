...
export async function contentFileActivityRows(db, actor, { clientId = null, contentId = null, limit = 60 } = {}) {
  ...
  return db.select({ id: a.id, eventType: a.eventType, metadataJson: a.metadataJson, occurredAt: a.occurredAt,
    ...
    .where(and(eq(a.workspaceId, actor?.workspaceId || ''), clientId ? eq(a.clientId, String(clientId)) : undefined,
      contentFileReadCondition(actor, { contentId })))
    .orderBy(desc(a.occurredAt), sql`activity_events.rowid DESC`).limit(Math.min(60, Math.max(1, limit)));
}

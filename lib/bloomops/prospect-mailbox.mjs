...
const c=schema.prospectRecoveryCollections,m=schema.prospectRecoveryMessages,x=schema.prospectRecoveryScopes,k=schema.prospectMonitoringCheckpoints;
...
async function runMailbox(db,actor,env,sessionId,input,{fetcher=fetch,clock=()=>new Date()}={},recovery=false,checkpoint=false){
 ...
 if(recovery&&result.status==='collected'){
  ...
  batch.push(insertSelected(db,x,{workspaceId:actor.workspaceId,runId,...result.scope,createdAt:completed},s,finish()));

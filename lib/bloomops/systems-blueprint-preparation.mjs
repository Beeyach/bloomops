...
export async function verifiedBlueprintDefinition(definitionJson,definitionHash) {
  ...
  if(encoded.definition.blueprintKey!=='ghl_build'||encoded.definitionJson!==definitionJson||encoded.definitionHash!==definitionHash)throw new SystemsBlueprintError('invalid_definition');
...
export async function systemsBlueprintOptions(db,{actor,projectId}={}) {
  ...
  return {ok:true,expected:generationPreconditions(source.context),components:source.definition.components.map(component=>({key:component.logicalKey,label:titles.get(component.actionKey)}))};
}

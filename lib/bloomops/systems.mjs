...
import { homeDeliverables, listProjectSummaries } from './work-projections.mjs';
...
export async function systemsProjection(db, actor, input = {}, { now = new Date() } = {}) {
  ...
  const [clients, services] = await Promise.all([facet(db, actor, 'clients', filters), facet(db, actor, 'services', filters)]);
  // Missing, foreign, revoked and incompatible filter IDs share one response.
  ...
  const [projects, deliverables] = await Promise.all([
    listProjectSummaries(db, actor, { condition, attentionFirst: true, page: filters.page, limit: SYSTEMS_PAGE_SIZE, now }),
    homeDeliverables(db, actor, { condition, now }),
  ]);

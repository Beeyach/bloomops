// D1 Systems is a read of the Work Core. No specialist state or write path.
...
import { homeDeliverables, listProjectSummaries, projectToday } from './work-projections.mjs';

export const SYSTEMS_PAGE_SIZE = 50;
export const SYSTEMS_FACET_LIMIT = 200;
...
export function systemsProjectCondition() {
  return sql`EXISTS (SELECT 1 FROM service_engagements se
    ...
    WHERE se.id=${p.serviceEngagementId} AND se.workspace_id=${p.workspaceId}
      AND se.client_id=${p.clientId} AND dept.slug='systems')`;
}

export function normalizeSystemsFilters(input = {}) {
...
// Facets come only from readable Systems Projects, including closed work.
...
async function facet(db, actor, kind, { clientId, serviceEngagementId }) {
  ...
  const scope = and(projectReadCondition(actor), systemsProjectCondition(),
  ...
  const rows = await query().limit(SYSTEMS_FACET_LIMIT + 1);
  const items = rows.slice(0, SYSTEMS_FACET_LIMIT);
  if (selected && !items.some(row => row.id === selected)) {
    const [row] = await query(eq(id, selected)).limit(1);
    if (!row) return null;
    items.push(row);
  }
  return { items, hasMore: rows.length > SYSTEMS_FACET_LIMIT };
}

export function systemsProjection(db, actor, input = {}, options = {}) {
  return timedNavigation('systems', () => readTogether(db, db => readSystemsProjection(db, actor, input, options)));
}
async function readSystemsProjection(db, actor, input, { now = new Date() } = {}) {
  const normalized = normalizeSystemsFilters(input);
  ...
  const condition = and(systemsProjectCondition(), filters.clientId ? eq(p.clientId, filters.clientId) : undefined,
  ...
  const readData = () => {
    const today = projectToday(db, actor, now, condition);
    return Promise.all([
      listProjectSummaries(db, actor, { condition, attentionFirst: true, page: filters.page, limit: SYSTEMS_PAGE_SIZE, now, today }),
  ...
  const [[clients, services], data] = await Promise.all([
    Promise.all([facet(db, actor, 'clients', filters), facet(db, actor, 'services', filters)]),
    !filters.clientId && !filters.serviceEngagementId ? readData() : null,
  ]);
  if (!clients || !services) return { ok: false };
  const [projects, deliverables] = data || await readData();

...
export async function postPageComment(db,actor,id,input){
 ...
 const threadId=input.threadId||input.requestId,now=new Date().toISOString(),threadGuard=and(eq(t.id,threadId),eq(t.workspaceId,actor.workspaceId),eq(t.pageId,id),eq(t.resolved,0),eq(t.revision,input.expectedRevision||1));
 ...
 await db.batch([
  activityForMutation(db,p,and(eq(p.id,id),guard),{workspaceId:actor.workspaceId,eventType:'PAGE_COMMENT_ADDED',subjectType:'page',subjectId:id,actorMembershipId:actor.membershipId,actorUserId:actor.userId,occurredAt:now}),mutation,

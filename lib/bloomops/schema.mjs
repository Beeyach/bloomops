...
// P3 follow-up automation is an explicit, approval-bound sequence. The
// scheduler is dormant unless a deployment separately enables its guarded
// entry point; these records never enable a provider on their own.
export const prospectFollowupSequences=sqliteTable('prospect_followup_sequences',{
 id:id(),workspaceId:text('workspace_id').notNull().references(()=>workspaces.id),prospectId:text('prospect_id').notNull(),deliveryId:text('delivery_id').notNull(),approvalId:text('approval_id').notNull(),identityId:text('identity_id').notNull(),
 state:text('state').notNull(),totalMessages:integer('total_messages').notNull(),nextMessage:integer('next_message'),secondScheduledAt:text('second_scheduled_at'),thirdScheduledAt:text('third_scheduled_at'),
 accountEmail:text('account_email').notNull(),connectionRevision:integer('connection_revision').notNull(),senderRevision:integer('sender_revision').notNull(),replyRevision:integer('reply_revision').notNull(),
 activatedByMembershipId:text('activated_by_membership_id').notNull(),activatorUpdatedAt:text('activator_updated_at').notNull(),lastReason:text('last_reason'),createdAt:createdAt(),updatedAt:updatedAt(),
},t=>[uniqueIndex('prospect_followup_sequences_ws_id_uq').on(t.workspaceId,t.id),uniqueIndex('prospect_followup_sequences_delivery_uq').on(t.workspaceId,t.deliveryId),index('prospect_followup_sequences_due_idx').on(t.state,t.secondScheduledAt,t.thirdScheduledAt),
 foreignKey({name:'prospect_followup_sequences_delivery_fk',columns:[t.workspaceId,t.prospectId,t.deliveryId],foreignColumns:[prospectDeliveries.workspaceId,prospectDeliveries.prospectId,prospectDeliveries.id]}),sameWorkspace('prospect_followup_sequences_approval_fk',t,prospectOutreachApprovals,t.approvalId),sameWorkspace('prospect_followup_sequences_identity_fk',t,prospectDeliveryIdentities,t.identityId),sameWorkspace('prospect_followup_sequences_actor_fk',t,workspaceMemberships,t.activatedByMembershipId),
 check('prospect_followup_sequences_state_chk',sql`state IN ('running','held','stopped','failed','complete')`),check('prospect_followup_sequences_count_chk',sql`total_messages BETWEEN 1 AND 3 AND (next_message IS NULL OR next_message BETWEEN 2 AND total_messages)`),check('prospect_followup_sequences_schedule_chk',sql`(total_messages=1 AND second_scheduled_at IS NULL AND third_scheduled_at IS NULL) OR (total_messages=2 AND second_scheduled_at IS NOT NULL AND third_scheduled_at IS NULL) OR (total_messages=3 AND second_scheduled_at IS NOT NULL AND third_scheduled_at IS NOT NULL)`),check('prospect_followup_sequences_revision_chk',sql`connection_revision>=1 AND sender_revision>=1 AND reply_revision>=1`),
]);
export const prospectFollowupAttempts=sqliteTable('prospect_followup_attempts',{
 id:id(),workspaceId:text('workspace_id').notNull().references(()=>workspaces.id),sequenceId:text('sequence_id').notNull(),prospectId:text('prospect_id').notNull(),messageNumber:integer('message_number').notNull(),messageId:text('message_id').notNull(),scheduledAt:text('scheduled_at').notNull(),state:text('state').notNull(),
 attemptedAt:text('attempted_at'),accountEmail:text('account_email'),providerMessageId:text('provider_message_id'),providerThreadId:text('provider_thread_id'),acceptedAt:text('accepted_at'),createdAt:createdAt(),updatedAt:updatedAt(),
},t=>[uniqueIndex('prospect_followup_attempts_ws_id_uq').on(t.workspaceId,t.id),uniqueIndex('prospect_followup_attempts_step_uq').on(t.workspaceId,t.sequenceId,t.messageNumber),uniqueIndex('prospect_followup_attempts_message_uq').on(t.messageId),sameWorkspace('prospect_followup_attempts_sequence_fk',t,prospectFollowupSequences,t.sequenceId),sameWorkspace('prospect_followup_attempts_prospect_fk',t,prospects,t.prospectId),
 check('prospect_followup_attempts_number_chk',sql`message_number IN (2,3)`),check('prospect_followup_attempts_state_chk',sql`state IN ('scheduled','submitting','uncertain','accepted','cancelled')`),check('prospect_followup_attempts_outcome_chk',sql`(state='scheduled' OR (attempted_at IS NOT NULL AND account_email IS NOT NULL)) AND (state!='accepted' OR (provider_message_id IS NOT NULL AND provider_thread_id IS NOT NULL AND accepted_at IS NOT NULL))`),
]);

...
export function changeDraftContext({logout=false}={}){
 try{
  if(logout)for(const key of Object.keys(localStorage))if((key.startsWith(SHEET_DRAFT_PREFIX)||key.startsWith(PROFILE_DRAFT_PREFIX)||key.startsWith(PAGE_DRAFT_PREFIX)))localStorage.removeItem(key);

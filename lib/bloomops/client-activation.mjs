...
import { onboardingGenerationReadiness, prepareOnboardingPlan, prepareOnboardingWrites } from './onboarding-generation.mjs';
...
export async function activationReadiness(db, actor, clientId) {
  ...
  actor = allowed.actor;
  const open = await onboardingGenerationReadiness(db, {workspaceId:actor.workspaceId,clientId});
  const prepared = open.ok ? await prepareActivation(db, actor, clientId) : open;

...
export async function activationSummary(db, workspaceId, clientId) {
  ...
  return row
    ? {
        ...
        retryAvailable:
          ...
          (row.deliveryStatus !== 'sending' || Date.parse(row.deliveryLeaseUntil) <= Date.now()),
      }
    : null;
}
...
export async function activateClient(
  db,
  { actor, clientId, mailer, appUrl, workspaceName, inviterName = null, now = new Date(), retryOnly = false },
) {
  ...
  if (!activation) {
    ...
    if (!prepared.ok) {
      ...
      const stillValid = sql`relationship_status = 'draft'
...
async function deliver(
  ...
  if (activation.deliveryStatus === 'sent') return result(activation, { alreadyActivated });
  const iso = now.toISOString(),
    ...
    .where(
      and(
        ...
        or(
          ...
          and(eq(a.deliveryStatus, 'sending'), lt(a.deliveryLeaseUntil, iso)),
        ),
      ),
    )
    .returning();
    ...
    await db.batch([
      db.insert(schema.activityEvents).select(
        db
          ...
          .from(a)
          .where(and(eq(a.id, activation.id), currentAttempt)),
      ),
      db
        .update(a)
        .set({ deliveryStatus: 'sent', deliveredAt: iso, deliveryLeaseUntil: null, updatedAt: iso })
        .where(owned),
    ]);
    return result(await find(db, actor.workspaceId, activation.clientId), { alreadyActivated });
  } catch {

...
export async function systemsBlueprintSetupOptions(db, { actor } = {}) {
  if (!await authorized(db, actor)) return failure('forbidden');
  ...
  return { ok: true, serviceTypes: rows.map(row => ({ id: row.id, name: row.name,
    ...
    available: !row.bindingId || (row.templateKind === 'systems' && row.templateSlug === DEFAULT_SLUG),
  })) };
}

export async function configureGhlBlueprint(db, { actor, input, now = new Date() } = {}) {
  if (!await authorized(db, actor)) return failure('forbidden');
  ...
  const existing = await defaultSnapshot(db, actor);
  if (current && current.templateId !== existing?.template.id) return failure('other_blueprint');
  // Enable verifies/provisions canonical GHL v1. Disable needs only the existing
  // default identity and never repairs/provisions a missing or invalid version.
  const installed = enabled ? await provisionGhlBlueprint(db, { actor, now }) : { ok: true, templateId: current.templateId };
  ...
  if (enabled) {
    ...
    requiredTarget = sql`EXISTS (SELECT 1 FROM templates gt JOIN template_versions gv
      ...
      AND gt.kind='systems' AND gt.slug=${DEFAULT_SLUG} AND gt.active=1

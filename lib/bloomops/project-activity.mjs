...
export async function projectActivity(db, actor, projectId) {
  ...
  const rows = await db.select({ id: a.id, eventType: a.eventType, metadata: a.metadataJson,
    ...
    .innerJoin(p, and(eq(p.workspaceId, a.workspaceId), sql`(${a.subjectType}='project' AND ${p.id}=${a.subjectId}) OR (${a.subjectType}='milestone' AND ${p.id}=${child.projectId}) OR (${a.subjectType}='action' AND ${p.id}=${action.projectId}) OR (${a.subjectType}='deliverable' AND ${p.id}=${deliverable.projectId})`))
    .leftJoin(m, and(eq(m.workspaceId, a.workspaceId), eq(m.id, a.actorMembershipId)))
    .leftJoin(schema.user, eq(schema.user.id, m.userId))
    .where(and(eq(p.id, String(projectId)), projectReadCondition(actor), sql`(${a.subjectType}='project' OR (${a.subjectType}='milestone' AND ${milestoneReadCondition(actor)}) OR (${a.subjectType}='action' AND ${actionReadCondition(actor)}) OR (${a.subjectType}='deliverable' AND ${deliverableReadCondition(actor)}))`))

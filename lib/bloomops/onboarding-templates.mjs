...
export async function createOnboardingVersion(db, { workspaceId, templateId, definition, notes = null, createdByMembershipId = null, now = new Date(), accessCondition = sql`1` }) {
  ...
  try {
    ...
    const [version] = await insertSelected(db, v, {id:crypto.randomUUID(),
    ...
    },t,and(eq(t.id,templateId),eq(t.workspaceId,workspaceId),accessCondition)).returning();
    return version ? { ok: true, version } : failure('conflict');
  } catch (e) {
...
export async function publishOnboardingVersion(db, { workspaceId, versionId, now = new Date(), accessCondition = sql`1` }) {
  ...
  const stillDraft = sql`${accessCondition} AND EXISTS (SELECT 1 FROM template_versions target WHERE target.workspace_id = ${workspaceId} AND target.id = ${versionId} AND target.status = 'draft')`;
  await db.batch([
    ...
    db.update(v).set({ status: 'published', publishedAt: iso, updatedAt: iso }).where(and(eq(v.workspaceId, workspaceId), eq(v.id, versionId), eq(v.status, 'draft'), accessCondition)),
  ]);

...
export async function currentPublishedVersion(db, workspaceId, slug) {
  if (!workspaceId || !ONBOARDING_CATEGORIES.includes(slug)) return null;
  const rows = await db.select({ version: v }).from(v)
    .innerJoin(t, and(eq(t.id, v.templateId), eq(t.workspaceId, v.workspaceId)))
    .where(and(eq(v.workspaceId, workspaceId), eq(t.kind, 'onboarding'), eq(t.slug, slug), eq(t.active, true), eq(v.status, 'published'))).limit(1);

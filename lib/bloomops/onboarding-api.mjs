import { configureOnboardingItem } from "./onboarding-guidance.mjs";
import {
  requireAuthorized,
  requireAccess,
  getActor,
...
export async function onboardingResponse(
  ...
  let body = {};
  if (portal || ["configure", "waive", "not_applicable"].includes(operation)) {
    // Bounded before parsing; no other request fields become operational data.
    const raw = await req.text();
    if (raw.length > 8192)
      return json({ error: "Use the fields in the step form." }, 400);
    try {
      body = JSON.parse(raw);
    } catch {
      return json({ error: "Use the fields in the step form." }, 400);
  ...
  try {
    const result = !portal && operation === "configure"
      ? await configureOnboardingItem(access.db, { actor: access.actor, clientId: id, itemId, input: body })
      : await mutateOnboardingItem(access.db, {
      actor: access.actor,
      clientId: id,
      itemId,
    ...
    if (result.reason === "invalid_guidance") return json({ error: "Add clear instructions and a valid HTTPS destination, or choose confirmation only." }, 400);
    if (result.reason === "guidance_changed") return json({ error: "Your team is preparing or updating this step. Refresh to see the current instructions." }, 409);

...
export function pendingSheetDraft(query,edit,draft,changes){
 const fields=edit?(draft===(edit.row[edit.key]??'')?[]:[{id:edit.row.id,key:edit.key,before:edit.row[edit.key]??null,value:draft}]):(changes||[]).map(c=>({id:c.row.id,key:c.key,before:c.row[c.key]??null,value:c.value}));
 return {query,fields};
}

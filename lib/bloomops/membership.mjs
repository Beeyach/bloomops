...
export async function resolveWorkspaceAccess(db, userId, { workspaceId = null } = {}) {
  ...
  const rows = await workspaceAccessQuery(db, userId, { workspaceId });
  return rows[0] || null;
}

// Shared selection and ordering for the ordinary loader and the correlated
// current-session read. userId may be the outer session's SQL column.
export function workspaceAccessQuery(db, userId, { workspaceId = null, condition, selection = { membership: schema.workspaceMemberships, workspace: schema.workspaces } } = {}) {
  const conditions = [

...
// A3's only permission: managing members and invitations of the workspace.
// A4 replaces this with the capability engine.
...
export async function setMembershipStatus(db, { workspaceId, membershipId, status, actorMembership, now = new Date() }) {
  ...
  const rows = await db
    ...
    .set(patch)
    .where(and(eq(schema.workspaceMemberships.id, target.id), eq(schema.workspaceMemberships.workspaceId, workspaceId), eq(schema.workspaceMemberships.status, target.status)))
    .returning();
  ...
  await recordActivity(db, {
    ...
    actorUserId: actorMembership?.userId || null,
    metadata: { from: target.status, to: status, role: target.role },
    occurredAt: iso,
  });

...
// Small internal overview projection. Apply the same client/service assignment
// rules as visibleServiceIds in SQL before the bound, using live membership and
// assignments rather than a previously loaded scope. No historical scope notes.
export async function clientServiceSummary(db, actor, clientId) {
  if (!actor?.scope || !INTERNAL_ROLES.includes(actor.role)) return { items: [], hasMore: false };
  const e = schema.serviceEngagements;
  const scope = actor.role === 'team_member' ? sql`(
    EXISTS (SELECT 1 FROM client_assignments ca WHERE ca.workspace_id=${e.workspaceId}
      AND ca.client_id=${e.clientId} AND ca.membership_id=${actor.membershipId}) OR
    EXISTS (SELECT 1 FROM service_assignments sa WHERE sa.workspace_id=${e.workspaceId}
      AND sa.service_engagement_id=${e.id} AND sa.membership_id=${actor.membershipId}))` : sql`1`;
  const rows = await withCatalogue(db.select({ id: e.id, serviceTypeName: schema.serviceTypes.name,
    packageName: e.packageName, status: e.status }))
    .where(and(eq(e.workspaceId, actor.workspaceId), eq(e.clientId, String(clientId)), liveProjectActor(actor), scope))
    .orderBy(sql`CASE WHEN ${e.status} IN ('completed','cancelled') THEN 1 ELSE 0 END`, asc(schema.serviceTypes.name), asc(e.id)).limit(6);
...
export async function updateServiceEngagement(db, { workspaceId, clientId, service, input, actorMembershipId = null, actorUserId = null, now = new Date() }) {
  ...
  const iso = now.toISOString();
  const table = schema.serviceEngagements;
  const condition = and(eq(table.workspaceId, workspaceId), eq(table.clientId, String(clientId)), eq(table.id, service.id),
    ...[...DETAIL_FIELDS, 'status'].map(key => sql`${table[key]} IS ${service[key] ?? null}`));
  const event = (eventType, metadata) => activityForMutation(db, table, condition, {
    workspaceId, eventType, subjectType: 'service_engagement', subjectId: service.id,
    clientId: String(clientId), serviceEngagementId: service.id, actorMembershipId, actorUserId,
    metadata: { serviceTypeName: service.serviceTypeName, ...metadata }, occurredAt: iso,
  });
  const writes = [];

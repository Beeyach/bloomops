...
export async function transitionContent(db,{actor,contentId,input,now=new Date()}) {
  const resource=await loadContentResource(db,actor,contentId);
  ...
  const condition=and(contentReadCondition(actor),eq(c.id,String(contentId)),eq(c.revision,expectedRevision),eq(c.stage,current.stage),isNull(c.publishedAt),

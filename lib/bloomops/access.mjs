...
import { selectedWorkspace } from './workspace-selection.mjs';
...
export async function getAccess(source, { env = null, url = null } = {}) {
  ...
  const workspaceId = selectedWorkspace(headers);
  const { identity, resolved: joinedWorkspace } = await timedNavigation('identity', () => readWorkspaceIdentity(db, () => getIdentity(auth, headers), { workspaceId }));
  if (!identity) return null;
  const resolved = joinedWorkspace !== undefined ? joinedWorkspace : await timedNavigation('membership', () => resolveWorkspaceAccess(db, identity.user.id, { workspaceId }));
  return {
    user: identity.user,

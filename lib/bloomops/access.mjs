...
// cookie, and an identity with no membership is refused the same way. The
// actor (capabilities, assignments, client links) is loaded once per
...
export async function getActor(access) {
  ...
  if (!access.actor) access.actor = await loadActor(access.db, access);
  return access.actor;
}

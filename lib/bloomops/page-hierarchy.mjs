...
export async function createTreePage(db,actor,input,{snapshot=null}={}){
 ...
 for(let attempt=0;attempt<3;attempt++){
  ...
  const condition=and(eq(w.id,actor.workspaceId),administratorCondition(actor),epoch(actor,tree.revision),snapshot?.condition,sql`NOT EXISTS(SELECT 1 FROM bloomops_pages WHERE workspace_id=${actor.workspaceId} AND creation_request_id=${input.requestId})`);
  await db.batch([
   ensureTree(db,actor),
   insertSelected(db,p,{id,workspaceId:actor.workspaceId,creationRequestId:input.requestId,title:snapshot?.title||'Untitled',body:snapshot?.body||'',revision:1,createdAt:iso,updatedAt:iso},w,condition),

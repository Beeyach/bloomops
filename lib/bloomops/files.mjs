...
async function writeAttempt(db, bucket, actor, file, bytes) {
  try {
    ...
    try {
      ...
      if (current.status === 'ready' && current.objectKey === file.objectKey && objectMatches(await bucket.head(current.objectKey), current, { etag: true })) return success(file.id, true);
    } catch {}
    return { ok: false, reason: 'upload_failed' };
  }
}

async function resume(db, bucket, actor, current, bytes, sha256, now) {
  ...
  await cleanupAttempts(db, bucket, current.id);
  return writeAttempt(db, bucket, actor, { ...current, objectKey: key, leaseUntil, revision: current.revision + 1 }, bytes);
}
...
export async function retryFile(db, { bucket, actor, fileId, input, bytes, now = new Date() }) {
  ...
  return resume(db, bucket, actor, current, bytes, digestHex(await crypto.subtle.digest('SHA-256', bytes)), now);
}

export async function changeFile(db, { bucket, actor, fileId, operation, visibility, expectedRevision, now = new Date() }) {
  const access = await managedFile(db, actor, fileId);
  if (!access.ok) return access;
...
export async function downloadFile(db, { bucket, actor, fileId }) {

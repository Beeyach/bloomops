...
export const CLIENT_NARRATIVE_FIELDS=Object.freeze(['clientSummary','workCompleted','limitations','nextActions']);
...
export function reportInput(input,template){
 if(!template||!exactKeys(input,['title','periodStart','periodEnd','timezone','channel','accountLabel','scopeLabel','commentary','metrics','comparisonPublicationId',...CLIENT_NARRATIVE_FIELDS]))return {error:'Unsupported report fields or template version.'};
 const comparisonPublicationId=input.comparisonPublicationId==null||input.comparisonPublicationId===''?null:input.comparisonPublicationId;if(comparisonPublicationId!==null&&!reportId(comparisonPublicationId))return {error:'Choose a valid prior published report.'};
 const narrative={};for(const key of CLIENT_NARRATIVE_FIELDS){const value=Object.hasOwn(input,key)?input[key]:'';if(!safeText(value,8000))return {error:`Check ${key}.`};narrative[key]=value.trim();}
 ...
 return {value:{...input,...narrative,comparisonPublicationId,title:input.title.trim(),accountLabel:input.accountLabel.trim(),scopeLabel:input.scopeLabel.trim(),commentary:input.commentary.trim(),metrics}};
}

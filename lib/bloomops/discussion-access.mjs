...
export function discussionParentCondition(actor, parent) {
  ...
  return discussionTypedParentCondition(actor, parent.type, parent.id);
}

// Server-only composition may pass a SQL column ID; HTTP callers use the
// validated literal-parent wrapper above. Types always remain a fixed enum.
export function discussionTypedParentCondition(actor, type, id) {
  if (!actor?.scope || !DISCUSSION_TYPES.includes(type)) return sql`0`;

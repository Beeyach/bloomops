...
import {comparisonSource,comparisonWriteGuard,savedReportComparison} from './client-report-comparisons.mjs';
...
export async function getClientReport(db,actor,clientId,id){
 ...
 const comparisonSelection=schema.clientReportComparisons;
 // Header, metric rows and selected comparison belong to one D1 snapshot. A
 // concurrent legitimate save/archive cannot masquerade as revoked access.
 const [rows,metrics]=await db.batch([
  // D1 batch returns named objects before Drizzle decodes positional fields.
  // Both parent tables have `name`; distinct SQL aliases prevent one value
  // collapsing and shifting every following field in the result mapping.
  db.select({report:r,clientName:sql`${c.name}`.as('report_client_name'),serviceName:sql`${schema.serviceTypes.name}`.as('report_service_name'),packageName:sql`${s.packageName}`.as('report_package_name'),selectedComparison:sql`${comparisonSelection.publicationId}`.as('report_comparison_id')}).from(r)
   ...
   .leftJoin(comparisonSelection,and(eq(comparisonSelection.workspaceId,r.workspaceId),eq(comparisonSelection.reportId,r.id)))
   .where(reportCondition(actor,clientId,id)).limit(1),
  db.select({key:m.metricKey,state:m.state,value:m.value,sourceNote:m.sourceNote,collectedAt:m.collectedAt,sourceKind:m.sourceKind,importId:m.importId}).from(m)
   .where(and(eq(m.workspaceId,actor?.workspaceId||''),eq(m.reportId,id),sql`EXISTS (SELECT 1 FROM client_report_drafts WHERE ${reportCondition(actor,clientId,id)})`)).orderBy(asc(m.metricKey)),
 ]);
 if(!rows[0])return null;
 const {report,selectedComparison,...names}=rows[0],template=reportTemplate(report.templateId,report.templateVersion);if(!template) return null;
 // A malformed stored report still cannot return partial metric data.
 ...
 const comparison=await savedReportComparison(db,actor,data,selectedComparison);
 // Recheck current authority, not whether this coherent snapshot is still newest.
 if(!(await db.select({id:r.id}).from(r).where(reportCondition(actor,clientId,id)).limit(1)).length)return null;
 return {...data,...comparison,canEdit:canEditReports(actor)&&!data.archivedAt,canManageArchive:canEditReports(actor),calculations:reportCalculations(data)};
}
...
export async function saveClientReport(db,actor,clientId,id,input){
 ...
 const {metrics,comparisonPublicationId,...draft}=validated.value;
 ...
 if(comparisonPublicationId)intent.push(['comparisonPublication',comparisonPublicationId]);
 ...
 const comparisonContext={...draft,id,clientId,serviceEngagementId:input.serviceEngagementId,templateId:template.id,templateVersion:template.version};
 if(comparisonPublicationId&&!await comparisonSource(db,actor,comparisonContext,comparisonPublicationId))return {ok:false,reason:'invalid',error:'Choose an available compatible prior publication, or remove the comparison before saving.'};
 const comparisonGuard=comparisonWriteGuard(db,actor,comparisonContext,comparisonPublicationId);
 ...
 const fresh=and(eq(w.id,actor.workspaceId),reportClientCondition(actor,clientId,{edit:true}),serviceGuard,comparisonGuard);
 ...
 db.update(r).set({...values,revision:input.expectedRevision+1}).where(and(reportCondition(actor,clientId,id),reportClientCondition(actor,clientId,{edit:true}),serviceGuard,eq(r.serviceEngagementId,input.serviceEngagementId),eq(r.templateId,template.id),eq(r.templateVersion,template.version),eq(r.revision,input.expectedRevision),sql`${r.archivedAt} IS NULL`,comparisonGuard)).returning({id:r.id});
 const written=and(eq(r.id,reportIdValue),eq(r.workspaceId,actor.workspaceId),eq(r.mutationId,mutationId));
 const selection=schema.clientReportComparisons;
 const selectionWrites=[db.delete(selection).where(and(eq(selection.workspaceId,actor.workspaceId),eq(selection.reportId,reportIdValue),sql`EXISTS (SELECT 1 FROM client_report_drafts WHERE ${written})`)),...(comparisonPublicationId?[insertSelected(db,selection,{workspaceId:actor.workspaceId,reportId:reportIdValue,publicationId:comparisonPublicationId},r,written)]:[])];
 const results=await db.batch([header,...selectionWrites,db.delete(m).where(and(eq(m.workspaceId,actor.workspaceId),eq(m.reportId,reportIdValue),sql`EXISTS (SELECT 1 FROM client_report_drafts WHERE ${written})`)),

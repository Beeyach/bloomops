...
export async function getClientReport(db,actor,clientId,id){
 ...
 const [rows,metrics]=await db.batch([
  db.select({report:r,clientName:c.name,serviceName:schema.serviceTypes.name,packageName:s.packageName,selectedComparison:comparisonSelection.publicationId}).from(r)

...
import {reportTemplate,reportInput,reportId,exactKeys,reportCalculations} from './client-report-values.mjs';
...
export async function reportServices(db,actor,clientId,{search='',page=1}={}){
 ...
 const rows=await db.select({id:s.id,name:schema.serviceTypes.name,packageName:s.packageName,status:s.status}).from(s)
...
export async function listClientReports(db,actor,clientId,{page=1}={}){
 ...
 .where(reportCondition(actor,clientId)).orderBy(desc(r.createdAt),asc(r.id)).limit(21).offset((page-1)*20);
 return {client,items:rows.slice(0,20),more:rows.length>20,page,canEdit:canEditReports(actor)};
}
export async function getClientReport(db,actor,clientId,id){
 ...
 const metrics=await db.select({key:m.metricKey,state:m.state,value:m.value,sourceNote:m.sourceNote,collectedAt:m.collectedAt}).from(m)
 ...
 delete data.creationHash;delete data.mutationId;
 return {...data,canEdit:canEditReports(actor),calculations:reportCalculations(data)};
}
export async function saveClientReport(db,actor,clientId,id,input){
 ...
 const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(JSON.stringify([clientId,input.serviceEngagementId,template.id,template.version,draft,metrics])));
 ...
 const header=id===null?insertSelected(db,r,{...values,id:reportIdValue,workspaceId:actor.workspaceId,clientId,serviceEngagementId:input.serviceEngagementId,templateId:template.id,templateVersion:template.version,creatorMembershipId:actor.membershipId,creatorUserId:actor.userId,requestId:input.requestId,creationHash,revision:1,createdAt:now},w,fresh).onConflictDoNothing().returning({id:r.id}):
 db.update(r).set({...values,revision:input.expectedRevision+1}).where(and(reportCondition(actor,clientId,id),reportClientCondition(actor,clientId,{edit:true}),serviceGuard,eq(r.serviceEngagementId,input.serviceEngagementId),eq(r.templateId,template.id),eq(r.templateVersion,template.version),eq(r.revision,input.expectedRevision))).returning({id:r.id});

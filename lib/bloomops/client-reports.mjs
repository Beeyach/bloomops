...
export async function getClientReport(db,actor,clientId,id){
 ...
 const comparison=await savedReportComparison(db,actor,data);
 if(!(await db.select({id:r.id}).from(r).where(and(reportCondition(actor,clientId,id),eq(r.revision,report.revision))).limit(1)).length)return null;

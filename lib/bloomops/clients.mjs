...
import { clientCreationAuthority, clientCreationFingerprint, findClientCreation } from './client-creation.mjs';
...
// D1 permits 100 bound variables per statement. A full client page or
// assignment scope can exceed that; bind the authorized ID set as one JSON
// parameter while retaining a relational membership predicate.
const matchingIds = (column, ids) => sql`${column} IN (SELECT value FROM json_each(${JSON.stringify(ids)}))`;
...
function scopeClause(actor) {
  ...
  return and(eq(t.workspaceId, actor.workspaceId), matchingIds(t.id, ids));
}
...
export function listClients(db, actor, options = {}) {
  return readTogether(db, db => readClients(db, actor, options));
}
async function readClients(db, actor, { filter = 'all', limit = 200 } = {}) {
  ...
  const contact = schema.clientContacts;
  // The partial unique primary-contact index guarantees one row per Client.
  // Joining it avoids a third round trip without widening the Client scope.
  const [countRows, rows] = await Promise.all([countQuery, db
    .select({
      id: t.id,
      name: t.name,
      company: t.company,
    ...
    .leftJoin(contact, and(eq(contact.workspaceId, t.workspaceId), eq(contact.clientId, t.id), eq(contact.isPrimary, true)))
    .where(where)
  ...
  return {
    ...
    clients: rows.map((r) => decorate({
      ...
      primaryContact: r.primaryContactId ? { name: r.primaryContactName, email: r.primaryContactEmail } : null,
    })),
  };
}
...
export async function createClient(db, { workspaceId, input, actorMembershipId = null, actorUserId = null, now = new Date() }) {
  const context = { workspaceId, actorMembershipId, actorUserId };
  const bound = input && ['requestId', 'userId', 'workspaceId'].some(k => Object.hasOwn(input, k));
  if (bound && (input.userId !== actorUserId || input.workspaceId !== workspaceId)) return { ok: false, reason: 'not_found' };
  if (bound && (typeof input.requestId !== 'string' || !REQUEST_ID.test(input.requestId))) return { ok: false, reason: 'invalid', errors: { form: 'Reload this form before saving.' } };
  const errors = {};
  ...
  const ownerId = trimmed(input?.ownerMembershipId) || null;
  const fingerprint = bound ? await clientCreationFingerprint({ name: name.value, contactName: contactName.value,
    contactEmail: contactEmail.value, company: company.value, website: website.value,
    timezone: timezone.value, startDate: startDate.value, ownerMembershipId: ownerId }) : null;
  const match = row => row.userId === actorUserId && row.fingerprint === fingerprint
    ? { ok: true, clientId: row.clientId, slug: row.slug, unchanged: true }
    : { ok: false, reason: 'conflict' };
  if (bound) {
    const prior = await findClientCreation(db, context, input.requestId);
    if (prior) return match(prior);
  }
  const owner = await validateOwner(db, workspaceId, ownerId);
  if (!owner.ok) return { ok: false, reason: 'invalid', errors: { ownerMembershipId: owner.message } };
  const authority = clientCreationAuthority(context);
  const notCreated = async () => {
    const current = await db.select({ id: schema.workspaces.id }).from(schema.workspaces)
      .where(and(eq(schema.workspaces.id, workspaceId), authority)).limit(1);
    // An owner becoming ineligible is a correctable selection conflict,
    // not lost access to the user's entire form and its recovery copies.
    return current.length ? { ok: false, reason: 'conflict' } : { ok: false, reason: 'not_found' };
  };
  const eligibleOwner = owner.value ? sql`EXISTS (SELECT 1 FROM workspace_memberships om WHERE om.workspace_id=${workspaceId}
    AND om.id=${owner.value} AND om.status='active' AND om.role IN ('owner','admin','project_manager'))` : sql`1`;
  const fresh = and(eq(schema.workspaces.id, workspaceId), authority, eligibleOwner,
    bound ? sql`NOT EXISTS (SELECT 1 FROM client_creation_receipts WHERE workspace_id=${workspaceId} AND request_id=${input.requestId})` : undefined);
  ...
  for (const slug of candidates) {
    ...
    const created = and(eq(schema.clients.id, clientId), eq(schema.clients.workspaceId, workspaceId));
    try {
      const result = await db.batch([
        insertSelected(db, schema.clients, {
          id: clientId,
          workspaceId,
          name: name.value,
          ...
          createdAt: iso,
          updatedAt: iso,
        }, schema.workspaces, fresh).returning({ id: schema.clients.id }),
        insertSelected(db, schema.clientContacts, {
          id: contactId,
          workspaceId,
          clientId,
          ...
          createdAt: iso,
          updatedAt: iso,
        }, schema.clients, created),
        activityForMutation(db, schema.clients, created, {
          workspaceId,
        ...
        ...(bound ? [insertSelected(db, schema.clientCreationReceipts, {
          workspaceId, requestId: input.requestId, userId: actorUserId, clientId, fingerprint, createdAt: iso,
        }, schema.clients, created)] : []),
      ]);
      if (bound) {
        const saved = await findClientCreation(db, context, input.requestId);
        return saved ? { ...match(saved), ...(saved.clientId === clientId ? { unchanged: false } : {}) } : await notCreated();
      }
      return result[0].length ? { ok: true, clientId, slug } : await notCreated();
    } catch (err) {
...
export async function updateClient(db, { workspaceId, client, input, actorMembershipId = null, actorUserId = null, now = new Date(), conditionGuard = sql`1` }) {
  ...
  const iso = now.toISOString();
  const table = schema.clients;
  const condition = and(eq(table.workspaceId, workspaceId), eq(table.id, client.id), conditionGuard,
    ...[...DETAIL_FIELDS, 'health'].map(key => sql`${table[key]} IS ${client[key] ?? null}`),
    sql`${table.ownerMembershipId} IS ${client.owner?.membershipId || null}`);
  const writes = [];
  const event = (eventType, metadata) => activityForMutation(db, table, condition, {
    workspaceId, eventType, subjectType: 'client', subjectId: client.id, clientId: client.id,
    actorMembershipId, actorUserId, metadata, occurredAt: iso,
  });
  ...
  writes.push(db.update(table).set({ ...patch, updatedAt: iso }).where(condition).returning({ id: table.id }));
  const results = await db.batch(writes);
  if (!results.at(-1).length) return { ok: false, reason: 'conflict' };

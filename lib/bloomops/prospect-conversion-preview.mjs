...
import {validateEmail, validateName, validateContactName, validateWebsite, clientStatusLabel} from './clients.mjs';
...
async function prospect(db,actor,id){
 ...
 return (await db.select({id:p.id,workspaceId:p.workspaceId,revision:p.revision,businessName:p.businessName,personName:p.personName,publicEmail:p.publicEmail,website:p.website}).from(p).where(and(prospectCondition(actor),eq(p.id,id))).limit(1))[0]||null;
}
...
export async function getConversionOptions(db,actor,id,input={}){
 ...
 const [clients,types]=await Promise.all([
  ...
  db.select({id:t.id,name:t.name,slug:t.slug}).from(t).where(and(eq(t.workspaceId,actor.workspaceId),eq(t.active,true),guard(actor,id),q.serviceQuery?sql`${t.name} LIKE ${like(q.serviceQuery)} ESCAPE '!'`:undefined)).orderBy(asc(t.name),asc(t.id)).limit(21).offset((q.servicePage-1)*20),
 ]);
...
export async function previewProspectConversion(db,actor,id,input){
 ...
 return {ok:true,previewOnly:true,conversionReview:review,client:client?{...client,statusLabel:clientStatusLabel(client.relationshipStatus)}:{id:null,name:profile.businessName,company:null,website:profile.website,relationshipStatus:'draft'},contact:primary?{name:primary.name||null,email:primary.email||null}:null,

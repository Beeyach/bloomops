...
export async function getProspectResults(db,actor,{now=new Date()}={}){
 ...
 const [row]=await db.select({
  ...
  emailsSent:sql`(SELECT count(DISTINCT d.account_email||char(0)||d.provider_message_id) FROM prospect_deliveries d WHERE d.workspace_id=${actor.workspaceId} AND d.state='accepted' AND d.account_email IS NOT NULL AND d.provider_message_id IS NOT NULL AND d.accepted_at<=${asOf})`,

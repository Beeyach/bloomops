import {attachPreview,previewContext,previewLiveCondition} from './preview-policy.mjs';
...
export async function freshOnboardingActor(db, actor) {
  ...
  if (access?.membership.id !== actor.membershipId) return null;
  const fresh = await loadActor(db, access), preview = previewContext(actor);
  if (!preview) return fresh;
  const live = await db.get(sql`SELECT 1 AS ok WHERE ${previewLiveCondition(actor)}`);
  return live ? attachPreview(fresh, preview) : null;
}

...
export async function mutateOnboardingItem(
  db,
  { actor, clientId, itemId, operation, reason, now = new Date() },
  attempt = 0,
) {
  ...
  const valid = sql`${i.status}=${item.status} AND ${i.visibility}=${item.visibility}
    ...
    AND ${active} AND ${contact} AND ${submitting ? sql`NOT ${submission}` : operation === "verify" ? submission : sql`1`}`;
  const verified =
    operation === "verify" ||
  ...
  try {
    ...
    return mutateOnboardingItem(
      db,
      { actor, clientId, itemId, operation, reason, now },
      attempt + 1,
    );
  }
}

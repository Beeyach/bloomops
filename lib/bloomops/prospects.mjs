import {and,asc,desc,eq,or,sql,getTableColumns} from 'drizzle-orm';
...
export async function getProspect(db,actor,id,{activityPage=1}={}){
 ...
 const [sources,history,imports,csvSources]=await readTogether(db,db=>Promise.all([
  ...
  db.select({batchId:schema.prospectCsvBatches.id,fileName:schema.prospectCsvBatches.fileName,importedAt:schema.prospectCsvBatches.createdAt,rawJson:schema.prospectCsvRows.fieldsJson}).from(schema.prospectCsvRows).innerJoin(schema.prospectCsvBatches,and(eq(schema.prospectCsvBatches.id,schema.prospectCsvRows.batchId),eq(schema.prospectCsvBatches.workspaceId,schema.prospectCsvRows.workspaceId))).where(and(eq(schema.prospectCsvRows.workspaceId,actor.workspaceId),eq(schema.prospectCsvRows.prospectId,String(id)),administratorCondition(actor,{purpose:'prospecting'}))).limit(1),
 ]));
 return {profile,sources,csvSource:csvSources[0]?{...csvSources[0],raw:JSON.parse(csvSources[0].rawJson),rawJson:undefined}:null,importSource:imports[0]||null,activity:history.slice(0,20).map(({metadataJson,...event})=>{let metadata=null;try{metadata=JSON.parse(metadataJson);}catch{}return {...event,metadata:{fields:Array.isArray(metadata?.fields)?metadata.fields.filter(k=>Object.hasOwn(PROSPECT_FIELDS,k)):[],...(['PROSPECT_MANUAL_CONTACT','PROSPECT_INTEREST_RECORDED','PROSPECT_REPLY_RESOLVED'].includes(event.eventType)&&typeof metadata?.note==='string'?{note:metadata.note.slice(0,1000)}:{})}};}),activityPage,moreActivity:history.length>20};
}
...
export async function createProspect(db,{actor,input,now=new Date()}){
 if(!permitted(actor,'prospecting.manage')||input&&Object.hasOwn(input,'userId')&&input.userId!==actor.userId)return missing();
 if(!input||Object.keys(input).some(k=>!['userId','workspaceId','requestId','fields'].includes(k))||input.workspaceId!==actor.workspaceId||!REQUEST_ID.test(input.requestId||''))return invalid({form:'Reload the workspace and try again.'});
...
export async function updateProspect(db,{actor,id,input,now=new Date(),additionalCondition=undefined,captureUndo=false}){
 if(!permitted(actor,'prospecting.manage')||input&&Object.hasOwn(input,'userId')&&input.userId!==actor.userId)return missing();
 if(!input||Object.keys(input).some(k=>!['userId','workspaceId','expectedRevision','fields','sources'].includes(k))||input.workspaceId!==actor.workspaceId||!Number.isInteger(input.expectedRevision)||input.expectedRevision<1)return invalid({form:'Reload this profile before saving.'});
 ...
 const condition=and(prospectCondition(actor),eq(p.id,String(id)),eq(p.revision,input.expectedRevision),additionalCondition);
 ...
 const sourceBefore=captureUndo?await db.select(getTableColumns(f)).from(f).innerJoin(p,and(eq(p.id,f.prospectId),eq(p.workspaceId,f.workspaceId))).where(condition):[];
 const sourceValues=source=>({workspaceId:actor.workspaceId,prospectId:String(id),sourceKind:'manual',...source,updatedByMembershipId:actor.membershipId,updatedAt:iso});
 const undo=captureUndo?sourceChanges.map(source=>({key:source.fieldKey,beforeValue:current.profile[source.fieldKey],afterValue:normalized.fields[source.fieldKey],beforeSource:sourceBefore.find(row=>row.fieldKey===source.fieldKey)||null,afterSource:sourceValues(source)})):undefined;
 const eventId=crypto.randomUUID(),writeCondition=(additionalCondition||captureUndo)?and(prospectCondition(actor),eq(p.id,String(id)),eq(p.revision,input.expectedRevision),sql`EXISTS(SELECT 1 FROM activity_events WHERE id=${eventId} AND workspace_id=${actor.workspaceId})`):condition;
 const results=await db.batch([
  (additionalCondition||captureUndo)?insertSelected(db,a,{id:eventId,...activityValues({workspaceId:actor.workspaceId,eventType:'PROSPECT_UPDATED',subjectType:'prospect',subjectId:String(id),actorMembershipId:actor.membershipId,actorUserId:actor.userId,metadata:{fields:changedFields,...(undo?{undo}:{})},occurredAt:iso}),createdAt:iso},p,condition):activityForMutation(db,p,condition,{workspaceId:actor.workspaceId,eventType:'PROSPECT_UPDATED',subjectType:'prospect',subjectId:String(id),actorMembershipId:actor.membershipId,actorUserId:actor.userId,metadata:{fields:changedFields},occurredAt:iso}),
  ...sourceChanges.map(source=>{const values=sourceValues(source);return insertSelected(db,f,values,p,writeCondition).onConflictDoUpdate({target:[f.workspaceId,f.prospectId,f.fieldKey],set:{sourceKind:values.sourceKind,sourceUrl:values.sourceUrl,verification:values.verification,checkedAt:values.checkedAt,updatedByMembershipId:values.updatedByMembershipId,updatedAt:iso}});}),
  db.update(p).set({...patch,revision:sql`${p.revision}+1`,updatedAt:iso}).where(writeCondition).returning({revision:p.revision}),
 ]);
 return results.at(-1)[0]?{ok:true,revision:results.at(-1)[0].revision,...(captureUndo?{undoEventId:eventId}:{})}:{ok:false,reason:'conflict'};
}

// Undo only a server-recorded sheet edit by its original member. The caller
// supplies an event identity, never replacement provenance or checked dates.
export async function undoProspectSheetEdit(db,{actor,id,eventId,now=new Date()}){
 if(!permitted(actor,'prospecting.manage')||typeof eventId!=='string'||!REQUEST_ID.test(eventId))return missing();
 const condition=and(prospectCondition(actor),eq(p.id,String(id)));
 const [event]=await db.select({metadata:a.metadataJson}).from(a).innerJoin(p,and(eq(p.workspaceId,a.workspaceId),eq(p.id,a.subjectId))).where(and(condition,eq(a.id,eventId),eq(a.subjectType,'prospect'),eq(a.eventType,'PROSPECT_UPDATED'),eq(a.actorMembershipId,actor.membershipId))).limit(1);
 if(!event)return missing();
 let changes;try{changes=JSON.parse(event.metadata).undo;}catch{}if(!Array.isArray(changes)||changes.length!==1)return invalid({form:'This edit has no supported undo.'});
 const change=changes[0];if(!Object.hasOwn(PROSPECT_FIELDS,change.key))return missing();
 const consumed=sql`EXISTS(SELECT 1 FROM activity_events ue WHERE ue.workspace_id=${actor.workspaceId} AND ue.subject_type='prospect' AND ue.subject_id=${String(id)} AND ue.event_type='PROSPECT_UPDATED' AND json_extract(ue.metadata_json,'$.undoOf')=${eventId})`;
 const [done]=await db.select({id:p.id}).from(p).where(and(condition,consumed)).limit(1);if(done)return {ok:true,unchanged:true};
 const current=await getProspect(db,actor,id);if(!current)return missing();
 const sources=await db.select(getTableColumns(f)).from(f).innerJoin(p,and(eq(p.id,f.prospectId),eq(p.workspaceId,f.workspaceId))).where(and(condition,eq(f.fieldKey,change.key))).limit(1),source=sources[0]||null;
 const equal=(a,b)=>a===null||b===null?a===b:JSON.stringify(Object.entries(a).sort())===JSON.stringify(Object.entries(b).sort());
 if(current.profile[change.key]!==change.afterValue||!equal(source,change.afterSource))return {ok:false,reason:'conflict'};
 const sourceStillCurrent=sql`EXISTS(SELECT 1 FROM prospect_field_sources uf WHERE uf.workspace_id=${actor.workspaceId} AND uf.prospect_id=${String(id)} AND uf.field_key=${change.key} AND uf.source_kind=${source.sourceKind} AND uf.source_url IS ${source.sourceUrl} AND uf.verification=${source.verification} AND uf.checked_at IS ${source.checkedAt} AND uf.updated_by_membership_id=${source.updatedByMembershipId} AND uf.updated_at=${source.updatedAt})`;
 const guard=and(condition,eq(p.revision,current.profile.revision),sql`${p[change.key]} IS ${change.afterValue}`,sourceStillCurrent,sql`NOT ${consumed}`),iso=now.toISOString();
 const old=change.beforeSource;
 // The conditional event claims this write transaction before source restore
 // changes its own guard. A failure rolls the claim and restoration back.
 const claimId=crypto.randomUUID(),claimed=and(condition,eq(p.revision,current.profile.revision),sql`EXISTS(SELECT 1 FROM activity_events WHERE id=${claimId} AND workspace_id=${actor.workspaceId})`);
 const restoreGuard=and(eq(f.workspaceId,actor.workspaceId),eq(f.prospectId,String(id)),eq(f.fieldKey,change.key),sql`EXISTS(SELECT 1 FROM bloomops_prospects WHERE ${claimed})`);
 const results=await db.batch([
  insertSelected(db,a,{id:claimId,...activityValues({workspaceId:actor.workspaceId,eventType:'PROSPECT_UPDATED',subjectType:'prospect',subjectId:String(id),actorMembershipId:actor.membershipId,actorUserId:actor.userId,metadata:{fields:[change.key],undoOf:eventId},occurredAt:iso}),createdAt:iso},p,guard),
  old?db.update(f).set({sourceKind:old.sourceKind,sourceUrl:old.sourceUrl,verification:old.verification,checkedAt:old.checkedAt,updatedByMembershipId:old.updatedByMembershipId,updatedAt:old.updatedAt}).where(restoreGuard):db.delete(f).where(restoreGuard),
  db.update(p).set({[change.key]:change.beforeValue,revision:sql`${p.revision}+1`,updatedAt:iso}).where(claimed).returning({revision:p.revision}),
 ]);
 if(results.at(-1)[0])return {ok:true,revision:results.at(-1)[0].revision};
 const [replayed]=await db.select({id:p.id}).from(p).where(and(condition,consumed)).limit(1);return replayed?{ok:true,unchanged:true}:{ok:false,reason:'conflict'};
}

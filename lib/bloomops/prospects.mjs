...
export async function getProspect(db,actor,id,{activityPage=1}={}){
 ...
 const [sources,history]=await readTogether(db,db=>Promise.all([
 ...
 return {profile,sources,activity:history.slice(0,20).map(({metadataJson,...event})=>{let metadata=null;try{metadata=JSON.parse(metadataJson);}catch{}return {...event,metadata:{fields:Array.isArray(metadata?.fields)?metadata.fields.filter(k=>Object.hasOwn(PROSPECT_FIELDS,k)):[]}};}),activityPage,moreActivity:history.length>20};
}

...
export const ACTIONS = {
  ...
  // manage is for the people who coordinate delivery.
...
export async function loadServiceResource(db, workspaceId, serviceEngagementId) {
  ...
  const rows = await db
    ...
    .where(and(eq(schema.serviceEngagements.workspaceId, workspaceId), eq(schema.serviceEngagements.id, String(serviceEngagementId))))
    .limit(1);
  const row = rows[0];
  if (!row) return null;

...
import { previewAllows } from './preview-policy.mjs';
...
export const ACTIONS = {
  'pages.shared': { roles: ALL_ROLES },
  'pages.edit': { roles: ALL_ROLES },
  'pages.view': { roles: ADMIN_ROLES },
  'pages.manage': { roles: ADMIN_ROLES },
  'workspace.create': { roles: ADMIN_ROLES },
  'prospecting.view': { roles: ADMIN_ROLES },
  'prospecting.manage': { roles: ADMIN_ROLES },
  ...
  // Creation names the existing internal Client. All other Project actions
  // name a Project; project assignment never grants its parent or siblings.
  'project.create': { roles: DELIVERY_ROLES, resource: true },
  'project.view': { roles: ALL_ROLES, resource: true },
  'project.manage': { roles: DELIVERY_ROLES, resource: true },
  'project.assign': { roles: DELIVERY_ROLES, resource: true },
  // Milestone descriptors carry their Project id; this adds child scope only.
  'milestone.view': { roles: ALL_ROLES, resource: true },
  'milestone.manage': { roles: DELIVERY_ROLES, resource: true },
  'deliverable.view': { roles: ALL_ROLES, resource: true },
  'deliverable.manage': { roles: DELIVERY_ROLES, resource: true },
  'file.view': { roles: ALL_ROLES, resource: true },
  'file.manage': { roles: DELIVERY_ROLES, resource: true },
  'content.list': { roles: INTERNAL_ROLES },
  'portal.content.list': { roles: ['client'] },
  'portal.content.view': { roles: ['client'], resource: true },
  'content.create': { roles: INTERNAL_ROLES, resource: true },
  'content.view': { roles: INTERNAL_ROLES, resource: true },
  'content.calendar': { roles: INTERNAL_ROLES },
  'content.platforms': { roles: INTERNAL_ROLES, resource: true },
  'content.transition': { roles: INTERNAL_ROLES, resource: true },
  'content.manage': { roles: INTERNAL_ROLES, resource: true },
  'content.approval.request': { roles: DELIVERY_ROLES, resource: true },
  'content.approval.withdraw': { roles: DELIVERY_ROLES, resource: true },
  'approval.view': { roles: ['client'], resource: true },
  'approval.respond': { roles: ['client'], resource: true },
  'content.file.manage': { roles: INTERNAL_ROLES, resource: true },
  'recording.view': { roles: ['client'], resource: true },
  'recording.upload': { roles: ['client'], resource: true },
  'action.list': { roles: INTERNAL_ROLES },
  'action.view': { roles: INTERNAL_ROLES, resource: true },
  'action.manage': { roles: DELIVERY_ROLES, resource: true },
  'action.progress': { roles: INTERNAL_ROLES, resource: true },
  'action.dependencies': { roles: DELIVERY_ROLES, resource: true },
...
async function loadAssignedScope(db, actor) {
  const [projectRows, clientRows, serviceRows] = await Promise.all([
    db.select({ projectId: schema.projectAssignments.projectId })
    .from(schema.projectAssignments)
    .where(and(eq(schema.projectAssignments.workspaceId, actor.workspaceId), eq(schema.projectAssignments.membershipId, actor.membershipId))),
    db
...
export async function loadActor(db, access) {
  ...
  // Owner already holds every capability; explicit grants cannot add to that
  // baseline. Other roles still read their current grants on every request.
  const [grants, scope] = await readTogether(db, db => Promise.all([
    kind === 'contact' || actor.role === 'owner' ? [] : loadCapabilityKeys(db, actor),
    kind === 'workspace' ? { kind: 'workspace' } : kind === 'assigned' ? loadAssignedScope(db, actor) : loadContactScope(db, actor),
  ]));
  actor.capabilities = actorCapabilities(actor.role, grants);
  actor.scope = scope;
  return actor;
}
...
export function inScope(actor, resource) {
  ...
  // Only internal loaders construct these descriptors after live Project SQL.
  if (['ads_content','ads_content_parent'].includes(resource.type) && resource.authorizedMembershipId === actor.membershipId && resource.projectId) return true;
  if (resource.type === 'project' && actor.scope?.kind === 'assigned' && actor.scope.projectIds?.has(resource.id)) return true;
  if (resource.type === 'milestone' && actor.scope?.kind === 'assigned' && actor.scope.projectIds?.has(resource.projectId)) return true;
  if (resource.type === 'deliverable' && actor.scope?.kind === 'assigned' && actor.scope.projectIds?.has(resource.projectId)) return true;
  if (resource.type === 'file' && actor.scope?.kind === 'assigned' && actor.scope.projectIds?.has(resource.projectId)) return true;
  if (resource.type === 'action' && actor.scope?.kind === 'assigned'
    && (actor.scope.projectIds?.has(resource.projectId) || (actor.role === 'team_member' && resource.assigneeMembershipId === actor.membershipId))) return true;
...
export function evaluate(actor, { action, resource = null } = {}) {
  ...
  if (!previewAllows(actor, action)) return deny('preview_read_only');
  ...
  if (action === 'action.progress' && actor.role === 'team_member'
    && (resource?.type !== 'action' || resource.assigneeMembershipId !== actor.membershipId)) return deny('assignment');

...
async function projectToday(db, actor, now, condition) {
...
export async function listProjectSummaries(db, actor, { status = 'all', clientId = null, projectId = null, attentionOnly = false, attentionFirst = false, condition, page = 1, limit = 200, now = new Date() } = {}) {
  if (!internal(actor)) return { items: [], hasMore: false };
  const cap = capped(limit, 200), today = await projectToday(db, actor, now, condition);
...
export async function homeDeliverables(db, actor, { now = new Date(), condition } = {}) {
  if (!internal(actor)) return { items: [], hasMore: false };
  const today = await projectToday(db, actor, now, condition);

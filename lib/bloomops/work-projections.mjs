...
import { actionTodayCondition, actionViewCondition, listActions, openActionCondition, readableActionToday } from './actions.mjs';
...
function projectSummariesQuery(db, actor, { status, clientId, projectId, today, condition }) {
...
export async function listProjectSummaries(db, actor, { status = 'all', clientId = null, projectId = null, attentionOnly = false, attentionFirst = false, condition, page = 1, limit = 200, now = new Date(), today: suppliedToday } = {}) {
  if (!internal(actor)) return { items: [], hasMore: false };
  const cap = capped(limit, 200), today = await (suppliedToday ?? projectToday(db, actor, now, condition));
  const offset = (capped(page, 1, 10000) - 1) * cap;
  const q = db.$with('readable_project_summaries').as(projectSummariesQuery(db, actor, { status, clientId, projectId, today, condition }));
...
export async function homeDeliverables(db, actor, { now = new Date(), condition, today: suppliedToday } = {}) {
  if (!internal(actor)) return { items: [], hasMore: false };
  const today = await (suppliedToday ?? projectToday(db, actor, now, condition));
...
export function homeProjection(db, actor, options = {}) {
  return readTogether(db, db => readHomeProjection(db, actor, options));
}
async function readHomeProjection(db, actor, { now = new Date() } = {}) {
  // Two distinct scopes: Action-only grants must not widen readable Projects.
  // Sharing these exact reads removes duplicates, not authorization checks.
  const actionDay = internal(actor) ? readableActionToday(db, actor, now) : null;
  const projectDay = internal(actor) ? projectToday(db, actor, now) : null;
  const [actionSections, projects, deliverables, recent] = await Promise.all([
    Promise.all(HOME_ACTION_VIEWS.map(async view => {
      const result = internal(actor) ? await listActions(db, actor, { view }, { now, limit: HOME_ACTION_LIMIT, today: actionDay }) : { items: [], hasMore: false };
      return [view, { items: result.items || [], hasMore: Boolean(result.hasMore) }];
    })),
    listProjectSummaries(db, actor, { attentionOnly: true, limit: HOME_PROJECT_LIMIT, now, today: projectDay }),
    homeDeliverables(db, actor, { now, today: projectDay }), recentOutputs(db, actor, { now }),
  ]);

...
export async function saveWorkspacePage(db,actor,id,input){
 ...
 if(input&&Object.hasOwn(input,'userId')&&input.userId!==actor.userId)return missing();
 const keys=['workspaceId','expectedRevision','title','body',...(input&&Object.hasOwn(input,'userId')?['userId']:[])];
 if(typeof id!=='string'||!REQUEST_ID.test(id)||!exact(input,keys)||input.workspaceId!==actor.workspaceId||!Number.isSafeInteger(input.expectedRevision)||input.expectedRevision<1||typeof input.title!=='string'||!input.title.trim()||input.title.trim().length>200||typeof input.body!=='string'||new TextEncoder().encode(input.body).length>1800000)return invalid();

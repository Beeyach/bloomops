...
export function profileDraftValue(value,scope,now=Date.now()){
 ...
 const creating=scope.prospectId==='new';
 if(creating&&(value.section!=='identity'||typeof value.creationRequestId!=='string'||!/^([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i.test(value.creationRequestId||'')||value.revision!==1))return null;
 if(value.fieldSet!==undefined&&(value.fieldSet!=='primary_contact'||creating||value.section!=='identity'))return null;
 const keys=value.fieldSet==='primary_contact'?['personName','publicEmail']:Object.keys(PROSPECT_FIELDS).filter(k=>PROSPECT_FIELDS[k].section===value.section),fields={},before={},sources={},sourceBefore={};
 ...
 return {version:1,id:value.id,...scope,...(creating?{creationRequestId:value.creationRequestId}:{}),...(value.fieldSet?{fieldSet:value.fieldSet}:{}),section:value.section,revision:value.revision,updatedAt:value.updatedAt,fields,before,sources,sourceBefore};
}
...
export function createProfileDraftStore(storage,scope,writerId,now=()=>Date.now()){
 ...
 return {own,valid,list,snapshot:key=>valid()&&key.startsWith(prefix)?snapshot(key):null,remove,
  write(input){
   ...
   purge();if(!storage.getItem(own)&&list().length>=PROFILE_DRAFT_COPIES)throw Error('Ten recovery copies are already kept for '+(scope.prospectId==='new'?'new prospects in this workspace':'this prospect')+'. Discard an old copy before keeping another.');

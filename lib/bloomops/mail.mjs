...
export function magicLinkEmail({ url, expiresMinutes = 15 }) {
  const subject = 'Your BloomOps sign-in link';
  const text = [
    'Use this link to sign in to BloomOps:',
    url,
    '',
...
export function invitationEmail({ url, workspaceName, roleLabel, inviterName, expiresDays = 7 }) {
  ...
  const subject = `You're invited to ${workspaceName} on BloomOps`;
  const text = [
    `${who} to join ${workspaceName} on BloomOps as ${roleLabel}.`,
    '',
  ...
  const html = layout(
    `Join ${workspaceName}`,
    [
      `${escapeHtml(who)} to join <strong>${escapeHtml(workspaceName)}</strong> on BloomOps as ${escapeHtml(roleLabel)}.`,

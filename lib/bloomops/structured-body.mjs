...
export async function readStructuredBody(request,{maxBytes=65536}={}){
 const reader=request.body?.getReader();if(!reader)throw new InvalidBodyError();
 const chunks=[];let size=0;
 for(;;){const {value,done}=await reader.read();if(done)break;size+=value.byteLength;if(size>maxBytes){await reader.cancel();throw new InvalidBodyError();}chunks.push(value);}

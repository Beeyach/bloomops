...
async function membershipForEmail(db, workspaceId, email) {
  const rows = await db
    .select({ id: schema.workspaceMemberships.id, status: schema.workspaceMemberships.status })
...
export async function createInvitation(db, { workspaceId, email, role, clientId = null, inviteeName = null, invitedByMembershipId = null, now = new Date() }) {
  const normalized = normalizeEmail(email);
  ...
  if (existing && existing.status === 'active') return { ok: false, reason: 'already_member' };

  const pending = await findPendingRow(db, workspaceId, normalized);
  if (pending) {
    const rotated = await rotateInvitation(db, pending, { role, clientId, inviteeName, invitedByMembershipId, now });
  ...
  let invitation;
  try {
    invitation = (await db.insert(schema.workspaceInvitations).values(values).returning())[0];
  } catch (err) {
  ...
  await recordActivity(db, {
    workspaceId,
    eventType: ACTIVITY.INVITATION_SENT,
    subjectType: 'invitation',
    subjectId: invitation.id,
...
async function rotateInvitation(db, pending, { role = pending.role, clientId = pending.clientId, inviteeName = pending.inviteeName, invitedByMembershipId = pending.invitedByMembershipId, now }) {
  const token = generateInvitationToken();
  const iso = now.toISOString();
  const rows = await db
    .update(schema.workspaceInvitations)
    ...
    .where(and(eq(schema.workspaceInvitations.id, pending.id), eq(schema.workspaceInvitations.status, 'pending')))
    .returning();

...
export async function onboardingGenerationReadiness(db, {workspaceId, clientId}) {
  const existing = await openInstance(db, workspaceId, clientId);
  return existing ? existingResult(existing) : {ok:true};
}

// Internal composition primitive: validated writes, to execute in ONE batch.
export async function prepareOnboardingWrites(db, { workspaceId, clientId, plan, now = new Date() }) {
  if (!await clientExists(db, workspaceId, clientId)) return failure('not_found');
  if (!plan || plan.workspaceId !== workspaceId || plan.clientId !== clientId) return failure('not_found');
  const readiness = await onboardingGenerationReadiness(db, {workspaceId, clientId});
  ...
  return { ok: true, instanceId, statements };
}

export async function persistOnboardingPlan(db, options) {
  const prepared = await prepareOnboardingWrites(db, options);
  if (!prepared.ok) return prepared;
  const { instanceId, statements } = prepared;

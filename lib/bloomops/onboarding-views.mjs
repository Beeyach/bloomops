import {previewLiveCondition} from './preview-policy.mjs';
...
export async function onboardingView(
  ...
  const activation = (
    await db
      ...
      .from(a)
      .where(and(eq(a.workspaceId, ws), eq(a.clientId, clientId), previewLiveCondition(actor)))
      .limit(1)
  )[0];
  const instance = (
    await db

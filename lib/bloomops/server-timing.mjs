// Inactive PERF4 primitive. No deployed importer, subscriber, stream wrapper or log sink.
// Local callers must explicitly opt in. Staging activation requires a separate decision.
...
export function createServerTiming(options = {}) {
  try {
    const { enabled, environment, route, now = () => performance.now(), emit } = options;
    if (enabled !== true || environment !== 'development' || !routes.has(route)) return off;
    const start = now();

...
import { workspaceSessionDatabase } from './workspace-session.mjs';
...
export function createAuth({ env, db = null, mailer = null, requestUrl = null, now = () => new Date() }) {
  ...
  return betterAuth({
    ...
    database: drizzleAdapter(workspaceSessionDatabase(database), {
      provider: 'sqlite',
      schema: {
        user: schema.user,
    ...
    advanced: {
      // One current database read for session + user, with no cookie cache.
      database: { joins: true },
    ...
    hooks: {
      ...
      before: createAuthMiddleware(async (ctx) => {
        if (ctx.path === '/update-user' && ctx.body && Object.hasOwn(ctx.body, 'image')) {
          throw new APIError('BAD_REQUEST', { message: 'Use Profile to change your photo.' });
        }
          ...
          before: async (user) => {
            ...
            if (!invitation) {
              throw new APIError('FORBIDDEN', {
                ...
                message: 'This email address has not been invited to a Bloomsi workspace.',
              });
            }
            return { data: { ...user, image: null, email, name: user.name || invitation.inviteeName || nameFromEmail(email) } };
          },
        },
      },
    },

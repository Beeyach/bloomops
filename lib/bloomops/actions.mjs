import {assignmentNotificationInsert} from './notification-events.mjs';
...
export function actionTodayCondition(now, zones, timezone = c.timezone, { directLookup = false } = {}) {
  ...
  // Home's full runtime timezone map is repeatedly read by date predicates.
  // A scalar lookup avoids expanding json_each for each candidate Action.
  if (directLookup) return sql`coalesce(json_extract(${days}, '$.' || json_quote(${timezone})), ${actionCalendarDay(now)})`;
  return sql`coalesce((SELECT value FROM json_each(${days}) WHERE key=${timezone}), ${actionCalendarDay(now)})`;
}

// This runtime-owned list contains no tenant or authorization state. It lets
// a composition enqueue its still-fresh, authorization-scoped timezone read
// in the same native D1 batch as the projections that consume Client days.
const commonTimezones = typeof Intl.supportedValuesOf === 'function' ? Intl.supportedValuesOf('timeZone') : [];
const commonTimezoneSet = new Set(commonTimezones);
const commonTimezoneRows = commonTimezones.map(timezone => ({ timezone }));
export function commonActionToday(now, options) {
  return { today: actionTodayCondition(now, commonTimezoneRows, c.timezone, options), timezones: commonTimezoneSet };
}

export function actionTimezonesCovered(zones, knownTimezones) {
  return zones.every(({ timezone }) => !timezone || knownTimezones.has(timezone));
}
...
export function readableActionTimezones(db, actor, filters = {}) {
  return joined(db, { timezone: c.timezone }).where(and(actionReadCondition(actor), filterCondition(filters))).groupBy(c.timezone);
}

export async function readableActionToday(db, actor, now, filters = {}) {
  return actionTodayCondition(now, await readableActionTimezones(db, actor, filters));
}

export async function listActions(db, actor, input = {}, { now = new Date(), limit = ACTION_LIMIT, today: suppliedToday, scopeCondition } = {}) {
  ...
  let today;
  if (['overdue', 'today', 'upcoming'].includes(view)) {
    today = suppliedToday == null ? await readableActionToday(db, actor, now, filters)
      : typeof suppliedToday?.then === 'function' ? await suppliedToday : suppliedToday;
  } else today = actionTodayCondition(now, []);
  ...
  const rows = await dtoQuery(db, actor).where(and(actionReadCondition(actor), scopeCondition, filterCondition(filters), actionViewCondition(actor, view, today)))
...
export async function createAction(db, { actor, projectId, input, requestId, now = new Date() }) {
  ...
  const id = newId(), iso = now.toISOString(), eventId = crypto.randomUUID();
  ...
  const results = await db.batch([
    ...
    activityForMutation(db, a, eq(a.id, id), actionEvent(actor, access.project, values, 'ACTION_CREATED', { details: patch }, now), eventId),
    assignmentNotificationInsert(db,actor,{eventId,actionId:id,now:iso}),
  ]);
...
async function applyPatch(db, actor, access, patch, events, now, { progress = false } = {}) {
  ...
  // The first event must follow a successful UPDATE. Each later event follows
  // that one-row receipt insert in the same D1 batch. Revision alone cannot
  // prove this write won: a concurrent writer can already hold that revision.
  const written = and(eq(a.id,current.id),eq(a.workspaceId,actor.workspaceId),eq(a.revision,current.revision+1),sql`changes()=1`);
  const results = await db.batch([
    db.update(a).set({ ...patch, revision: sql`${a.revision}+1`, updatedAt: now.toISOString() }).where(condition).returning({ id: a.id }),
    ...events.map(([type, metadata]) => activityForMutation(db, a, written, actionEvent(actor, access.resource, current, type, metadata, now), type==='ACTION_ASSIGNEE_CHANGED'?eventId:undefined)),
    ...(Object.hasOwn(patch,'assigneeMembershipId')?[assignmentNotificationInsert(db,actor,{eventId,actionId:current.id,now:now.toISOString()})]:[]),
  ]);
  if (results[0].length) return actionSuccess(current.id);

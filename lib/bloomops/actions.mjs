...
export async function listActions(db, actor, input = {}, { now = new Date(), limit = ACTION_LIMIT } = {}) {
  ...
  const zones = ['overdue', 'today', 'upcoming'].includes(view)
    ? await joined(db, { timezone: c.timezone }).where(and(actionReadCondition(actor), filterCondition(filters))).groupBy(c.timezone)
    : [];
  const today = actionTodayCondition(now, zones);

...
export async function listActions(db, actor, input = {}, { now = new Date() } = {}) {
  ...
  const today = sql`coalesce((SELECT value FROM json_each(${days}) WHERE key=${c.timezone}), ${actionCalendarDay(now)})`;
  const open = sql`${a.status} NOT IN ('done','cancelled')`;
  ...
  const rows = await dtoQuery(db, actor).where(and(actionReadCondition(actor), filterCondition(filters), views[view]))
    ...
    .limit(ACTION_LIMIT + 1).offset((page - 1) * ACTION_LIMIT);
  return { ok: true, items: rows.slice(0, ACTION_LIMIT).map(row => dto(row, now)), hasMore: rows.length > ACTION_LIMIT, page };
}

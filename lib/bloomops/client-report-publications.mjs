...
export function publishedReportCondition(actor,{portal=false}={}){
 if(portal){
  ...
  return and(eq(r.workspaceId,actor.workspaceId),liveProjectActor(actor),sql`EXISTS (SELECT 1 FROM client_contacts cc WHERE cc.workspace_id=${r.workspaceId} AND cc.client_id=${r.clientId} AND cc.user_id=${actor.userId})`);
 }
...
export async function reportPublicationReview(db,actor,clientId,id){
 ...
 return {snapshot:reportPublicationSnapshot(report),draftRevision:report.revision,current:current||null,canEdit:canEditReports(actor),hasPortalRecipient:!!eligible};
}
export async function changeReportPublication(db,actor,clientId,id,input){
 ...
 const report=await getClientReport(db,actor,clientId,id);if(!report)return absent();
 ...
 if(input.kind==='publish'&&current?.kind==='publish'&&current.draftRevision===report.revision)return {ok:true,id:current.id,unchanged:true};
 ...
 const condition=and(eq(r.id,id),eq(r.clientId,clientId),publishedReportCondition(actor),reportClientCondition(actor,r.clientId,{edit:true}),eq(r.revision,input.expectedRevision),

...
import {comparisonWriteGuard} from './client-report-comparisons.mjs';
...
export function reportPublicationSnapshot(report){
 ...
 return {formatVersion:1,...(report.comparison?{comparison:report.comparison}:{}),title:report.title,clientName:report.clientName,serviceName:report.serviceName,packageName:report.packageName,
...
export async function reportPublicationReview(db,actor,clientId,id){
 ...
 return {snapshot:reportPublicationSnapshot(report),draftRevision:report.revision,current:current||null,comparisonUnavailable:!!report.comparisonUnavailable,archived:!!report.archivedAt,hiddenByArchive:!!current&&current.sequence<=report.publicationFloor,canEdit:report.canEdit,hasPortalRecipient:!!eligible};
}
export async function changeReportPublication(db,actor,clientId,id,input){
 ...
 const report=await getClientReport(db,actor,clientId,id);if(!report)return absent();if(input.kind==='publish'&&report.comparisonUnavailable)return {ok:false,reason:'invalid',error:'The saved comparison is unavailable. Choose an available prior publication or remove it and save before publishing.'};if(report.archivedAt)return {ok:false,reason:'invalid',error:'Restore this archived report before changing publication.'};
 ...
 const condition=and(eq(r.id,id),eq(r.clientId,clientId),publishedReportCondition(actor),reportClientCondition(actor,r.clientId,{edit:true}),eq(r.revision,input.expectedRevision),sql`${r.archivedAt} IS NULL`,input.kind==='publish'?comparisonWriteGuard(db,actor,report,report.comparisonPublicationId):sql`1`,

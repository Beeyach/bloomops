...
import { loadInternalContentResource, internalContentReadCondition, internalContentWriteCondition } from './content-internal-access.mjs';
...
export async function authorizeContentFiles(db, actor, contentId, { portal = false, upload = false } = {}) {
  const resource = await (portal ? loadRecordingResource(db,actor,contentId) : loadInternalContentResource(db,actor,contentId,{write:upload}));
...
export function contentFilesQuery(db, actor, contentId, { portal = false, ready = false } = {}) {
  return joined(db, portal ? safeFields : internalFields).where(and(contentFileReadCondition(actor, { contentId, portal, ready }),
...
export async function listContentFiles(db, actor, contentId, options = {}) {
  return { items: await contentFilesQuery(db, actor, contentId, options) };
}
...
function policyFor(contentId = null, { portal = false, upload = false, write = true } = {}) {
  const options = { contentId, portal, upload, write, includeArchived: !portal };
...
export async function uploadContentFile(db, { bucket, actor, contentId, input, bytes, portal = false, now = new Date() }) {
  ...
  if (access.resource.productionArea === 'ads' && (value.purpose !== 'asset' || !['internal','restricted'].includes(value.visibility)))
    return {ok:false,reason:'invalid',errors:{form:'Upload an internal or restricted creative asset.'}};
  ...
  const subject = and(eq(c.id, String(contentId)), portal ? recordingRequestCondition(actor) : internalContentWriteCondition(actor), contentFileVisibilityCondition(actor, value.visibility));
...
export const downloadContentFile = (db, options) => downloadFile(db, options, policyFor(null, { portal: options.actor?.role === 'client', write:false }));
...
export async function canRestrictContentFiles(db, actor, contentId) {
  const [row] = await db.select({ id: c.id }).from(c).where(and(eq(c.id, String(contentId)), internalContentReadCondition(actor), contentFileVisibilityCondition(actor, 'restricted'))).limit(1);
  return Boolean(row);
}

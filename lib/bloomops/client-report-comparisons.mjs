...
export async function savedReportComparison(db,actor,report){
 const t=schema.clientReportComparisons,d=schema.clientReportDrafts;
 const [selection]=await db.select({id:t.publicationId}).from(t).innerJoin(d,and(eq(d.workspaceId,t.workspaceId),eq(d.id,t.reportId))).where(and(eq(d.id,report.id),eq(d.workspaceId,actor.workspaceId),eq(d.revision,report.revision),reportClientCondition(actor,d.clientId))).limit(1);

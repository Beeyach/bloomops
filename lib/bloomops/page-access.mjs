...
export function activePageMember(actor){
 ...
 return sql`EXISTS(SELECT 1 FROM workspace_memberships wm JOIN workspaces ws ON ws.id=wm.workspace_id WHERE wm.id=${actor.membershipId} AND wm.user_id=${actor.userId} AND wm.workspace_id=${actor.workspaceId} AND wm.status='active' AND ws.status='active')`;
}
...
export function pagePermission(actor,pageId){
 ...
 return sql`CASE WHEN ${administratorCondition(actor)} THEN 'edit' ELSE (
  ...
  SELECT g.permission FROM lineage JOIN bloomops_page_grants g ON g.page_id=lineage.id AND g.workspace_id=${actor.workspaceId}
...
export const pageEditCondition=(actor,id)=>sql`${pagePermission(actor,id)}='edit'`;

export const pageCommentCondition=(actor,id)=>sql`${pagePermission(actor,id)} IN ('comment','edit')`;

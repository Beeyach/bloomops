...
import { previewLiveCondition, previewClientCondition } from './preview-policy.mjs';
...
export function liveProjectActor(actor) {
  ...
  return sql`${previewLiveCondition(actor)} AND EXISTS (SELECT 1 FROM workspace_memberships m JOIN workspaces w ON w.id=m.workspace_id
...
export function projectReadCondition(actor, { portal = false } = {}) {
  ...
  return and(eq(p.workspaceId, actor.workspaceId), liveProjectActor(actor), previewClientCondition(actor, p.clientId), scope, visibility);
}

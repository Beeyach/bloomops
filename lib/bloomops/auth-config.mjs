...
export const DEFAULT_MAIL_FROM = 'BloomOps <onboarding@resend.dev>';
...
export function resolveAppUrl(env = {}, requestUrl = null) {
  ...
  if (!configured) {
    throw new Error('BLOOMOPS_APP_URL is not configured for this environment. Authentication is disabled until it is.');
  }
  if (!configured.startsWith('https://')) {
    throw new Error('BLOOMOPS_APP_URL must be an https origin outside development.');
  }
  return configured;
}
...
export function authSecret(env = {}) {
  ...
  if (secret === DEV_SECRET && !isDevelopment(env)) {
    throw new Error('BLOOMOPS_AUTH_SECRET is the development fallback. Set a real secret for this environment.');
  }
  if (secret.length >= 32) return secret;
  if (secret) throw new Error('BLOOMOPS_AUTH_SECRET must be at least 32 characters.');
  if (isDevelopment(env)) return DEV_SECRET;
  throw new Error('BLOOMOPS_AUTH_SECRET is not configured for this environment. Authentication is disabled until it is.');
}
...
export function mailConfig(env = {}) {
  ...
  if (transport === 'resend' && !apiKey) {
    throw new Error('BLOOMOPS_RESEND_API_KEY is required for the resend mail transport.');
  }

...
export function actionCalendarDay(now, timezone = null) {
  ...
  // fallback when a Client has not chosen one. Formatters contain no request,
  // identity, membership or business data, so they are safe to reuse.
  const key = timezone || 'UTC';
  let formatter = dayFormatters.get(key);
  if (!formatter) {
    formatter = new Intl.DateTimeFormat('en-CA', { timeZone: key, year: 'numeric', month: '2-digit', day: '2-digit' });

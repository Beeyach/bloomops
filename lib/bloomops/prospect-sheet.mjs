...
export async function listProspectSheet(db,actor,input={}){
 ...
 const [counts,rows]=await db.batch(queries);const total=Number(counts[0].total);
 // Only an empty filtered result needs a bounded existence probe. Never count
 // the whole workspace for empty-state copy, and retain current authority.
 const filtered=q.view!=='all'||['q','fit','region','platform','batch','never'].some(k=>q[k]);
 const workspaceEmpty=total===0&&(!filtered||!(await db.select({id:p.id}).from(p).where(prospectCondition(actor)).limit(1)).length);
 return {query:q,total,rows,workspaceEmpty};
}

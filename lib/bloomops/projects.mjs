...
import { liveProjectActor, loadProjectResource, projectResource, projectAssignmentCondition, projectReadCondition } from './project-access.mjs';
...
export async function listProjects(db, actor, { clientId = null, status = 'all', limit = 200, openOnly = false } = {}) {
  ...
  const rows = await joined(db, columns()).where(and(projectReadCondition(actor),
    openOnly ? sql`${p.status} NOT IN ('completed','cancelled','archived')` : undefined,
...
export async function portalProjects(db, actor, { projectId = null, limit = null, offset = 0 } = {}) {
  const query = db.select({ id: p.id, label: sql`coalesce(${p.clientLabel}, ${p.name})`, status: p.status,
  ...
  const rows = await (Number.isSafeInteger(limit) && limit > 0 && limit <= 101
    ? query.limit(limit).offset(Number.isSafeInteger(offset) && offset >= 0 ? offset : 0) : query);
...
export async function authorizeProject(db, actor, projectId, action) {
  // getProject's SQL already proves current workspace, membership, scope and
  // visibility before selecting the internal DTO. Do not fetch the same row
  // twice. As with Content descriptors, retain only this actor's proven grant;
  // the separately authorized assignment directory is not a policy input.
  const project = await getProject(db, actor, projectId);
  if (!project) return { ok: false, reason: 'not_found' };
  const resource = projectResource(project, project.visibility === 'restricted' ? [actor.membershipId] : [], { internal: true });
  if (!evaluate(actor, { action, resource }).allowed) return { ok: false, reason: 'forbidden' };
  return { ok: true, project, resource };
}

// Shared preparation for explicit creation and atomic reusable-setup creation.
// The caller must include condition in its actual INSERT, never just this read.
export async function prepareProjectCreation(db, { actor, clientId, input, now = new Date() }) {
  ...
  return {ok:true,values,condition};
}

export async function createProject(db, { actor, clientId, input, now = new Date() }) {
  const prepared = await prepareProjectCreation(db,{actor,clientId,input,now});
  if (!prepared.ok) return prepared;
  const {values,condition} = prepared, id=values.id;

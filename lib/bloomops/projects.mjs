...
export async function projectOptions(db, actor, { clientId = null } = {}) {
  ...
  const clients = await db.select({ id: c.id, name: c.name }).from(c).where(and(eq(c.workspaceId, actor.workspaceId),
    liveProjectActor(actor), clientId ? eq(c.id, String(clientId)) : undefined)).orderBy(c.name).limit(200);
  const services = await db.select({ id: s.id, clientId: s.clientId, name: t.name }).from(s)
  ...
  const departments = await db.select({ id: d.id, name: d.name }).from(d)
  ...
  const members = await db.select({ membershipId: m.id, name: schema.user.name, role: m.role }).from(m)

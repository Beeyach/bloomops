...
Owner interaction correction: Platform and Fit open options on the first click, without entering an intermediate edit form. Selection retains the existing review before save; only Custom platform needs text entry. Outreach also has a direct dropdown into the existing contact, interest, reply-handled, outreach-review and client-handoff flows. Preselect the requested actual-event form; changing the menu alone must not fabricate a status or send a message. Selecting a field value must visibly lead to its unsaved review and Save changes action on desktop and phones, with keyboard focus and readable before/after labels.

...
Owner interaction correction, superseded by full-table QA: Platform and Fit open options on the first click and save the selected value directly. Show the chosen value while saving, then Saved or a recoverable failure. Choosing another field after a successful save must not ask to discard it. Custom platform and text cells use one explicit Save field action. Bulk/pasted changes retain their explicit Apply step. Newer owner correction: ordinary failed/recovered cells use inline recovery without a global review panel or discard prompt when another cell is edited. Keep the attempted value and show Retry beside that field; for a real concurrent change, show the current saved value and offer Use my value or Keep saved. Replacement still compares against the value the user saw. Preserve independent unfinished cells across field/view changes, collapse older recovery-copy metadata, and retire already-saved copies after an authorized check. Latest owner correction: text-cell editing keeps the row height stable and shows a labelled Save button. Older copies are accessible through a compact Drafts toolbar menu, not a separate recovery banner. Remove the Current platform source-detail editor from the full profile while preserving stored provenance. Actual page departure with unsaved work keeps its leave protection. Save failures, lost responses, current authority and concurrent-edit guards must remain intact.

...
The exact merged code, required contracts/history, auth adapters, route implementations, rendering boundaries and performance harnesses were inspected before selecting the change. Baseline builds preceded the optimization. Baseline timing used base application behavior with only the fixed-label timing markers added; candidate runs use the same markers and harness. All benchmark builds report `dev`, the existing local default when CI stamping variables are absent; that label is **not** source-revision proof. Application/test implementation commit: `e347c57631106dd3761234528e732efc691a961f`. Full tests and the Cloudflare build were rerun successfully on this exact commit. The final PR head includes the subsequent documentation-only verification/provenance correction; it does not change the tested application. The committed numeric evidence retains each benchmark run's actual `dev` label for transparency. The final exact-head build uses the existing CI stamping variables, without modifying configuration.
...
For reproduction, use Node 22+, installed lockfile dependencies, external Playwright/Chromium, local-only development bindings and current local migrations. Reuse `scripts/navigation-perf-fixture.mjs`'s documented representative and stress modes from PERF2; do not point any fixture command at staging/production. Build each application revision with `npm run build` and `npm run cf:build`; set the existing `WORKERS_CI_COMMIT_SHA` and `WORKERS_CI_BRANCH` variables to the actual source revision/branch when a stamped local build is required (otherwise `/api/version` honestly says `dev`). To instrument the base, apply only the PERF3 fixed-label marker changes in `access.mjs`, `work-projections.mjs` and `systems.mjs` plus `navigation-timing.mjs`, retaining the base auth/membership path. Use the same completion/worker harness files for both builds. Stop the owned preview before rebuilding/restarting.

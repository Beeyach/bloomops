...
## Selected raw imports (P2 local implementation)

Source preview/export requires the same user's current Owner/Admin authority in the selected Prospecting destination and an operational source workspace. Only explicitly selected untouched raw identities are eligible. The source is read within the currently bound database; this does not connect to the separate original LTB app. Recorded work, suppression, ambiguous evidence or missing required history prevent readiness.

`prospect_import_receipts` owns the immutable request identity, actor, source workspace, reconciled file outcome counts and completion time. `prospect_import_rows` retains each file ordinal/outcome, source identity/label and the initial mapped fields for imported rows. The unique destination/source-workspace/source-record mapping survives profile edits and prevents recreation. Only imported rows point to canonical profiles; duplicates/rejections are retained as outcomes and are never merged.

An explicit commit imports all Ready rows from one unchanged preview, bounded to 50 file records and 1 MiB. A single D1 batch rechecks live authority, exact source evidence/fields and current destination duplicates, creates raw-only profiles with one creation event each, then seals the receipt only when every outcome and event is complete. Any failed selected row rolls the batch back. Exact actor/request/payload retries return the original receipt under current authority; changed payloads conflict. Import does not start automation or copy old audits, qualification, drafts, sends or schedules.

Receipt history and profile provenance belong to the destination and remain readable to its current Owner/Admin after source access is revoked. Commit retries still require both authorities. Imported provenance never marks a field verified; subsequent manual checks and edits use canonical P1 field sources and revision rules. Additive migration `0026` defines both tables and completion/immutability guards; deployment status remains in BUILD_STATE.
...
## Manual prospect skills and context (P2C1 local implementation)

The Prospecting-only Skills Library has three built-in versioned manual definitions with explicit inputs, instructions and JSON output schemas. Context format `bloomsi.prospect-context` version 1 exports the selected prospect's canonical fields, revision, field-source provenance and immutable import provenance. It excludes activity/actor identity and unrelated records. All task content is untrusted research input; proposed work does not establish approved price/claims or permission to send.

Copy/download uses current workspace authority and rejects a stale skill version or profile revision. A final scoped revision read after provenance reads prevents returning mixed-revision context. Exports do not write a prospect, report, receipt or activity event. An exported revision is a dated snapshot, not a lock or authorization for future changes. P2C2 owns structured result validation, separate full-report storage and conflict-aware application; none is implied by export success.

## Manual result artifacts (P2C2 local implementation)

`prospect_skill_results` stores immutable full reports and original structured artifacts separately from canonical profile fields. Workspace/prospect/actor foreign keys scope each result. Normalized result identity deduplicates equivalent field/URL/email spellings; request hashes bind exact retry choices. Saving explicitly selected fields, unverified source records, one activity event and one profile revision occurs in one D1 batch. A stale preview or revoked authority prevents the entire write. Full reports and follow-up suggestions remain inspectable; historical accepted field snapshots do not become a second editable profile or approved/scheduled sequence.


## Outreach content review (P3A local implementation)

`prospect_senders` holds one explicitly configured Google Workspace sender identity per workspace. Provider/address/display name and revision belong to that workspace; there is no token, connection or inherited credential in this table. Saving identity never enables delivery.

`prospect_outreach_drafts` holds one current versioned sequence per prospect: recipient, timezone, subject, introduction and two follow-up bodies. Incomplete drafts can be saved. The source-profile revision and optional same-workspace immutable skill-result reference retain provenance. Selecting a saved result supplies its two follow-ups for explicit review while preserving the current draft introduction; later results do not automatically modify a sequence.

`prospect_outreach_approvals` holds immutable content-review snapshots bound to the exact draft, profile and sender revisions, approving member and membership update stamp. Complete messages, configured sender, Strong fit, checked current public recipient and dated evidence/proposed work are required along with an explicit human acknowledgement. Live Owner/Admin Prospecting authority and all expected revisions are checked inside atomic D1 write batches. Retries/concurrent approval do not duplicate snapshots or activity. Edits and approver revocation/demotion invalidate current approval without deleting history. Restoring a membership with a new update stamp does not restore its old approval.

Overview reads the current sender and saved-draft states together. Unsaved inventory is absent; content approval is separate from sending, connection, schedule and provider-confirmed results. P3A has no provider calls or send/reply/schedule counters. Cross-workspace foreign keys and immutable-approval triggers arrive in additive migration `0028`; the original prospect and send tables remain untouched.


## Google connection and timing preview (P3B local implementation)

`prospect_google_attempts` records hashed, single-use, ten-minute OAuth state bound to the current session, workspace/member stamp, sender and connection revisions. The PKCE verifier is encrypted with workspace/attempt associated data. A callback claims the attempt before provider exchange, then rechecks live authority and revisions when atomically attaching the grant and activity event.

`prospect_google_connections` holds one versioned grant per workspace, with verified primary/sender addresses, authorizing-member stamp, scope and encrypted tokens. A dedicated configuration/key is required; client DTOs omit secrets. Accepted matching aliases are supported. Sender address edits, authorizer changes/revocation and unusable keys invalidate the local connected state. Disconnect clears token/account/scope fields and advances the revision, invalidating pending callbacks. Neither action changes content approvals, schedules or Results. Migration `0029` adds these two tables without changing legacy connections.

P3C1 adds `check_id`, `check_expires_at`, `checked_at` and `check_status` through additive migration `0030`. An explicit Owner/Admin check claims a 90-second lease and advances the connection revision before refreshing access. Exact session, acting-member, original-authorizer, sender and revision guards apply at claim and completion; a 30-second completed-check cooldown bounds repeated requests. Expired leases can recover, while old responses cannot overwrite disconnect/reconnect or newer checks. Completion and `PROSPECT_GOOGLE_CHECKED` activity are atomic. Check status is `unchecked`, `healthy`, `temporary` or `reconnect`; active leases and access expiry are separate read-time states. Temporary failures retain encrypted tokens, including validated rotations obtained before a later identity-probe failure, without claiming currently verified access. GET status makes no provider request. No check sends email, starts a job or continuously monitors revocation.

Timing preview reads a current saved draft and checks its profile/sender/draft revisions. It calculates the first recipient-local weekday time from 09:00 inclusive to 11:00 exclusive, then three and five further business days at the same local clock time. IANA/Intl handles DST; invalid, missing or repeated civil times fail explicitly. Holidays are not excluded. Nothing is persisted or queued. Provider consent is tested locally with injected responses; live consent and explicit refresh are verified; send/reply acceptance remains pending. See [Google setup](GOOGLE_CONNECTION_SETUP.md).


## Controlled introduction receipts (P3C2 local implementation)

`prospect_deliveries` currently represents controlled tests only, never cold-outreach Results or a scheduled sequence. Migration `0031` adds one permanent receipt per workspace/prospect, a same-workspace immutable approval reference and stable RFC Message-ID. Sender/recipient/subject/body are read from the approval snapshot. Identity cannot change or be deleted; all state and provider outcome fields remain relational.

Transitions are prepared → submitting/cancelled; submitting → accepted/uncertain/cancelled; uncertain → accepted. Cancellation after a claim occurs only when the final preflight fails before provider transport. There is no transition back to prepared, no replacement receipt for the same prospect, and no automatic retry. This conservative controlled-test restriction is not a general sequence-rescheduling policy.

Preparation, cancellation and submission claim each use current workspace Owner/Admin authority and atomic activity writes. Submission also binds the exact current approval, checked recipient source, profile/draft/sender versions, acting-member stamp, original Google authorizer, grant revision and live session. Dedicated development/loopback/r2-dev settings must enable precisely this receipt and recipient. The final authority/expiry check is immediately before transport; it cannot recall a request already submitted. Provider outcome recording deliberately survives revocation in flight; response reads still require live authority.

A failed response or process/database interruption cannot unlock submission. Recovery checks only the original account's Sent message matching the stable RFC Message-ID, with matching sender/recipient/subject metadata. No match, multiple matches, unavailable provider or mismatched metadata stays unresolved. Provider message/thread IDs and acceptance time record Google acceptance, not inbox delivery. No follow-ups, inbox/reply polling, Results counters or legacy connection access are implemented by this slice.


## Verified provider identities (P3C3D1 local implementation)

Migration `0033` adds `prospect_delivery_identities`. Each immutable record belongs to one accepted delivery and workspace, and stores the original Google account, exact accepted provider message/thread, valid returned RFC Message-ID, verifying membership, connection/sender revisions and verification time. Unique indexes prevent assigning the same returned RFC or provider message to two deliveries within a workspace/account. Same-workspace receipt/member foreign keys and an accepted-account/provider receipt trigger preserve the association; update and delete are forbidden.

The existing exact-thread reader supplies identity only after validating the unique accepted SENT message, original thread/account and approved sender/recipient. Google may return an RFC identity different from the submitted `prospect_deliveries.message_id`; the submitted receipt is never rewritten. The returned identity is server-only and is not added to reply/delivery DTOs or activity metadata. The existing delivery DTO continues to contain its original receipt fields.

The guarded reply completion batch atomically registers this identity, emits one `PROSPECT_DELIVERY_IDENTITY_VERIFIED` creation event, deduplicates observations and completes the saved check. Replays preserve the first verification provenance. Identity or observation-association conflicts finish unresolved/held without partial new identities/observations; prior records remain intact. Stop, authority/revision changes and replaced/expired leases still win. Empty verified snapshots cannot clear an earlier hold or authorize sending. Existing accepted deliveries are not backfilled from submitted IDs.

This association is the foundation for subsequent discovery, not mailbox coverage or a confirmed-human/hard-bounce classification. Broader mailbox discovery, DSN validation and recovery remain unfinished. Original-submitted-ID-only uncertain-send recovery stays unresolved when it cannot find a verified match; no resend is permitted.

### Saved discovery runs (P3C3D3 local implementation)

A mailbox review belongs to one workspace and connected account. `prospect_discovery_states` stores its revision, current lease, immutable startup baseline, incremental history cursor and coverage hold. `prospect_discovery_runs` stores immutable actor/session-membership stamp, connection/sender revisions, source cursor and one terminal result. `prospect_discovery_targets` snapshots each accepted delivery, its registered identity (nullable when registration is incomplete), and the held reply-state revision. Direct workspace and composite relationship foreign keys enforce isolation. These are relational operational records; the temporary JSON comparison used by SQL is not another stored ledger.

The explicitly enabled local domain command claims a 90-second lease with revision matching and a 30-second completed-run cooldown. It atomically holds all 1..100 accepted recipients before provider access. Existing stopped/held evidence remains intact and the existing address-wide recipient protection derives its block from those reply states. Any outstanding exact-thread lease, including an expired lease awaiting exact-thread recovery, blocks mailbox claiming. The complete accepted delivery/identity set, reply revisions and current authority must still match before transport and completion. Changed authority, deliveries, stops or expired/replaced leases cannot advance progress; a later claim supersedes an expired run.

Startup reads only the verified account profile baseline and records coverage as `unverified`. It does not backfill earlier mail. Subsequent bounded history walks atomically save observations, their deduplicated activity, terminal run and progress. Gaps, incomplete identities, unattributed outcomes and collisions keep the old cursor and a sticky `gap` coverage hold. Empty success cannot clear either recipient holds or gaps. The local status DTO omits raw cursors, provider/identity IDs, tokens and snapshots. No route, job, broad live mailbox access, send grant or hard-bounce classification is enabled. Historical coverage and explicit gap/unattributed recovery remain later acceptance work.

### Historical collection proposals (P3C3D4A)

The provider-only backfill collector derives its time range from the earliest accepted delivery in the complete validated workspace/account registry. It lists available messages with Spam and Trash included, then reads bounded approved metadata. Exact registered ancestry produces delivery-linked observations. Valid incoming messages with no known root produce separate unassigned proposals containing only provider message/thread IDs, received time and needs-review classification. No subject/address matching, raw headers or mail content is retained.

These server-only results are proposals, not new operational records. They do not update D3 progress. Terminal listing records enumeration completion while coverage remains unverified, history catch-up remains false and no next cursor is offered. Ambiguous roots, malformed identities/ancestry, missing metadata, page/byte/time limits and provider errors return no partial proposals. The normal history reader retains its strict all-or-nothing unattributed-message behavior. A future guarded recovery command must persist proposals with current authority and target snapshots, then establish explicit coverage/catch-up evidence before any review or gap-resolution UI.

### Saved recovery evidence (P3C3D4B)

Recovery reuses the account discovery claim, current authority/lease guards, recipient holds and complete delivery/identity/reply snapshots. Immutable run kind distinguishes recovery from normal discovery. A recovery captures an account-profile history boundary before available-message collection, then walks changes from that boundary. Each stage checks the current claim and authority. Matching/unassigned evidence is merged only when repeated provider identity, time, assignment and classification agree.

`prospect_recovery_collections` records collection start time, captured history boundary, explicit complete/unresolved catch-up result and exact assigned/unassigned counts. `prospect_recovery_messages` stores immutable relational proposals tied to that collection and, for assigned evidence, the registered run target. Assigned observations reuse the canonical deduplicated reply observations; unassigned proposals carry no prospect/recipient guess. Collection, messages, canonical observations, activity counts and terminal run seal atomically. Database guards require complete counts and canonical observations before sealing and reject post-terminal insertion or identity changes.

A failed catch-up can retain successful prior collection with an explicit unresolved boundary. Recovery never advances or resets D3's operational cursor and cannot finish as a cursor-advancing checked run. Coverage and all recipient holds/stops remain conservative. The current Owner/Admin summary returns only status, times and counts, without provider/history IDs or raw correspondence. Main local recovery remains disabled; no route/job or live mailbox read belongs to this domain slice.

### Current-account mailbox review (P3C3D4C)

`/prospecting/mailbox` is an Owner/Admin page linked from Overview and Sender setup, sharing Overview navigation. `GET /api/bloomops/prospecting/mailbox` selects the current workspace/connected account server-side and returns safe account/revision/count/reason/latest-attempt/last-five-collection facts. Read access never contacts the provider or starts a run. Repeated authorization predicates scope deliveries and collections; grant health and token lifetime determine action availability. Missing verified identities direct the user to conversation checks. No raw provider/history identifiers, correspondence, token boxes or identity snapshots are exposed.

POST uses existing same-origin/session/structured-body/access boundaries (4096 bytes) and the accepted recovery command's exact acknowledgement/account/revision contract, lease and target guards. The UI prevents duplicate submission, refreshes status after a returned outcome and focuses feedback. Collection, catch-up and historical coverage remain separate; collected or checked labels cannot grant sending or clear a hold. Main recovery remains disabled until the separately scoped live acceptance. No schema change.

### Delivery-status field parsing (P3C3E1)

`parseProspectDeliveryStatus` accepts only an already-decoded ASCII message/delivery-status field block. It returns parsed canonical final/original recipient addresses plus reported Action and enhanced Status code, or an unresolved result with no partial recipients. It has no caller, provider/MIME reader, storage, automatic verdict or workspace association. Reporting-MTA is required and bounded DNS syntax is checked; diagnostic strings, envelope IDs, MTA names and unknown extension values are discarded. Required/duplicate/misplaced fields, unsupported address grammar, malformed status codes and input/recipient/field limits fail closed. Recognized optional standard MTA/diagnostic/date fields are validated against documented conservative subsets before discard. The unfolded-value limit counts raw whitespace before normalization. Reported action and status remain independent, including failed4.x.x after retries end.

These are untrusted input facts. Matching an address is not delivery provenance. Future integration must verify the exact accepted message's registered identity and immutable recipient before interpreting a report, and define authenticity/ambiguity handling before a hard-bounce stop. No coverage, reply or sending state changes. The conservative grammar and limits are documented in [P3](phases/P3.md#p3c3e1-acceptance-bounded-delivery-status-field-parser).

### Delivery-report MIME association (P3C3E2)

`associateGoogleDeliveryReport` interprets a supplied bounded Gmail MIME tree and a caller-owned saved candidate/complete accepted registry. The shared `registeredReplyIdentities` checks the same workspace/account, accepted receipt, registered returned ID and verified provenance as discovery; history readers additionally retain their cursor gate. The legacy Gmail human-body extractor was inspected but not reused because its recursive text/HTML fallback is unsuitable as evidence parsing.

Only a top-level three-part delivery-status report is supported: opaque human text/HTML/alternative, inline E1 field data and returned original headers or bounded raw message/rfc822. A future provider reader must cap responses before parsing; this helper budgets recognized MIME fields/data and never fetches missing attachments. Unsupported trees/encodings, duplicate routing fields/parameters, malformed DSNs and contradictory recipients return no proposal. Raw original-message body text and human explanation are never attribution sources or output.

The candidate message/thread/time must agree with its saved workspace/account context and remain incoming. Returned original Message-ID, sender and recipient must match exactly one immutable accepted message; final/original DSN recipients must also agree. Shared threads, addresses, subjects, daemon names and envelope IDs cannot substitute for returned identity. Contradictory known outer ancestry and sent-identity reuse are rejected. Successful output contains only delivery/prospect/provider association, time, reported action/status and explicit unverified authenticity/held state. This is not a confirmed bounce, authenticated report, database observation, stop, hold release or send grant. See [P3](phases/P3.md#p3c3e2-acceptance-bounded-mime-report-association) for exact limits and unsupported forms.

### Exact delivery-report provider read (P3C3E3)

`prospect-delivery-report-provider.mjs` reuses `readGoogleReplyJson` and E2's shared candidate/registry validation and MIME association. It reads one saved candidate with a fixed authenticated-account message GET, full MIME payload and explicit top-level field selection. Caller context is cloned before network waiting; saved canonical time and registered sent-ID collisions fail preflight. One15-second/256KiB envelope covers the response; redirects, retries, profile/list/attachment fetches, raw/snippet fields and automatic refresh are absent. Errors discard content; only safe held/unverified facts escape. The MIME subtree is requested intact so unsupported depths and extra parts cannot be hidden by projection.

This is an uncalled adapter, not a mailbox command or authorization boundary. Current workspace membership, original-account grant/token binding, saved candidate provenance, lease, post-read revalidation and durable review remain the future domain caller's responsibility. There is no body persistence, automatic bounce/stop or hold release. Normal live access remains disabled; the earlier one-run header-only approval does not authorize this full-payload reader against real mail.

### Guarded durable report evidence (P3C3E4)

`prospect-report-checks.mjs` checks one server-read terminal recovery message. Local development/r2-dev, exact discovery workspace/account and exact report run/message enablement are all required, alongside explicit reviewed input and current Owner/Admin session/member authority. Existing `replyAccountAccess`, `liveActor` and `liveGrant` retain healthy original-account token, scopes, sender revision and authorizer checks. Before reading, all1..100 accepted messages must have verified identities and settled held/stopped reply states; the discovery lease must be settled.

Additive0036 stores report checks and immutable relational target snapshots. A single checking row per account acts as a90-second lease; the atomic claim increments each reply revision without changing hold/stop evidence. Thirty-second completed-check cooldown and expired-check supersession bound explicit retries. Current authority/grant, unchanged discovery revision, exact complete accepted/identity set and every reply revision guard pre-read and completion. Concurrent thread/mailbox work or manual stops invalidate late results. Terminal safe evidence and activity commit atomically; failures retain no partial evidence. Previously associated provider messages cannot be re-read through another collection.

Only check provenance, target identity/revisions and reported action/status/association facts persist. Recovery candidates and canonical observations remain immutable; conflicting prior assignments/observations cannot be silently reassigned. Report evidence remains unauthenticated, held and separate from a confirmed bounce/manual stop. SQL guards protect same-workspace candidate/account/target links, immutable provenance/targets/terminal records, one active check and complete nonnull terminal fields. No body/header/diagnostic/recipient/token storage, raw-data DTO, cursor/coverage change, automatic stop/resume, HTTP caller or enabled live setting is added.


### Delivery-report review (P3C3E5)

`/prospecting/mailbox/reports` reuses the current-account Owner/Admin mailbox authority and adds a bounded read model over saved recovery candidates and terminal report checks. The newest saved provenance wins for repeated messages; at most50 distinct candidates are returned, with an explicit truncation indicator. Raw run/message IDs and correspondence never enter the DTO. A SHA256 reference over workspace/account/run/message is an opaque selection, not authorization; the server resolves it against the current authorized saved collection.

GET revalidates current membership/account/sender after asynchronous reads. POST requires same-origin, strict4096-byte structured input, current revisions and explicit acknowledgement of reading report contents, then calls E4 with server-derived provenance. E4 still owns the exact local enablement, live grant, claim and completion guards. Merely opening or reloading the screen makes no provider request. Existing holds remain, and associated reports show only the reported outcome, status code, time and unverified authenticity with a link to the conversation. Association is not human confirmation or verified historical coverage. No new schema, body/header persistence, automatic stop, hold release, send or production action.


### Report-linked human stop (P3C3E6)

The report review DTO includes the matched conversation's current reply revision, safe existing stop decision and whether an associated failed5.x report is eligible for human review. The report itself remains unauthenticated. A strict same-origin confirmation endpoint resolves the opaque selection server-side and requires current workspace/account/sender revisions, the exact reply revision, an explicit personal-review acknowledgement and a nonempty bounded evidence note. No provider access is required; an unhealthy token does not prevent a protective stop. Disconnect/account replacement or changed revisions invalidate the report-linked command.

The existing permanent stop command rechecks the associated report's exact workspace/prospect/accepted delivery/account, failed5.x outcome, current account/sender revisions, active membership/session and reply revision inside its atomic batch. Existing reply stop fields remain the operational authority and continue recipient-wide protection across duplicate prospects. The append-only PROSPECT_OUTREACH_STOPPED event records source=manual_report_review and its safe reportCheckId as audit provenance; no provider message/run IDs or correspondence are added. This provenance is not another operational JSON state machine. The original independent manual-stop path remains supported and rejects all report-linking fields. Only the server-resolved report confirmation path can supply provenance to the separate internal report-stop entry point. Replay cannot add another decision, and event/state failures roll back together.

Human confirmation never authenticates the report, changes its terminal association, rewrites mailbox observations/coverage or releases another hold. No schema change, send, provider read or automatic bounce classification.


### Reviewed monitoring checkpoint (P3C3F1)

A checkpoint establishes the operational starting cursor for a future explicit check. It does not verify older mail, start a background monitor or release any hold. The source must be the latest terminal recovery collection, completed within five minutes, with complete catch-up and zero unassigned messages. Current account/sender/grant, exact accepted identity set (1–100) and reply revisions must still match. A current Owner/Admin session, discovery workspace/account gate, disabled-by-default local checkpoint flag and explicit acknowledgement are required. The browser submits an opaque SHA256 selection and current revisions; only the server resolves source provenance. No provider request occurs.

Additive migration0037 adds immutable `prospect_monitoring_checkpoints` linking a checkpoint discovery run to its original recovery collection, with same-workspace foreign keys and unique source consumption. Existing claim, target, lease and completion guards protect the operation. SQL constrains the recent matching collection and permits only its completed catch-up cursor. The operational cursor advances monotonically; an earlier baseline, gap/uncertainty, original recovery records and held/stopped decisions remain unchanged. Recovery alone still cannot advance progress.

An incomplete claim is not a saved checkpoint. The read model reports the most recent successful terminal point separately from a newer unfinished attempt; only terminal completion emits `PROSPECT_MONITORING_CHECKPOINT`. Replay, changed authority/account/targets/replies, competing work, expiry and rollback cannot promote a late result. Mailbox review labels the collection, saved starting point and unresolved older mail separately. Verified historical coverage and held-conversation resolution remain unfinished.


### Workspace Pages (P4A)

Canonical `bloomops_pages` is separate from inherited `pages`. It owns the current workspace document title, HTML body, revision, immutable creation request and timestamps. No legacy document is copied or shared implicitly. A unique workspace/request key makes repeated creation idempotent; SQL protects immutable identity and requires the next revision on updates. The body is bounded to1,800,000 UTF8 bytes without truncation. Lists return50 metadata rows with pagination and no document bodies.

Current Owner/Admin membership in an active workspace is rechecked in domain reads and atomic writes. BloomOps routes apply current session, role, same-origin and structured body controls. Same-workspace revision predicates and safe PAGE_CREATED/PAGE_SAVED activity commit in one D1 batch; stale saves cannot overwrite another edit or leave misleading activity. Other internal roles see an explanatory Pages landing screen without document access. Client/portal/public sharing and operational projections are not enabled.

The PageEditor wrapper reuses RichEditor's supported parser, serializer, blocks and slash menu. Its optional onChange callback enables serial autosave while legacy defaults remain unchanged. The Bloomsi wrapper disables legacy database node views/insertion; stored configuration remains in the static node without legacy API calls. Editor code is dynamically loaded on the document route, outside the Pages list and other app areas.

A browser-tab draft is keyed by authenticated user, workspace and page. Edits during an in-flight save are sent next against the returned revision; storage is cleared only after all current changes save. Failed or conflicting requests retain the draft and stop automatic retry. Recovery at a different server revision requires explicit review; users can download the editable draft or choose the saved version. Storage failure is visible, and navigation warns while edits remain unsaved. This is tab-local recovery, not a cross-device backup or version-history service.

...
The shell queries `EXISTS` via a one-row read and displays Home/Content navigation only if any Content is currently readable, including older Published Content. Every ordinary navigation requests a fresh shell. A direct empty list remains useful and carries no hidden count. Projects, onboarding, Deliverables and B5 Files keep their existing Home projections.

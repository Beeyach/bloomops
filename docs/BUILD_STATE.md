...
**PERF4 correlated server timing is paused at its configuration/diagnostic-output decision on `perf/perf4-server-timing`. PERF3 remains operationally OPEN; PERF4 remains OPEN; D2 remains BLOCKED.** Exact main/application baseline is `2b892275abf0856dff10f31d49b2ff2da57acd5f`; the continuation started at `e9030164d2602fe944d36d6cd5631af9573565a4`. Accepted signed-in evidence places most delay between single-RSC response headers and body EOF but does not prove the dominant live server phase. This supersedes the historical implementation/audit-pending status below without closing PERF3.

## PERF4 inactive primitive and decision (2026-09-11)

Read [PERF4_RESULTS.md](PERF4_RESULTS.md) and [sanitized numeric evidence](evidence/PERF4_server_timing.json). The continuation prompt explicitly permits a tested reusable primitive when the required runtime boundary needs a configuration decision. The generated OpenNext entrypoint and A11's disabled telemetry prevent a complete response/stream correlation candidate within current authority. The new collector is unimported by deployment, default off and development-opt-in only; staging and production cannot activate it. It prepares fixed response timing fields, a fresh opaque ID, bounded phase offsets/durations and a matching final event, but installs no Worker wrapper, D1 adapter, logger or product hook. Existing product/auth/query/config sources are unchanged.

Verification: 30 focused primitive tests; 4,816/4,816 full tests; Next/OpenNext builds; native Systems 23 and Home/projections 34; local zero-to-current 22; syntax/diff and built-code exclusion checks passed. An external bundled Python alias was needed by five inherited packaged-skill tests. Paired in-memory issued-session/projection measurements retain 40 off/on samples per Home/Systems route and representative/exact-3× scale: median deltas 0.040–0.132 ms, 134–137 uncompressed additional header-field bytes. These are provisional primitive costs, not Worker/RSC/browser or log-delivery overhead; integration overhead remains unmeasured.

No PR is opened: the staging candidate is incomplete. Next decision is authorization for a staging-gated custom Worker entrypoint and a separately audited sanitized completion-event destination, with retention/removal defined and existing telemetry privacy preserved. After implementation, actual Worker/stream privacy/revocation/overhead checks, independent exact-head audit, separately authorized merge, both exact-SHA workflows, and correlated signed-in staging measurements remain required. No merge, deployment, infrastructure/configuration change, remote data mutation, external message, product optimization or D2 work occurred. The accepted prior local diagnosis commit `dd9fe8e7a40b48724b9a012bc6c09c5dac5105d0` was unavailable; its reported evidence is labeled as such, never presented as inspected source.

...
| Finance Lite | F3 in progress on feat/finance-lite-20260916 from current main fe6488c. Contract phases/F.md; manual records and normal Team → Finance access administration implemented with additive0055. Native D1 fresh/upgrade20, full7382/7382, completed build and local zero-to-current pass at b3d04e4; hosted disposable D1 run35075249523 also passes, including unchanged global integrity validation and cleanup. PR92 remains draft. Browser verification found pre-hydration input loss; Finance controls now wait for handlers. The first PR CI also exposed the same issue in Reporting new-period dates, being corrected with a controlled delayed-script regression. Latest browser/CI and staging acceptance remain pending; no actual payments/accounting. |

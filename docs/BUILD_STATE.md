...
| --- | --- | --- |
| S01 | Profile actions and sources | FIXED AND RETESTED locally: real evidence/draft anchors, separated truthful helper, field-source alignment. Staging pending. |
| S02 | Niche/Observed facts provenance | FIXED AND RETESTED locally: shared source/verification/time/editor icons; unknown verification remains Not checked. Staging pending. |
| S03 | Email/website provenance | FIXED AND RETESTED locally: saved unchecked source through real editor, wrapped source URL,1440/390 overflow checks. Staging pending. |
| S04 | Shared labels and values | FIXED AND RETESTED locally: computed13px semibold labels versus readable15px values; shared form/fact tokens. Staging pending. |
| S05 | Multiple/custom purchased services | IMPLEMENTED AND VERIFIED locally at1eb87a6; staging pending. Canonical engagements plus additive immutable receipt items; custom reusable offerings restricted to current Owner/Admin and workspace-settings API access. Native concurrency/zero-row rollback and completed-Worker three-service create/edit/remove/review/save/reload checks pass. |
| S06 | Client prospect search/prefill | IMPLEMENTED AND VERIFIED locally at1eb87a6; staging pending. Add Client searches authorized current-workspace prospects, reviews saved identity and enters canonical explicit sale handoff; manual entry remains separate and is never overwritten. Existing conversions link to their client; browser confirms selection has no side effects and preserves independent manual edits. |
| S07 | Actionable handoff recovery | FIXED AND RETESTED locally; staging pending. Live synthetic reproduction on af71130: contact missing and Kajabi default warning have no repair action. Reuse guarded profile editor; keep setup in a separate tab so handoff choices remain intact. Local completed-build repair/retry/re-review/conversion passes; staging retest pending. |
| S08 | Compact Work views | FIXED AND RETESTED locally: bounded Actions/Projects links; broad responsive/zoom sweep passes. Staging pending. |
| S09 | Social platform selector | IMPLEMENTED AND VERIFIED locally: custom platform round-trip, All and Clear; authorized option sources tested. Staging pending. |
| S10 | Approved compact Social views | APPROVED PATTERN RETAINED. Individually bounded Content list/Calendar links, no full-width rail or black underline; rendered desktop/mobile/zoom checks pass. |
| S11 | Quiet filters | FIXED AND RETESTED locally: quiet filter surface and responsive controls; staging pending. |
| S12 | Writing-first Page tools | FIXED AND RETESTED locally: toolbar disclosure closed by default, Escape/focus, viewport bounds1920/1440/768/390; writing/selection preserved. Staging pending. |
| S13 | Page movement and pointer controls | NOT REPRODUCED with three distinguishable blocks on Windows staging and local build: move/undo/save/reload and non-forced pointer click work. Inapplicable one-block movement is now de-emphasized; final staging retest pending. |
| S14 | Workload explanation/refresh | FIXED AND RETESTED locally: full counting rules retained in explanation; small named Refresh. Staging pending. |
| S15 | Compact Team views | FIXED AND RETESTED locally: three real compact view links plus authorized Finance utility. Staging pending. |
| S16 | Workspace dropdown | IMPLEMENTED AND VERIFIED locally: bounded authorized dropdown/View all; cancelled dirty navigation makes zero selection writes; confirmed switch makes one. Staging pending. |
| S17 | Friendly empty-state icons | FIXED AND RETESTED locally: modest supplementary icon; distinct empty/filter/denial messages retained. Staging pending. |
| S18 | Settings purpose/action gaps | FIXED AND RETESTED locally: plain delivery-setup purpose; action left edge and >=16px description gap asserted. Staging pending. |
| S19 | Workspace chooser identity | FIXED AND RETESTED locally: full signed-in identity and secondary Sign out visible,1440/390 no overflow. Staging pending. |

First correction checkpoint (not deployed): contact repair, template setup return, compact Work/Team controls, actual Social platform choices, Page toolbar tools, and safe workspace dropdown are implemented. The single in-session source review found two races (selection before accepted navigation; contact edit during conversion), corrected before release. Completed-build verification at8a6b941 passed45 focused browser checks and the broader UI sweep, including real zoom, pointer controls, mobile tools and cancel/confirm workspace switching. S05/S06 is a later unverified build input. Local full suite:7396/7396, exit0, no skips with a temporary `python` alias to installed Python3. The first execution's five `python ENOENT` tool failures remain recorded outside the worktree; no tests were waived. Old completed-build browser reproduced the missing contact repair action. No real pilot records were mutated. Subsequent S05/S06 work is tracked above. At1eb87a6 full7412/7412, receipt-focused35, native13 and local migration/idempotency passed. Completed-build focused browser55 and broad UI357 passed with no runtime errors. One earlier build exit143 is retained, not counted as success; the recorded retry exited0 before browser launch. Visual inspection then found purchased-fieldset specificity and Settings action alignment gaps; final correction adds geometry checks. PR98 validation uses isolated local resources. Final staging remains pending.

S05/S06 implementation choices from this owner task: up to ten distinct purchased services, separately normalized package/scope, one transactional client/contact/conversion with legacy receipt compatibility. Custom offering names are editable with timestamp compare-and-swap; no prices, provider links or default-template renames. New services use common onboarding unless an existing mapped type requires another category. Add Client offers a reviewed prospect entry path beside independent manual entry, rather than silently copying values into an unrelated create-client request. Supported prospect timezone is retained; UTC is represented as the equivalent Client value Etc/UTC, unknown remains unknown, unsupported Client zones receive an explicit preview notice. Generated additive migration0056; applied migrations untouched. Native check command: `node scripts/pilot-handoff-native-local.mjs` (local-only Miniflare, fresh/populated upgrade and simultaneous requests). Focused tests: `node --test tests/bloomops-prospect-multi-handoff.test.mjs tests/bloomops-service-offerings.test.mjs tests/pilot-feedback-ui.test.mjs`; completed-build browser: `node scripts/pilot-feedback-browser-local.mjs` with the existing UI harness environment. Existing PR UI suite includes these scenarios.
...
PR96 merged as `ae5694d` (tree071d5bc equals tested candidateee3c3b1). CI35116293122 passes full7391 and feature checks, including UI298/report159. Staging35118801721 and disposable-D135118801855 pass; no migrations applied (56 retained). Live ae5694d correctly renders saved report Client/Service context, real Onboarding sidebar/checklist, and CSV1/2-row preview.
Final small sibling pass: `fix/app-qa-final-spacing-20260916`, base `ae5694d`,
...
| --- | --- | --- | --- |
| Sidebar → Prospecting | Team Member generic not-found despite HTTP200; Owner/Admin access works | Explicit restricted-role explanation with workspace recovery; existing authorization unchanged | FIXED AND RETESTED on0039: same B session sidebar/denial/API403; Owner sidebar usable. No authorized Owner failure reproduced. Local genuine empty, restricted, history/reload regressions pass; live fresh Owner session, workspace chooser, back/forward/reload PASS |
...
| Shell / notification / scope | No new delivery defect observed | None | Live separate B Page comment → A reply → B unread1 → rendered notification click → correct Page/discussion and unread clears PASS on d294. Grant revoked, inbox/count and old-link access removed. Current0039 search/mobile, bell Escape/focus return/View all, preferences toggle/reload/restore PASS |
| Prospect records / Skills / CSV | CSV preview says "Review 1 rows" | Reuse singular/plural formatter | Live d294 create/invalid timezone/edit/reload/search/views/Clear/CSV import/Skills task and context downloads PASS. Singular fix live ae5694d one/two-row preview PASS; preview-only records were not imported |
| Clients / Services / Onboarding / Work | Global Onboarding falsely says checklist unavailable; per-Client checklist works | Canonical authorized Client list links directly to each real checklist; no new engine or access grant | Live Client create/edit long name/Service create-edit PASS. Global recovery implemented with empty/limited-list tests and actual sidebar→activated checklist regression; live ae5694d sidebar to six real requirements PASS. No invitation sent to example.test |
...
| Ads / Systems | No new setup mutation defect observed | None | Live Settings GHL binding disable/restore with stale409 recovery PASS, original true restored. Synthetic Ads Project created through Ads→Work, detail/status/edit/reload/Ads filter/reset PASS. Real creative idea→Script/edit/reload PASS on0039; no provider calls |
| Pages / comments / sharing | No new saved-writing/access defect observed | None | PASS d294: parent/subpage title/body save/reload; Move root/back; comment/reply/resolve; genuine B comment grant and notification click, then revoke/stale denial. 0039 template capture/create independent copy, context, heading/block keyboard move,390/768 Windows and offline failure→Retry→saved reload PASS |
| Team / Finance capabilities | No permission defect observed | None | PASS d294: People, workload and department show actual synthetic Action; B Finance None→View gives read-only200, restored None gives404. Department heading has correct DOM/rendered space. Real sibling defect: refresh touches empty results, now corrected with scoped results-action gap and explanation spacing |
...
Corrected source `1fac133` passes focused26, native46, full7391/7391 exit0 with no skips, completed Cloudflare build exit0, UI298 and reporting159 built-Worker checks with zero runtime errors. Artifact hashes and served assets match the completed build. Remote validation and first live retests passed as recorded above. Final sibling patch changes only Team result/explanation spacing and newly-created report provenance labels to parentheses. Existing immutable snapshots, PDF rendering, metrics and stored user punctuation are untouched. Focused24, full7392/7392, completed build, UI303 and reporting159 pass at4d4c2c6. Ready-event35118780727 separately failed in Playwright route interception cleanup (route already handled), after successful report assertions and before later scenarios. Its raw failure is retained. The report harness now drains released handlers before removal; no assertion or failure exit is suppressed. Rechecking that harness against the same completed build before the next PR.
...
| --- | --- |
| Core Clients/Services/onboarding, Work Core, Social, Systems | Existing A–D foundations usable on main; supported onboarding-default administration is merged in PR83; genuine staging activation/contact acceptance/single-account Project access passed at62ae623 (issue80 multi-account clarification remains). Preserve existing engines. |
...
| Reporting drafts | N3A and creation-retry correction merged in PR81 as ba83c2d; full7264 and feature CI pass on equivalent tree; staging deployment/remote-D1 and genuine QA GHL/social save/edit/private preview passed. |
| Reporting completion | Reviewed CSV(PR82), supported onboarding/portal activation(PR83), immutable publication/revision/withdrawal/client access/PDF and empty new-period reuse(PR84), archive/private restore(PR85), and compatible saved comparisons/charts(PR86) are merged and staging verified. The PR86 release1973e2f reached52 domain migrations. PR86 candidate ff30302 has the same tree as merged1973e2f; full7301/7301 and feature CI35047921174 pass (report58/native45/browser149, notifications60/preferences23). Staging35049258412, disposable remote-D135049258348 and ready-event35049255092 pass. Genuine separate internal/portal actors passed19 comparison checks; actual three-page GHL/social PDFs show correct -32/-17 differences and exclude private source notes. Archive16 and publication31 live checks remain retained. No provider, custom-metric designer, scheduled sending or PERF3 acceptance is implied. Consolidated stabilization remains ahead. |
...
| Reusable Work and Pages / Operations | Reusable Work is merged in PR87 as5325819, tree-equivalent to validated candidatebf6607b. Full7323/7323 and feature CI35053689479 pass (Work21/native22/browser23; report browser149, notifications60/preferences23). Staging35055162518 and disposable remote-D135055162489 pass; migration0052 brings staging to53. Genuine internal/portal actors passed26 live checks, including saved authoring, exact date/dependency preview, real generation/destinations, retry, second event, immutable prior jobs, revision, retirement/restore and portal denial, with zero runtime errors. Ready-event35055150872 also passes. Pages record context is merged in PR88 asdd11062, tree-equivalent to validated candidatebe43ae9. Corrected full7339/7339 and feature CI35059987518 pass, including the real concurrent report archive/read correction (report60/native45/browser151, Page13/native15/browser20). Staging35061564915 passed; migration0053 brings staging to54. Genuine internal/portal actors passed15 live checks: saved context, unchanged writing, Project→linked Page navigation, narrow layout, independently authorized portal Project-summary link and revocation. Temporary Project visibility and Page grant were restored. Disposable remote-D135061564859 failed with a network fetch during unchanged inherited migration053; cleanup succeeded and its single infrastructure retry passed. Ready-event35061561822 also passed both jobs, with full7339/7339. Page templates are merged in PR89 as994cb8a, tree-equivalent to validated candidateedc5ca4 and feature437a3a5. Corrected CI35063271184 passes full7355/7355 and all feature checks. Staging35064983120 and disposable D135064983122 pass, migration0054 brings staging to55. Genuine Owner/portal checks confirm saved-writing capture, five-width persisted preview, keyboard-created private Page, independent edits, immutable versions, retirement/restore and portal denial; source writing restored, zero runtime errors. The helper lost the creation response after navigation; it resumed from that same successfully-created Page without duplicating it. Live exact-request retry was NOT RUN because its UUID was not retained; native and CI retry checks remain the evidence. Original partial and resumed receipts are retained. Existing P4A–I and N1F editor/recovery remain complete. Remaining Operations/Finance scope stays open. |
| Team workload / Operations | F1 Action workload merged in PR90 asbafd6e0, tree-equivalent to featuref5f688e and executed candidate42d3218. Full7362/7362 and feature CI35063471857 pass, including workload7/native9/browser13. Staging35065785639 and disposable D135065785618 pass; existing55 migrations are unchanged and application is served asbafd6e0. Genuine internal/portal actors passed16 live checks: existing Action assignment→summary→keyboard destination, five widths, portal denial, no runtime errors, original assignment restored. Ready-event35065773625 passed both jobs. No migration or capacity scoring. F2 Department work now implements existing Projects/Actions by Social, Ads, Systems and Operations, preserving exact record authority and historical/all-work views; source9baabf7 passes focused64, full7368/7368 with zero skips, completed build and26 native-Worker browser checks, zero runtime errors, unchanged artifacts and5 matching served assets. Labelled record context follows the design contract. PR91 is merged asfe6488c; its tree46418bd7 matches executed candidate531fb5f and feature43d4e84. Full7368/7368 and feature CI35067483215 pass, with13 Team/department Node and26 native-Worker browser checks. Staging35069421783 and disposable D135069421796 pass;55 migrations unchanged. Genuine Owner/portal sessions passed14 read-only live department checks onfe6488c, with no runtime errors or fixture/settings changes. Ready-event35069418131 completed successfully in both jobs. Broader form recovery remains unfinished. Contract phases/F.md. |
| Finance Lite | Merged in PR92 as52dacbf, tree-equivalent to candidate7f85ec9 and featured5d8b5c. Full7383/7383 and feature CI35078207113 pass; staging35080412866 and disposable D135080412889 pass. Additive0055 brings staging to56 domain migrations. Genuine Owner/Team/portal actors passed22 live checks through normal Finance access management, manual Client/Service records, exact currency totals, editing, archive/restore, five widths and access revocation; original grants were restored and two clearly synthetic records retained. Ready-event35080397958 also passed both jobs. No payments, provider calls or real accounting integration. Evidence: PR92 release comment and reporting-integration/finance-staging. |
...
Recovery checkpoint: Ubuntu-F is registered on F:, Windows F:667GiB/C:118GiB free; ext4 writable and fsync/read/delete probe passed. Local commits backed up in a verified D: Git bundle. The interrupted full/build logs are retained; recovered full7273/7273 passed after restoring the temporary Python alias. Browser tooling is isolated outside worktrees. Onboarding browser coverage now follows default installation through supported activation/contact invitation and genuine portal authentication. Completed buildf29e1f5 passed; final harnesscbf5d7b passed79 built-Worker checks with zero runtime errors, unchanged artifact hashes and5 matching served JavaScript assets, including setup→activation→genuine contact invitation acceptance. Only browser harness changes follow the built source. Native onboarding checks5 pass. PR83 validation/staging setup are now complete; ready-event validation35025722188 is observed separately. No repeat of passing CSV staging writes.

### Stabilization backlog

- Reporting comparison tables wrap aggressively at320px; improve narrow table typography during stabilization while retaining keyboard access and complete values.
- Reporting PDF currently embeds bundled Latin Instrument Sans; unsupported glyphs return an explicit export error. Add broader font fallback before claiming international-script PDF coverage. No silent substitution.
- Recovered WSLg/software-GL process can stall Chromium actionability. With DISPLAY/WAYLAND_DISPLAY/XDG_RUNTIME_DIR unset and BLOOMOPS_DISABLE_SOFTWARE_RASTERIZER=1, the actual publication browser102 passed. No assertions are disabled; failed attempts remain preserved.
...
- Issue80: supported single-account onboarding setup/activation/contact acceptance passed on62ae623. Multi-account read-model versus intentional activation guard remains a separate clarification; reporting uses the proven single-account topology.
...
N2E implementation is locally committed as `1754750705af9175825a4467af5c1673b748c197` in `/home/ary/Developer/bloomops-n2-notifications`, branch `feat/n2-in-app-notifications`, based on current main `f989428ccc6150befb457860b21c468b2ba656ab`. The existing [N2E contract](phases/N2.md#n2e-bounded-contract--useful-in-app-notifications) retains record mentions/prior participants, Page replies with inherited access, and canonical Action assignments. Migration0046 adds scoped inbox state/preferences/mutes. Corrections include preference-reason precedence, saved recipient user identity, source-success guards for zero-row assignment writes, deletion cleanup, monotonic mark-all cutoffs, and settings recovery.

[Local evidence and verification commands](previews/notifications/README.md):22 focused notification tests,273 discussion/Page/Action regressions and29 native D1 checks pass. Full `npm test` is7193/7198 with five independently reproduced baseline failures; none were disabled or weakened. Fresh and repeated no-op migrations pass.39 built-Worker/browser checks pass with synthetic data, including Page/record/task access revocation, keyboard/touch and error recovery. Matching local Home/Clients/Work samples retain11/3/7 statements and two invocations per route. LIVE PERFORMANCE NOT VERIFIED. The exact stamped build exits0. The complete source/evidence packet is ready for independent audit; the milestone is not independently accepted, merged or deployed.

The owner's stop line is independent-audit handoff. No independent audit, push, PR, merge, deployment, shared-data or outreach action occurred. The original dirty D2 checkout and all other worktrees remain untouched. N3/N4, broader recovery, collaboration mail, optional integrations, prospecting and paid/video work remain outside this task.

## Previous checkpoint — N2D accepted in staging

N2C is merged through [PR76](https://github.com/Bloomwired/bloomops/pull/76) as `0c9bcc940d6ffb1828df715b21cc6543e32bcea2`. Its tree exactly matches accepted headc08dc0f; only docs/evidence followed reviewed runtimef13bb8f. [Deploy staging34910695046](https://github.com/Bloomwired/bloomops/actions/runs/34910695046) passed on that merge. [Zero-to-current34910695031](https://github.com/Bloomwired/bloomops/actions/runs/34910695031) passed on that exact merge, including the second migration pass and cleanup. Both merged-main gates are closed. Live read-only checks confirm runtime0c9bcc9, healthy staging schema45 and own profile without desktop overflow. The prior103 focused checks,35 actual Worker/browser checks, clean Sol High review and ten feature-staging checks remain valid. No repeated review or broad tests were needed for an unchanged merge.

A separate worktree `/home/ary/Developer/bloomops-n2-record-discussion`, branch `feat/n2-record-discussion`, starts from0c9bcc9. N2D's bounded [contract](phases/N2.md) is prepared from the actual Page comments/activity model and client/project/action/deliverable authority. It includes typed-parent threads, internal/client visibility, replies, own edits/removal, resolve/reopen, authorized mentions, safe photos and portal/preview readers. Current runtime has no generic work/client comments table; the additive shared model must preserve existing Page discussion. N2D now has additive migration0045, fresh parent/audience guards, transactional comments/replies/mentions, own edit/removal, resolve/reopen, protected author photos and full-page internal/portal/preview routes with entry links.79 focused Node checks and the OpenNext build pass. The disposable local zero-to-current check passes, including the unchanged second pass. The final built Worker passes43 browser checks with zero runtime errors, covering five widths, keyboard/touch, real saves/retries, concurrent-edit recovery, portal/preview permissions and photos. QA corrected early-hydration input readiness/date formatting and a denied Client task URL that returned500. [Visuals, query/bundle measurements and limitations](previews/record-discussions/README.md) are recorded. One fresh Sol High review found no material issues; no re-review was needed. A subsequent CSS-only correction uses the existing Bloomsi heading-font token; the rebuilt Worker passes11 additional focused visual checks. Committed/pushed as `6d608286d0978f0ca99a29369186014dbe122246`. [Draft PR77](https://github.com/Bloomwired/bloomops/pull/77) includes the visual previews. [Staging workflow34914352231](https://github.com/Bloomwired/bloomops/actions/runs/34914352231) passed on that exact commit. **N2D is accepted in [staging](https://staging.ops.gobloomwired.com/discussions/client/097f94a802a8426cb017aa94fcc72d20).** Ten live checks pass: exact runtime/healthy schema46, client entry, empty discussion/read, internal default/empty Post, permitted-person lookup, missing-parent404, phone390/16px/44px, live Mention picker, resolved/return navigation and restored desktop. No staging record or photo writes; populated discussion/storage/permission coverage remains local. **First unfinished task: PR77 readiness/merge and its merged-main gates, then N2E in-app notifications. PR77 remains draft/unmerged.** N2E notifications, N3/N4, broader recovery and optional later integrations/email remain unfinished. Original LTB records/voices, production/DNS, real imports/outreach and paid/video work stay preserved.

## Previous checkpoint — N2C accepted in staging

N2B is merged through [PR75](https://github.com/Bloomwired/bloomops/pull/75) as `4fed444e14b977de621ea4770e4172b872e539c1`; the merge tree exactly matches reviewed head002b675. [Deploy staging34908016439](https://github.com/Bloomwired/bloomops/actions/runs/34908016439) and [zero-to-current34908016352](https://github.com/Bloomwired/bloomops/actions/runs/34908016352) both passed. The public staging version confirms4fed444. Existing219 Node/41 Worker-browser checks, one clean Sol High review and eight feature-staging checks remain the implementation evidence. Live preview coverage is the existing fictional QA client's empty contact picker; populated/permission coverage is local. No staging records were changed during this merge checkpoint. N2B is released.

Active worktree `/home/ary/Developer/bloomops-n2-profile-photos`, branch `feat/n2-profile-photos`, starts from4fed444. The bounded [N2C contract](phases/N2.md) follows existing account-wide `user.image`, FILES storage, Team permissions and Page discussions. N2C is implemented and locally verified: safe256px thumbnails, self-managed crop/save/remove, garden fallbacks and context-authorized delivery in Account, Team and Page discussions/preview.103 distinct focused Node checks pass (102 in the combined suite, with the added bounded-directory test; final photo-domain file24/24). The final OpenNext build and35 actual built-Worker browser checks pass. Five widths, phone hit tests, PNG/JPEG/WebP inputs, real R2 saves/removal, lost-response retry, stale saves, permissions and visible pre-hydration images are covered. Unrelated route bundle estimates are unchanged; each populated image delivery uses two bounded domain SELECTs even with250 extra members, plus request authentication/storage. [Evidence and visuals](previews/profile-photos/README.md). One fresh Sol High read-only review found no material findings; no re-review was needed. Runtime committed/pushed as `f13bb8f231e6f9e3f46828d53315fa6f87f69adf`; [draft PR76](https://github.com/Bloomwired/bloomops/pull/76) includes visual previews. [Staging workflow34910073830](https://github.com/Bloomwired/bloomops/actions/runs/34910073830) passed on that exact commit. **N2C is accepted in [staging](https://staging.ops.gobloomwired.com/profile).** Ten live checks pass: runtime/schema45, Account link, own-profile garden/default controls, desktop1440, temporary PNG crop, phone390/44px controls, Cancel preserving the saved version, missing-author404 and restored desktop. No staging record writes; actual storage mutations and populated author permissions remain local. Worker8811 is stopped. **First unfinished task: PR76 readiness/merge and its merged-main gates, then N2D record-linked discussion. PR76 remains draft/unmerged.** No new migration or dependency is planned. N2D/E, N3/N4 and the broader recovery inventory remain unfinished. Collaboration email, optional integrations, paid/video work, production/DNS and original LTB records/voices retain their boundaries.

## Previous checkpoint — N2B client preview accepted in staging
...
One fresh Sol High read-only review found no material findings; no re-review was needed. **Live in staging on `00f540bc1814498570a2ddedbfe0e7ae1c1d3180` through successful [workflow34907194523](https://github.com/Bloomwired/bloomops/actions/runs/34907194523).** [Draft PR75](https://github.com/Bloomwired/bloomops/pull/75) is open. Eight live checks pass: exact runtime/healthy schema45, client entry/preserved overview, desktop empty picker, unavailable-contact/missing-client404, phone390 layout/44px Back target and return/restored desktop. No staging record writes were made. This QA client has no connected Client account; populated preview and permission/mutation coverage remain local. [QA staging entry](https://staging.ops.gobloomwired.com/clients/097f94a802a8426cb017aa94fcc72d20/preview). **First unfinished task: PR75 readiness/merge and its merged-main gates, then N2C profile photos.** N2B is accepted in staging; the PR remains unmerged. Server checks are per request and post-storage; open-screen revocation is checked on focus/pageshow and every10 seconds, not pushed. External authored actions are inert; inline raster and recognized Bloomsi files are the preview image paths. No staging records have been changed for N2B.
...
The client Overview now leads with purchased services, bounded current projects/tasks, the next readable deadline and unresolved onboarding requests. Date queries inspect eligible records before limiting, preserve client timezones and distinguish overdue/dependency-blocked work. Onboarding preserves required-step progress and submitted/unverified meaning. Current membership, workspace and child visibility checks remain; service/project/task-only assignments do not open the internal client record. Existing client edit, health, contact, activation and tab flows remain available. No new migration, dependency, stored summary, cache or mutation endpoint.

75 focused Node checks, the final OpenNext build and24 built-Worker browser checks pass. One Sol High review identified All projects losing client scope; the link now opens the client's Projects tab and a second-client regression passes. The single focused re-review found no remaining material issues. Five widths, keyboard, real local save/reload, denied access and long-history behavior are verified. Summary execution stays18 SELECTs/567 bytes in the small synthetic fixture with or without250 historical projects; the old overview's200-project tab probe is reduced to one row. Rounded unrelated first-load bundle estimates are unchanged. [Evidence and visuals](previews/client-overview/README.md).

Live staging confirms the exact runtime, healthy schema45, all four sections, desktop1440/phone390 layout and touch targets, preserved client controls, Services/Onboarding/scoped Tasks navigation and missing-client404. The existing QA workspace had no clients; one clearly named fictional QA draft and its required example.invalid contact were created through the canonical API, without activation or invitation. [QA staging preview](https://staging.ops.gobloomwired.com/clients/097f94a802a8426cb017aa94fcc72d20). Populated overview, mutation and permission evidence remains local; live coverage is the empty QA state. Local Worker8809 is stopped.

**First unfinished task: PR74 readiness/merge, then define N2B's read-only client preview.** N2 as a whole is not complete. Profile photos, discussion, notifications and the broader save/recovery inventory remain separate work. Root D2 changes, other worktrees, original LTB records/voices, production/DNS, real imports/outreach and paid/video work remain preserved.
...
Final OpenNext build,35 focused Node tests and90 built-Worker browser checks pass (37 inline recovery,32 updated save regressions,21 quiet-save regressions). Coverage includes the observed conflict, intervening edits, replacement/Undo, multiple drafts, lost responses, failed clears, copy-cap handoff/retirement, authority denial and desktop/mobile. [Evidence and visual previews](previews/sheet-inline/README.md). Unrelated route bundle estimates are unchanged; Prospecting122→123kB. No backend/schema/dependency/workflow changes or new migrations. One fresh Sol High review identified stale recovery actions during text editing and stale local display after Keep saved. Both paths are corrected, with focused browser regressions added; the single permitted focused re-review accepted the editor fix and found a same-row partial-data revision issue. Direct confirmation now merges the canonical profile read for that write before applying its field/revision; the held-read regression includes a second same-row save. The final correction passes the rebuilt Worker’s37 inline checks plus53 existing save/quiet-save regressions, including the held-read same-row case. It was locally verified after the final permitted re-review; no additional independent signoff is claimed. **Live in [staging](https://staging.ops.gobloomwired.com/prospecting)** on runtime `f22a0b9eab6967b032f2c8c047d7e310f22a4461`; [workflow34893504562](https://github.com/Bloomwired/bloomops/actions/runs/34893504562) succeeded. Seven deployed checks pass: version/health45, one deliberately simulated503 retained inline, another real field save without discard, real Retry/readback, real Undo restoring freshly captured QA fields/sources, phone44px/no overflow and restored desktop. Zero confirmation dialogs. Only the designated fictional QA record was edited; expected edit/undo history and newer revision/time remain. Desktop/phone captures were inspected. Local Worker8807 is stopped. Next bounded task is PR73 readiness/merge before N2. PR73 remains draft/unmerged; N2 is unstarted. Original LTB/voices, root D2/unrelated work, production/DNS, real imports/outreach and paid/video remain preserved.

## Previous direction — quiet prospect sheet saves
...
One fresh Sol High review found no material findings; no re-review was needed. **Live in [staging](https://staging.ops.gobloomwired.com/prospecting)** on runtime `00d73a926a3c56f63d4c6bd58fc54313bf274d0e`; [workflow34887059226](https://github.com/Bloomwired/bloomops/actions/runs/34887059226) succeeded. **17 actual deployed checks pass**: version/health45, consecutive desktop Fit/Platform saves with canonical readback and no discard/review, real Undo and reload, six widths without horizontal overflow, phone save/undo and44px controls, and expanded preselected Outreach interest form without submission. Live desktop/phone screenshots were inspected. Only the designated fictional QA prospect was edited; all profile fields and sources are restored, with expected edit/undo history and newer revision/timestamp retained. Existing user recovery copies/preferences and other tabs were preserved. PR73 stays draft/unmerged and main remains85ab496. Next bounded task is PR73 readiness/merge before N2; N2 has not started. Original LTB/voices, root D2 work and other worktrees, production/DNS, real imports/outreach and paid/video work remain untouched.
...
One fresh Sol High deployment review found no material findings; no re-review was needed. The task-only Worker8804 is stopped. **Live in [staging](https://staging.ops.gobloomwired.com/prospecting)** on runtime `3949fc6577639388d24482f45d19723893c3bf47`; [workflow34883209541](https://github.com/Bloomwired/bloomops/actions/runs/34883209541) succeeded. **12 actual deployed checks pass**: version/health45, Fit review/focus/readable values, Tab to Save, six widths, phone Platform automatic focus/scroll, cancellation, byte-identical sheet rows and correct expanded Outreach form without submission. Live desktop/phone captures were inspected. The tool-created tab lacked the existing authenticated context; opening a task tab from that context resolved verification without owner action. No session-expiry claim is made. Existing user recovery copies and preferences were preserved. Main remains85ab496 and PR73 remains draft/unmerged. Next bounded task is PR73 readiness/merge before N2's client overview contract; N2 is unstarted. Original LTB/voices, other worktrees, production/DNS, real imports/outreach and paid/video work remain unchanged.
...
The final OpenNext build,33 focused sheet/draft tests and57 final built-Worker browser checks pass (54 main plus three true-touch checks). Coverage proves native first-click open without an editor, six widths, platform/custom/Fit review-save-undo, keyboard/Escape/focus, correct Outreach target/form, real synthetic contact/interest saves, unchanged Last contacted for interest, unsupported query rejection and zero provider requests. [Evidence](previews/sheet-fluid/README.md). A native hit-area check found42px inside a44px skin; the final overlay includes its border, and the rebuilt app passes all57 checks. One fresh Sol High deployment review found no material findings; no re-review was needed. **Live in [staging](https://staging.ops.gobloomwired.com/prospecting)** on runtime `faf50daf552dbd9dfa04088de6f70357843a6741`; [workflow34880217396](https://github.com/Bloomwired/bloomops/actions/runs/34880217396) succeeded. **Ten actual deployed checks pass**: version/health45, direct Platform/Fit selection to review and cancel without writes, correct expanded Outreach contact form, six widths and44px phone targets. Desktop and phone captures were inspected. Windows remote tooling does not expose native popup-open state even for the unchanged Rows per page selector; first-click/first-tap popup proof is from the57 final local Chromium checks, with deployed selection behavior independently verified. The task-only local Worker8803 is stopped; main remains85ab496. No backend/schema/workflow/dependency changes. PR73 remains draft; next bounded task is readiness/merge before N2. N2 is unstarted, and original LTB/voices, production/DNS and unrelated work remain unchanged.

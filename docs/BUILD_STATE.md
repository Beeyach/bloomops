...
**Internal pilot — actionable onboarding implemented and locally verified; independent review/publication pending.** The owner continued after the Astra High recommendation. Follow [PILOT.md](phases/PILOT.md): agency setup now supplies each Client's instructions and explicit agreement/upload-folder/booking/access link; unconfigured open steps wait for setup, confirmations pin the displayed revision, and completed history stays intact. The portal uses line icons (no emojis), fewer repeated headings and collapsed completed steps. First unfinished task: one Sol High review, then authorized publication and exact-merge staging/migration gates. The full human pilot remains open; Release D stays closed on `844ac905541df9e6fccb7dc313ab0a8d517da319`, and Release E has not started.

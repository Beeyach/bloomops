...
**Internal pilot — onboarding icon size/alignment locally verified; publication pending.** Owner reports that the new icons are oversized and titles sit against their upper half. The bounded presentation correction reduces artwork from 40px tiles/22px glyphs to 28px tiles/16px glyphs and centers the icon on the title itself, with Required/Optional on a separate row. Cloudflare build and 76 browser checks /17 captures pass, including measured centering at five widths. First unfinished task: publish through the authorized workflow and verify both release gates. No behavior/API/schema change; self-review applies to this spacing-only fix. Prior release remains deployed as `d36ff5f`. Ads contract preparation resumes afterward; manual pilot setup and quick picks remain deferred.

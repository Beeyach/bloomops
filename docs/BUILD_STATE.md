...
**D2 — explicit GHL setup interface — is now in progress.** The generation HTTP/UI slice is CLOSED on verified main `c5d6b2edd4cd843f3e96c054e495f00bfbecbfb5`, [PR #45](https://github.com/Bloomwired/bloomops/pull/45), with both exact-SHA workflows, staging identity and disposable cleanup verified. Next is a narrow Settings flow for authorized users to explicitly enable/disable GHL builds for an eligible Service Type, reusing the reviewed provisioning/binding functions. No automatic live binding, general template editor, schema change or provider execution. Work is isolated in `/home/ary/Developer/bloomops-d2-setup`, branch `feat/d2-ghl-setup-interface`. The owner's standing instruction is to continue until actual input is needed. **D2 remains open.** Contract: [D2_SETUP_INTERFACE.md](phases/D2_SETUP_INTERFACE.md).
...
## D2 setup interface local acceptance (2026-09-12)

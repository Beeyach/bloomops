...
- staging can sign in only its bootstrapped addresses, and a real magic-link click from one of those mailboxes remains a manual step. Mail arrives only if the configured `BLOOMOPS_MAIL_FROM` sender is on a domain verified in Resend

...
**Release E — E1 locally accepted and reviewed; publication gates pending.** Read [RELEASE_E](RELEASE_E.md) and [E1](phases/E1.md). The internal Ads placeholder now has a bounded read-only view of canonical Ads Projects, scoped filters and Work summaries/links in the isolated checkout. Shared read composition was extracted from Systems without changing its eligibility or grants. No schema, migration, provider fact, Content/approval/metric or portal change. Initial local acceptance passed. Sol High found no permission defect and requested a structured Ads summary presentation plus a staged status correction. The presentation fix passes 166 focused tests, a fresh build and 123 browser/HTTP checks with 47 captures; the single focused Sol High re-review resolved both findings. Astra High remains appropriate. Latest deployed staging remains `707d2be` with 22 migrations; E1 is not yet merged or deployed. Human pilot setup and reusable quick picks remain follow-up work.

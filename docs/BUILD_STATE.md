...
| S05 | Multiple/custom purchased services | NEW FUNCTIONALITY IN PROGRESS. Canonical engagements plus additive immutable receipt items; custom reusable offerings restricted to current Owner/Admin and workspace-settings API access. Focused and native concurrency/zero-row rollback checks pass; browser verification pending. |
| S06 | Client prospect search/prefill | NEW FUNCTIONALITY IN PROGRESS. Add Client searches authorized current-workspace prospects, reviews saved identity and enters canonical explicit sale handoff; manual entry remains separate and is never overwritten. Existing conversions link to their client. Browser verification pending. |

...
**PERF4 staging timing integration is deployed at exact authorized application `6da1c2467ae0a4ae31bcefcd1ca16482c7f5bae5`; signed-in browser/Analytics Engine correlation is complete and awaits review. Live latency targets remain unmet. PERF3/PERF4 remain OPEN; D2 remains BLOCKED.** The evidence branch is `perf/perf4-staging-timing-integration`; its later documentation-only HEAD is not the deployed application. Main remains `2b892275abf0856dff10f31d49b2ff2da57acd5f`. No production deployment, migration, PR, merge or optimization occurred.

## PERF4 completed signed-in staging correlation (2026-09-11)

After Cloudflare authentication and existing authenticated Windows Chrome passed the resume gates, the exact clean detached candidate was built using its own Git SHA as authorized process-local build provenance. An initial temporary-filesystem capacity failure occurred before upload; moving the same worktree to the main filesystem allowed the Next/lint/type/OpenNext build and staging deployment to pass. Worker version `a96a03e1-fd2f-4d8b-ae26-87cc4ea5f5ab` was deployed at `2026-09-11T12:00:44.805Z` with 100% staging traffic. Before and after capture, the version gate returned HTTP 200 / **`6da1c24`** / `perf/perf4-staging-timing-integration`. The seven-character SHA is the existing repository representation, not an altered version implementation.

The same signed-in Windows Chrome completed **7 discarded warmups + exactly 21 retained navigations**. Click-to-visible medians (ms): Home **821.9**, Clients **575.6**, Work **601.2**, Social **592.2**, Systems **775.1**, Team **573.3**, Ads **396.1**. Overall visible median/p95 **592.2/821.9**; settled **695.0/928.4**, including a deliberate 100 ms guard. All retained requests completed HTTP 200 with one root RSC request, no redirect/prefetch, and no extra client resource waterfall. No retained visible or settled navigation exceeded one second, but six route medians exceed ~500 ms. The historical PERF3 pattern persists; no controlled improvement/regression or manual subjective speed assessment is claimed.

Analytics Engine returned **63/63** expected records for **21** retained IDs: missing **0**, duplicates **0**, incomplete triplets **0**, visibly sampled **0** (all sample intervals 1). Home/Systems each show three serial native D1 waits; Ads one. Server total medians are **586/550/179 ms** respectively; inclusive Home/Systems projections **404/366 ms** contain the final two waits. Ordinals do not identify SQL categories, inclusive spans must not be added twice, and browser/Worker clocks remain separate. Deployed Worker clocks advance with I/O; zero actor/unattributed timings cannot prove zero CPU cost. The measured critical path supports a database-round-trip hypothesis but does not yet distinguish transport/placement from query execution or select the smallest exact optimization safely.

All retained live records pass fixed-schema/privacy validation. One separate harmless marker probe yielded three sanitized records after an initial ingestion delay. No logs, invocation logs, traces or tailing were enabled or accessed. Existing auth succeeded before and after capture; no real session/membership mutation, email or business-data write. **deployed remote revocation remains unproven; local active-workerd revocation passed**. No clearly disposable staging test identity was established.

See [PERF4_RESULTS.md](PERF4_RESULTS.md) for every observed timing, medians/ranges, all-route server phases, clock/method caveats and five-layer evidence separation; [new live evidence](evidence/PERF4_staging_correlation_live.json) contains all 28 browser rows, 63 retained server records, decoded joins and the separate privacy probe. The old blocked-attempt artifact remains historical. This handoff changes documentation/evidence only from `2807704f29e39d2d0918167e9be3baf855fb33ba`, with independent schema/correlation/distribution/privacy checks and clean diff hygiene. No new full-suite run is claimed for docs-only edits. Only the evidence branch is committed/pushed; its final SHA is reported separately from deployed `6da1c2467ae0a4ae31bcefcd1ca16482c7f5bae5`. Stop for review; no performance fix or D2.

## Historical PERF4 blocked staging attempt (2026-09-11; superseded above)
...
Read [PERF4_RESULTS.md](PERF4_RESULTS.md) for integration behavior, Analytics Engine schema/retention/removal, exact verification provenance and remaining gates. The custom Worker delegates OpenNext, observes request-local native D1 ordinals and streamed completion, and sends only fixed sanitized numeric records keyed by a fresh opaque ID. Explicit staging environment/opt-in/origin/route/binding gates control activation. Default/production requests bypass diagnostics; ordinary Worker logs, invocation logs and traces stay disabled. No domain/auth-policy/query/schema/migration/dependency changes were made.

Extended run 34584611556 on `317766f7cc45cdd6216467b924c6a91913498ba8` passed 4,832 full tests, 23 Systems and 34 Home native D1 checks, both builds and staging dry-run, but failed loopback authentication due to `127.0.0.1` versus configured `localhost`. Correction `f82a1f86770d41d4ee2eee705cd20ef71cd850cb` aligns the verifier origin, includes all response tests in focused CI and makes the pre-response exception fixture exercise the active wrapper. Local focused tests pass 46/46. [Corrected run 34585369973](https://github.com/Beeyach/bloomops/actions/runs/34585369973) on that exact SHA passed 46 focused / 4,832 full tests, 23 Systems / 34 Home native D1 checks, both builds, staging bundle dry-run, 144/144 built development auth/authorization checks and diff hygiene. The final handoff removes the temporary branch verifier and preserves its tested commands/source hashes in the [verification evidence](evidence/PERF4_integration_verification.json). Runtime files are unchanged from `06a4951`; no new build is claimed for the subsequent docs/workflow-removal commit.

Active follow-up starts at exact `623e3041d3af14ddfaab4dcd337ce9ae95454b11`, with matching branch, clean tree and no PR confirmed. [New active-runtime evidence](evidence/PERF4_active_runtime.json) records **79/79 scenarios** in the built custom Worker/workerd: 25 activation/bypass cases, 32 overlapping requests with workspace/stage isolation, six issued-session auth/revocation cases and 16 stream/sink cases. Suspended/deleted membership and deleted session reject the next request; reactivation/restoration succeed. First chunk/EOF, pending-read cancellation, original read/pre-response errors, status/cookies/no-store/bytes, backpressure and subscriber cleanup pass. Fixed-schema capture contains no hostile markers, with zero console calls or external network attempts. No runtime source changed.

The integrated off/on run retains **480 of 528** RSC requests at representative/exact-3× scale. Median paired EOF deltas are Home **+1.260/+0.920 ms**, Systems **+2.446/+1.954 ms**, Ads **+1.185/+1.255 ms**; all retained pairs add 86 timing-header bytes and zero body bytes. All 264 timed requests yield unique IDs and three points (792 total); zero missing/duplicate/off-path records. This is actual local Worker/HTTP overhead with an in-process sink, not historical primitive cost or real Analytics Engine delivery. No threshold was invented. Complete distributions and limitations are in [PERF4_RESULTS.md](PERF4_RESULTS.md).

Fresh verification also passes 46 focused / 4,832 full tests, native Systems 23 / Home 34, Next-through-OpenNext build, staging dry-run, syntax/evidence/diff checks. The earlier 144-check development auth smoke remains historical and proves only the inactive path; it was not rerun. The final handoff adds only local test scripts, sanitized evidence and documentation. No migration, dependency, deployment configuration, auth policy or runtime change. The local gate is satisfied and the candidate is ready for **separately authorized signed-in staging correlation**, with the measured overhead available for review. Real delivery/retrieval, edge/browser timing and live revocation/privacy remain unproven; no dominant deployed bottleneck or performance fix is claimed. PERF3/PERF4 stay OPEN and D2 BLOCKED. No deployment, PR, merge, real email, remote data or Leadsthatbloom change occurred.

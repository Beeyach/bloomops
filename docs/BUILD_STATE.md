...
The native projection smokes do not activate the timing adapter; the built development auth smoke proves the default bypass only. Active workerd privacy/revocation/stream correlation, real Analytics Engine delivery and representative/exact-3× off/on wrapper/browser overhead remain outstanding. Historical primitive overhead is not integration acceptance. Signed-in staging is not recaptured and no dominant live server stage is proven. The next bounded work is active-runtime verification followed by separately authorized exact-SHA staging correlation; independent review and eventual deployment/zero gates remain required. No staging/production/Leadsthatbloom changes, real communication or D2 work occurred.

...
| Reporting completion (current) | Reviewed CSV (PR82/38df2ea), supported onboarding defaults and genuine portal activation (PR83/62ae623), immutable publication/revision/withdrawal, client-only current versions, matching PDF and new-period empty-draft reuse (PR84/79f74b3) are merged and staging verified. PR84 full7284/feature119, staging35045440644/remote-D135045440594 and31 genuine two-actor live checks pass, plus both empty new-period flows and inspected PDF pages. Archive/restore (PR85/6e2c5a1) is merged: full7291/feature135, staging35046937086 and remote-D135046937093 pass;16 genuine staging checks confirm read-only archive, old-link/PDF denial, private restore and explicit republish for both templates. Compatible comparisons/charts are implemented on feat/reporting-comparisons-20260916, source9c2ef56: focused77/native45, full7301 and completed stamped build pass; 149 completed-build browser checks pass with zero runtime errors;6 served JavaScript assets match unchanged artifacts. The first run caught a stale social expected count17 after an existing CSV correction to18; only that harness expectation changed, and the same completed application build passed. Actual comparison PDF pages were inspected. PR86 validation35047600143 exposed an existing onboarding-native harness comparison of volatile D1 query duration (1ms versus0ms), with identical persisted rows. The corrected assertion checks successful reads and every stored column; native onboarding5 passes. Its normal follow-up CI and comparison staging acceptance remain. Comparisons use compatible prior publications, visibly unverified counts and no invented growth percentage or trend. CLIENT_REPORTING.md and phases/N3.md own details. |

...
**D2 — GHL build interface and HTTP boundary — is now in progress.** The preparation/generation backend is CLOSED on verified main `8df6d318ae1889691fe73a7fd19e155c4f6f5d69`, [PR #44](https://github.com/Bloomwired/bloomops/pull/44), with both exact-SHA workflows, staging identity and disposable cleanup verified. The next bounded work exposes the reviewed backend through protected Project options/preview/generation endpoints and a select → preview → confirm/retry flow. No automatic Service Type binding, schema change or provider execution. Work is isolated in `/home/ary/Developer/bloomops-d2-interface`, branch `feat/d2-generation-interface`. The owner's standing instruction is to continue until actual input is needed. **D2 remains open.** Contract: [D2_INTERFACE.md](phases/D2_INTERFACE.md).

...
## Current direction — authenticated staging acceptance and ordered merge of PR64–66

Owner-authorized scope: finish authenticated staging acceptance, fix real findings, make PR64/65/66 ready, merge in dependency order after checks, and deploy/verify the final merged staging SHA. Global search follows this release. Preserve original root D2/P2 work, LTB records/voices, production and DNS. No real imports/outreach, paid audits or video work/tests.

The task uses isolated `/home/ary/Developer/bloomops-stack-staging` and a separate detached integration worktree. Main remains `4b7acea`. The existing service defaults were missing from runtime fresh-workspace creation, leaving client handoff with no purchased service choices. Fix `c023a2d` adds the four canonical departments and five types inside the guarded creation transaction. Retry/customization/source boundaries remain intact; no migration or dependency change. The fix is on PR64 and merged forward into PR65/66 without rewriting history.

Authenticated baseline staging acceptance now works with the owner session. Only synthetic QA records were created: a fresh QA workspace, one manual fictional prospect and the two Sample CSV rows. Verified import preview/duplicate and invalid row selection, durable receipt (2 added/1 skipped/1 invalid), visible websites/profile navigation, bulk fit/save/undo, saved views/filters/date order/50-row preference through reload, manual audit batch download with unchanged prospect revisions, and desktop1440/mobile390/320 layouts without document overflow. No real prospect import or provider operation occurred. Baseline Undo wording is already corrected by PR65. Client conversion remains pending until the fresh-catalogue fix is deployed.

Fresh fix verification:86 focused tests pass (workspace authority, atomic rollback, canonical defaults, concurrent retries/customization preservation, services, session and sheet/draft regressions); OpenNext build passes. Actual built Worker/native D1 creates a fresh workspace, reuses its receipt on retry and offers all five services in handoff. Initial added-test assertion errors were corrected. Existing PR64/65/66 reviews and broader acceptance remain accepted. One bounded fresh Sol High release/fix review is complete with no material findings. Private evidence: `/home/ary/Developer/bloomops-stack-staging-evidence/`.

Candidate staging acceptance is complete on `04cd43a` ([successful workflow34831084932](https://github.com/Bloomwired/bloomops/actions/runs/34831084932)). A second fresh synthetic QA workspace offers all five services. Explicit conversion creates canonical Draft client/Planned Kajabi service and preserves its prospect backlink/receipt/permanent stop. Missing templates are correctly reported as a separate onboarding prerequisite. Live N1 checks pass for typed reload/recovery, values hidden until access validation, actual offline failure/retry, accurate undo, workspace switch/return, concurrent edit protection, wrong-user403 and foreign-record404 without writes. Desktop1440/mobile390/320 and20 public candidate checks pass. [Visual acceptance and limitations](previews/stack-staging-acceptance/README.md). Real logout/revocation/lost-response and full onboarding generation retain accepted isolated evidence; the owner session was not signed out.

First unfinished gate: make the three PRs ready and merge in dependency order, retargeting after each prerequisite. Main remains `4b7acea`; PR64 `c023a2d`,PR65 `6acf9ac`,PR66 runtime `04cd43a`. No configured PR checks/protection/rulesets exist; their absence is not a CI pass. Main pushes trigger staging deployment and disposable zero-to-current verification. Verify each exact head and final deployed SHA/health/authenticated UI. No production job/configuration change.

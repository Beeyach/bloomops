...
Release E preparation is complete; E1 is locally accepted and independently reviewed, with publication gates pending. This is a release contract, not evidence of shipped Ads functionality. [BUILD_STATE](BUILD_STATE.md) owns current execution status.
...
The later creative slice should extend the canonical Content model with an explicit, immutable Ads production context and a same-workspace, same-Client Ads Project relationship. Preserve existing Social records and routes as Social; do not migrate them based on their title, type, platform or department display text. Do not create a second editorial, File or approval engine.

The E2 contract must settle the exact additive schema and all affected callers before changing eligibility. Reads and writes must prove both current Ads parent eligibility and their own Content authority. A Project-only assignment must not become Client/Service management authority or a blanket grant to existing Social Content. Changing a parent's visibility, relationship or assignment must revoke dependent access immediately. Creative ownership remains responsibility, not a grant.
...
| E2 | Explicit Ads creative context within canonical Content, with safe parent binding and production workflow | Write its bounded contract first; additive schema and permission review required |

...
export const POST=withApiErrors(async(req,{params})=>{const {access,response:denied}=await requireAuthorized(req,{action:'prospecting.manage'});if(denied)return denied;const body=await readStructuredBody(req,{maxBytes:32000});return response(await (body.services!==undefined?previewMultiHandoff:previewProspectConversion)(access.db,access.actor,(await params).id,body));});

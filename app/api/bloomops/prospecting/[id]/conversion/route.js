...
export const POST=withApiErrors(async(req,{params})=>{const {access,response:denied}=await prospectAccess(req,'prospecting.manage');if(denied)return denied;return response(await convertProspect(access.db,access.actor,(await params).id,await readStructuredBody(req,{maxBytes:32000})));});

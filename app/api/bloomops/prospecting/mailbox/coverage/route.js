...
import {saveProspectHistoricalCoverage} from '@/lib/bloomops/prospect-mailbox-review.mjs';
import {prospectAccess} from '../../_shared.mjs';
export const dynamic='force-dynamic';
export const POST=withApiErrors(async req=>{const {access,response}=await prospectAccess(req,'prospecting.manage');if(response)return response;const input=await readStructuredBody(req,{maxBytes:4096});const result=await saveProspectHistoricalCoverage(access.db,access.actor,access.env,access.session.id,input);return result===null?notFound():json(result,result.conflict?409:200);});

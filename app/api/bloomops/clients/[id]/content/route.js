...
export const POST = withApiErrors(async (req,context) => {
  ...
  const {access,response}=await contentParentAccess(req,id,null); if(response)return response;
  const parsed=await contentBody(req,true); if(parsed.response)return parsed.response;
  const {requestId,...input}=parsed.body;
  return contentResponse(await createContent(access.db,{actor:access.actor,clientId:id,serviceEngagementId:null,input,requestId}),201);
});

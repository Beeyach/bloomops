...
export const GET=withApiErrors(async(req,{params})=>{
 ...
 if([...q.keys()].some(k=>!['page','services','search'].includes(k)||q.getAll(k).length!==1)||q.has('services')&&q.get('services')!=='true'||q.has('search')&&!q.has('services'))return json({error:'Invalid report query.'},400);
 const options={page:Number(q.get('page')||1),search:q.get('search')||''};

...
export const PATCH=withApiErrors(async req=>{const {access,response}=await requireAccess(req);if(response)return response;const result=await manageOnboardingSetup(access.db,await getActor(access),await readStructuredBody(req));return result.reason==='not_found'?notFound():json(result,result.ok?200:result.reason==='conflict'?409:400);});

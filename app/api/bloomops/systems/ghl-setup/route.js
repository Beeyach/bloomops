...
import { configureGhlBlueprint, systemsBlueprintSetupOptions } from '@/lib/bloomops/systems-blueprint-setup.mjs';
export const dynamic = 'force-dynamic';
const accessFor = req => workQueryAccess(req, requireAuthorized(req, { action: 'templates.manage' }));
function respond(result) {
  if (result.ok) return json(result);
  if (result.reason === 'forbidden') return json({ error: 'You do not have permission to manage GHL setup.' }, 403);
  ...
  return json({ error: 'This configuration changed or its GHL blueprint is unavailable. Refresh the configuration before trying again.' }, 409);
}
export const GET = withApiErrors(async req => {
  const { access, response } = await accessFor(req);
  return response || respond(await systemsBlueprintSetupOptions(access.db, { actor: access.actor }));
});
export const POST = withApiErrors(async req => {
  ...
  return respond(await configureGhlBlueprint(access.db, { actor: access.actor, input: parsed.body }));
});

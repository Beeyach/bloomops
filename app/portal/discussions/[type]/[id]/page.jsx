...
export default async function PortalDiscussionPage({params,searchParams}){
  const {access,actor}=await requireShell('portal'),parent=await params,q=await searchParams||{};
  ...
  return <RecordDiscussion initial={data} workspaceId={actor.workspaceId} api={`/api/bloomops/discussions/${parent.type}/${parent.id}`} returnHref="/portal" portal/>;
}

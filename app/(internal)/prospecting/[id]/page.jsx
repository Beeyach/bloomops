...
export default async function ProspectPage({params,searchParams}){const {access,actor}=await requireShell('internal');const {id}=await params;const query=await searchParams;const data=await getProspect(access.db,actor,id,{activityPage:Number(query.activityPage||1)});if(!data)notFound();const saved=await getProspectConversion(access.db,actor,id);if(!saved.ok)notFound();return <ProspectProfile key={JSON.stringify([actor.userId,actor.workspaceId,id])} userId={actor.userId} contactEvent={['contact','interest','resolved'].includes(query.contactEvent)?query.contactEvent:null} initial={{...data,conversion:saved.receipt,sheetFacts:await getProspectSheetFacts(access.db,actor,id)}}/>;}

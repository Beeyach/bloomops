...
export default async function CalendarPage({searchParams}) {
  ...
  if(!month||Object.keys(query).some(k=>!allowed.includes(k)||typeof query[k]!=='string'))return <><Notice tone="error">Choose a valid month and available filters.</Notice><Button href="/social/calendar">Reset calendar</Button></>;
  const {month:ignored,...filters}=query,result=await contentCalendar(access.db,actor,{...filters,start:month.start,end:month.end});
  if(!result.ok)return <><Notice tone="error">Choose a valid month and available filters.</Notice><Button href="/social/calendar">Reset calendar</Button></>;

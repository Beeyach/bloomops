...
export default async function Reports({params,searchParams}){
 ...
 return <div className="bo-report"><PageHeader title={`Reports for ${view.client.name}`} subtitle="Manage private drafts and explicitly published client versions."/>
 <div className="bo-form-actions"><Button href={`/clients/${id}`} variant="ghost">Back to client</Button>{view.canEdit&&<Button variant="primary" href={`/clients/${id}/reports/new`}>New report</Button>}</div>

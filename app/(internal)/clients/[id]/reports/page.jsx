...
export default async function Reports({params,searchParams}){
 ...
 {!view.items.length?<p>{view.archived?'No archived reports.':'No report drafts yet.'}</p>:<ul className="bo-rows">{view.items.map(r=><li key={r.id}><a href={`/clients/${id}/reports/${r.id}`}>{r.title}</a><p>{reportTemplate(r.templateId,r.templateVersion)?.label} — {r.periodStart} to {r.periodEnd}</p><p>{view.archived?'Archived — internal only':'Private draft'}</p></li>)}</ul>}

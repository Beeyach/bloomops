...
test('a fresh database reaches the A2 schema from the committed migrations alone', () => {
  ...
  assert.equal(expected.length, 102, 'P3 follow-up sequences add immutable schedule and attempt provenance');
...
test('migrations are additive and ordered, so applying them is deterministic', () => {
  ...
  assert.equal(files[58].tag, '0058_tricky_bromley');
  assert.equal(files[57].tag, '0057_neat_ricochet');
  assert.equal(files[56].tag, '0056_solid_gladiator');
  assert.equal(files[55].tag, '0055_chilly_alex_wilder');
  assert.equal(files[52].tag, '0052_colorful_norrin_radd');
  assert.equal(files[48].tag, '0048_clumsy_nekra');
  assert.equal(files[49].tag, '0049_easy_groot');
  assert.equal(files[50].tag, '0050_clammy_mordo');
  assert.equal(files[51].tag, '0051_broken_proteus');
  assert.equal(files[45].tag, '0045_record_discussions');
  assert.equal(files[46].tag, '0046_in_app_notifications');
  ...
  assert.equal(files[20].tag, '0020_d2_slice2c_version_guards');

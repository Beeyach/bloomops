...
test('C2 adds only current context and stage index; historical schema, triggers and rows survive',async()=>{
 ...
 const t=await setup();assert.equal(all(t.raw,'PRAGMA table_info(content_items)').length,26);assert.equal(all(t.raw,"SELECT name FROM sqlite_master WHERE type='trigger' AND tbl_name='content_items' AND name IN ('content_items_identity_immutable','content_items_social_service')").length,2);assert.deepEqual(all(t.raw,'PRAGMA foreign_key_check'),[]);
});

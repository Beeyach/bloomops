...
test('read model resolves only an opaque current selection and hides cursors',async c=>{
 ...
 assert.ok((await saveProspectMonitoringCheckpoint(t.db,t.actor,t.env,t.session,input)).checked);const after=await getProspectMailboxReview(t.db,t.actor,t.env);assert.ok(after.checkpoint.savedAt);assert.equal(after.checkpoint.reason,'used');assert.equal(after.coverage,'gap');assert.equal(after.historical.status,'unverified');
});

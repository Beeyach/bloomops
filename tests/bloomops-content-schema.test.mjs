...
test('C1 additive migration preserves all prior tables, composite ownership and snapshot chain',async()=>{
 ...
 const t=await setup();assert.equal(all(t.raw,'PRAGMA table_info(content_items)').filter(c=>c.name!=='stage_context').length,23);assert.deepEqual(all(t.raw,'PRAGMA foreign_key_check'),[]);

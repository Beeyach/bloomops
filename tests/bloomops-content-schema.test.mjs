...
test('C1 additive migration preserves all prior tables, composite ownership and snapshot chain',async()=>{
 ...
 const t=await setup();assert.equal(all(t.raw,'PRAGMA table_info(content_items)').filter(c=>!['stage_context','production_area','ads_project_id'].includes(c.name)).length,23);assert.deepEqual(all(t.raw,'PRAGMA foreign_key_check'),[]);

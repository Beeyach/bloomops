...
export default function ContentCalendar({result,month,query={},options}) {
  ...
  return <>
    ...
    <nav className="bo-content-pagination" aria-label="Calendar months">{month.previous&&<Button href={href({month:month.previous,page:'1'})}>Previous month</Button>}<p><strong>{new Intl.DateTimeFormat('en-US',{month:'long',year:'numeric',timeZone:'UTC'}).format(new Date(month.month+'-01T12:00:00Z'))}</strong></p>{month.next&&<Button href={href({month:month.next,page:'1'})}>Next month</Button>}</nav>

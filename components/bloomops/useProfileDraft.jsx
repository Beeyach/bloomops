...
export default function useProfileDraft({userId,workspaceId,prospectId,section,draft,onRecover,onLost}){
 ...
 function persist(){if(!valid()||!store.current)return;try{store.current.write(latest.current&&profileDraftDirty(latest.current)?latest.current:null);setStorageError('');}catch(e){if(alive.current)setStorageError(e.message);}}
 ...
 },[userId,workspaceId,prospectId,section]);
 ...
 async function read(){
  ...
  const response=await fetch('/api/bloomops/prospecting/'+prospectId+'/recovery',{method:'POST',cache:'no-store',signal:controller.signal,headers:{'content-type':'application/json'},body:JSON.stringify({userId,workspaceId})});
  ...
  if(body.userId!==userId||body.workspaceId!==workspaceId||body.profile?.id!==prospectId){invalidate(true);throw cancelled();}return body;
 }
 async function recover(key){
  if(recovering||!valid())return;setRecovering(true);setError('');
  try{const checked=store.current?.snapshot(key);if(!checked)throw Error('This recovery copy is no longer available.');const current=await read();if(!valid())return;source.current={key,raw:checked.raw};callbacks.current.onRecover(checked.value,current);}
  catch(e){if(valid()&&e.name!=='AbortError')setError(e.message);}finally{if(alive.current)setRecovering(false);}
 }
 function retire(){if(!valid())return;try{store.current?.write(null);if(source.current)store.current?.remove(source.current.key,source.current.raw);source.current=null;setStorageError('');list();}catch{setStorageError('The recovery copy could not be removed from this browser.');}}

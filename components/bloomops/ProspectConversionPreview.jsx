...
export default function ProspectConversionPreview({initial,userId,initialProfile}){
 ...
 const sequence=useRef(0),form=useRef(null),report=useRef(null),profile=options.profile,endpoint='/api/bloomops/prospecting/'+profile.id+'/conversion-preview';
 ...
 async function preview(event){event.preventDefault();if(busy||searching||saving)return;setConfirmed(false);setSaveError('');pending.current=null;setBusy(true);setMessage('');setErrors({});setResult(null);const stamp=++sequence.current;
  try{const response=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({workspaceId:profile.workspaceId,mode,clientId:mode==='existing'?chosenClient?.id:null,services:chosenServices.map(({id,packageName,scopeNotes})=>({serviceTypeId:id,packageName,scopeNotes}))})}),data=await response.json();if(!alive.current||stamp!==sequence.current)return;if(!response.ok||!data.ok){setErrors(data.errors||{});setMessage(data.errors?.form||'Check the highlighted fields and try again.');requestAnimationFrame(()=>form.current?.querySelector('[aria-invalid=true]')?.focus());return;}setResult(data);requestAnimationFrame(()=>report.current?.focus());}catch{if(alive.current&&stamp===sequence.current)setMessage('Couldn’t check this handoff. Your selections are still here.');}finally{if(alive.current&&stamp===sequence.current)setBusy(false);}
 }
 ...
 return <div className="bo-prospect-page bo-handoff"><Button href={'/prospecting/'+profile.id} variant="ghost" icon="chevron-left">Back to prospect</Button>
  ...
  <div className="bo-handoff-client"><fieldset disabled={busy||saving||!!offeringEditor}>
   <section><h2>Client</h2><div className="bo-handoff-mode" role="group" aria-label="Client destination">{[['new','New client'],['existing','Existing client']].map(([value,label])=><button type="button" key={value} aria-pressed={mode===value} onClick={()=>{invalidate();setMode(value);}}><Icon name={value==='new'?'plus':'clients'} size={18}/>{label}</button>)}</div>{errors.mode&&<p className="bo-handoff-error">{errors.mode}</p>}
    {mode==='new'&&<dl className="bo-handoff-facts"><div><dt><Icon name="clients" size={16}/>Proposed client name</dt><dd>{profile.businessName}</dd></div><div><dt><Icon name="user" size={16}/>Contact from prospect profile</dt><dd>{profile.personName||'Not recorded'}</dd></div><div><dt><Icon name="mail" size={16}/>Contact email to confirm</dt><dd>{profile.publicEmail||'Not recorded'}</dd></div></dl>}
  ...
  {mode==='new'&&profileData&&<EditableSection section="identity" title="Primary contact" displayTitle="Primary contact" fieldKeys={['personName','publicEmail']} data={profileData} userId={userId} disabled={saving||busy} onEditingChange={setIdentityEditing} onEdit={invalidate} onSave={saveIdentity} onCurrent={currentIdentity} onLost={()=>{alive.current=false;sequence.current++;setAccessLost(true);}}><p>Confirm the contact name and email before recording the sale. Only saved prospect details are shown.</p></EditableSection>}
  <form ref={form} onSubmit={preview} aria-label="Client handoff review" noValidate><fieldset disabled={busy||saving||!!offeringEditor}>
   <section><h2>Purchased services</h2>{choices('service',options.services,null,row=>setChosenServices(values=>values.some(v=>v.id===row.id)?values.filter(v=>v.id!==row.id):[...values,{...row,packageName:'',scopeNotes:''}]))}{errors.serviceTypeId&&<p className="bo-handoff-error">{errors.serviceTypeId}</p>}
    {chosenServices.map(service=><fieldset key={service.id} className="bo-handoff-purchased"><legend>{service.name}</legend>
     <div className="bo-handoff-field"><label htmlFor={'handoff-package-'+service.id}>Package name</label><span>Optional</span><input aria-invalid={!!errors.services?.[service.id]?.packageName} aria-describedby={errors.services?.[service.id]?.packageName?'handoff-package-error-'+service.id:undefined} id={'handoff-package-'+service.id} value={service.packageName} maxLength={120} onChange={e=>{invalidate();setChosenServices(values=>values.map(v=>v.id===service.id?{...v,packageName:e.target.value}:v));}}/></div>
     <div className="bo-handoff-field"><label htmlFor={'handoff-scope-'+service.id}>Agreed scope (required)</label><textarea required aria-invalid={!!errors.services?.[service.id]?.scopeNotes} aria-describedby={errors.services?.[service.id]?.scopeNotes?'handoff-scope-error-'+service.id:undefined} id={'handoff-scope-'+service.id} rows={3} value={service.scopeNotes} maxLength={2000} onChange={e=>{invalidate();setChosenServices(values=>values.map(v=>v.id===service.id?{...v,scopeNotes:e.target.value}:v));}}/></div>
     {['scopeNotes','packageName'].map(key=>errors.services?.[service.id]?.[key]&&<p key={key} className="bo-handoff-error" id={'handoff-'+(key==='scopeNotes'?'scope':'package')+'-error-'+service.id}>{errors.services[service.id][key]}</p>)}
  ...
  <div className="bo-handoff-research"><a href={'/prospecting/'+profile.id} target="_blank" rel="noopener noreferrer">Open full prospect research in a new tab</a><p>Your selected services remain here while you review the profile.</p></div>

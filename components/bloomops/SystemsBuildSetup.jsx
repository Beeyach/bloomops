...
export default function SystemsBuildSetup({ options: initialOptions, platform, endpoint }) {
  ...
  const save = () => perform(async () => {
    ...
    setProposal(null); setMustRefresh(false); toast(`${platform} builds ${input.enabled ? 'enabled' : 'disabled'} for ${name}.`);
  });
  return <div className="bo-page-narrow">
    <p className="bo-body">Enabling {platform} builds lets authorized project managers choose components and generate work in an empty planned project for that service.</p>
    <p className="bo-small">Existing projects stay as they are. This setup does not connect to or make changes in {platform}.</p>
    {options.serviceTypes.length ? <form onSubmit={event => { event.preventDefault(); if (!selected?.available || unchanged || busy || mustRefresh) return;
      ...
      <Field id="systems-service-type" label="Service type"><select {...fieldAria({ id: 'systems-service-type' })} className="bo-control" value={serviceTypeId} onChange={event => choose(event.target.value)} disabled={busy || mustRefresh}>
      ...
      {selected && (selected.available ? <>
        <p className="bo-small">{selected.binding?.enabled ? `${platform} builds are enabled for this service type.` : `${platform} builds are disabled for this service type.`}</p>
        <label className="bo-build-check"><input type="checkbox" checked={enabled} onChange={event => setEnabled(event.target.checked)} disabled={busy || mustRefresh} />Enable {platform} builds</label>
    ...
    <Dialog open={!!proposal} onClose={close} title={proposal?.enabled ? `Enable ${platform} builds` : `Disable ${platform} builds`}>
      <div className="bo-dialog-body">
        <p className="bo-body">{proposal?.enabled ? `Enable ${platform} builds for ${proposal.name}?` : `Disable future ${platform} builds for ${proposal?.name}?`}</p>

...
export default function FinanceAccess({member,scope}){
 ...
 return <form onSubmit={save} className="bo-finance-row" aria-label={member.name+' Finance access'}><h2 className="bo-h3">{member.name}</h2><p>{member.role.replaceAll('_',' ')}</p>{member.role==='owner'?<p>Owner: view and edit (inherent access).</p>:<><label>Finance access for {member.name}<select aria-label={'Finance access for '+member.name} disabled={busy} value={mode} onChange={e=>setMode(e.target.value)}><option value="none">None</option><option value="view">View</option><option value="edit">View and edit</option></select></label><Button type="submit" loading={busy} disabled={busy}>Save access</Button></>}<p role="status">{message}</p></form>;
}

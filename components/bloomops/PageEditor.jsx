...
export default function PageEditor({initial,userId}){
 const pages=usePages(),titleField=useRef(null),toolsRef=useRef(null);
 ...
 useEffect(()=>{
  const dismiss=event=>{const tools=toolsRef.current;if(!tools?.open)return;if(event.type==='keydown'&&event.key==='Escape'){event.preventDefault();tools.open=false;tools.querySelector('summary')?.focus();}else if(event.type==='pointerdown'&&!tools.contains(event.target))tools.open=false;};
 ...
 return <div className="bo-page-editor"><div className="bo-page-toolbar"><PageBreadcrumbs id={initial.id}/>  <details ref={toolsRef} className="bo-page-tools"><summary>Page tools<span className="sr-only">: templates and record context</span></summary><div className="bo-page-tools-panel">

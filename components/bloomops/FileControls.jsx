...
export default function FileControls({ projectId, contentId = null, portal = false, summary, deliverables = [], mayManage, canRestrict }) {
  ...
  const [purpose, setPurpose] = useState('recording');
  ...
  function open(kind, item = null) {
    ...
    requestId.current = crypto.randomUUID(); setPurpose('recording'); setDialog({ kind, item });
  }
  ...
  return <Section id={contentId ? 'content-files' : 'project-files'} title={contentId ? portal ? 'Your recordings' : 'Recordings & assets' : 'Files'} aside={mayManage && <Button id="upload-file" size="sm" disabled={busy || !ready} onClick={() => open('upload')}>{portal ? 'Upload recording' : 'Upload file'}</Button>}>
    <p className="bo-small">{contentId ? portal ? 'Send a short recording for this request. Your team will take it from here.' : 'Original recordings and production assets for this Content. Uploading does not change its production stage.' : 'Files for this Project and its Deliverables.'}</p>
    ...
    <Dialog open={Boolean(dialog)} onClose={close} title={dialog?.kind === 'archive' ? 'Archive file' : dialog?.kind === 'visibility' ? 'Change file visibility' : dialog?.kind === 'retry' ? 'Retry upload' : portal ? 'Upload recording' : 'Upload file'}
      ...
      {dialog && <form onSubmit={submit} noValidate><div className="bo-dialog-body">
        ...
        {dialog.kind === 'upload' && contentId && !portal && <Field id="file-purpose" label="Purpose"><select id="file-purpose" className="bo-control" value={purpose} disabled={busy} onChange={event => setPurpose(event.target.value)}><option value="recording">Recording</option><option value="asset">Production asset</option></select></Field>}
        {!portal && ['upload', 'visibility'].includes(dialog.kind) && <Field id="file-visibility" label="File visibility" hint={contentId ? 'Clients can download only shared recordings while a shared Content item is waiting for their recording. Production assets stay internal in this phase.' : 'Clients can download only when this File, its Project and any attached Deliverable are all shared.'}><select {...fieldAria({ id: 'file-visibility', hint: true })} className="bo-control" value={visibility} disabled={busy} onChange={event => setVisibility(event.target.value)}>

...
export default function ReportPublication({initial,clientId,reportId,scope}){
 const [data,setData]=useState(initial),[ack,setAck]=useState(false),[error,setError]=useState(''),[busy,setBusy]=useState(false),[checking,setChecking]=useState(false);
 ...
 async function change(kind){if(inFlight.current||checking)return;inFlight.current=true;setBusy(true);setError('');const g=++write.current;
 ...
 const {review,history}=data,locked=busy||checking||!!pending.current;
 ...
 <p role="status">{busy?'Recording publication change…':checking?'Checking current access…':review.current?.kind==='publish'?`Published version ${review.current.sequence} is available to the authorized client.`:review.current?'Withdrawn from client access. History is preserved.':'Private draft. No client version exists.'}</p>

...
export default function PublishedReport({initial,scope,api}){
 ...
 const [downloading,setDownloading]=useState(false),[ready,setReady]=useState(false),active=useRef(true),downloadGeneration=useRef(0);
 useEffect(()=>{active.current=true;setReady(true);return()=>{active.current=false;downloadGeneration.current++;};},[]);
 async function download(){if(!ready||downloading)return;setDownloading(true);setError('');const g=++downloadGeneration.current;try{const r=await fetch(api+'/pdf',{cache:'no-store'});if(!r.ok){const b=await r.json();throw Error(b.error||'PDF is no longer available.');}const blob=await r.blob();if(!active.current||g!==downloadGeneration.current)return;const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`client-report-v${data.version}.pdf`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}catch(e){if(active.current&&g===downloadGeneration.current)setError(e.message);}finally{if(active.current&&g===downloadGeneration.current)setDownloading(false);}}
 ...
 return data?<><Button disabled={!ready||downloading} onClick={download}>{downloading?'Preparing PDF…':'Download this published PDF'}</Button>{error&&<p role="alert">{error}</p>}<PublishedReportContent snapshot={data.snapshot} version={data.version} publishedAt={data.publishedAt}/></>:<p role="alert">{error}</p>;
}

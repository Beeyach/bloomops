...
export default function ProspectProfile({initial,userId,contactEvent=null}){
 ...
 const sources=<details id="provenance" className="bo-prospect-evidence bo-profile-sources"><summary>Sources & verification</summary>
  ...
  <ul className="bo-prospect-sources">{data.sources.map(source=><li key={source.fieldKey}><h3>{PROSPECT_FIELDS[source.fieldKey]?.label||'Field'}</h3><dl><div><dt><Icon name="external-link" size={16}/>Source</dt><dd>{safeProspectUrl(source.sourceUrl)?<a href={safeProspectUrl(source.sourceUrl)} target="_blank" rel="noreferrer">{source.sourceUrl}</a>:'Entered manually'}</dd></div><div><dt><Icon name={source.verification==='checked'?'shield-check':'info'} size={16}/>Verification</dt><dd>{source.verification==='checked'?'Checked against source':'Not checked'}</dd></div><div><dt><Icon name="calendar" size={16}/>Checked time</dt><dd>{time(source.checkedAt)}</dd></div><div><dt><Icon name="user" size={16}/>Last edited by</dt><dd>{source.actorName||'Workspace member'}</dd></div></dl></li>)}</ul>{!data.sources.length&&<p>No field sources recorded.</p>}
 </details>;

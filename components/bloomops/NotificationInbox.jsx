...
export default function NotificationInbox({initial,scope}){
 ...
 const version=useRef(0),pending=useRef(false),current=useRef({category,unread,page:data?.page||1});current.current={category,unread,page:data?.page||1};
 ...
 async function load(nextCategory=category,nextUnread=unread,page=1,{quiet=false}={}){const run=++version.current;if(!quiet)setBusy(true);setError('');
  try{const next=await notificationRequest(`${api}?${new URLSearchParams({category:nextCategory,unread:String(nextUnread),page:String(page)})}`);if(run!==version.current)return;
   if(!sameScope(next)){setData(null);setSettings(null);setError('Your account or workspace changed. Reload this page.');return;}
   setData(next);setCategory(nextCategory);setUnread(nextUnread);
  }catch(e){if(run===version.current){setError(e.message);if(quiet||[401,403,404].includes(e.status)){setData(null);setSettings(null);}}}finally{if(run===version.current)setBusy(false);}}
 useEffect(()=>{setReady(true);const refresh=()=>{if(document.hidden||pending.current)return;const s=current.current;load(s.category,s.unread,s.page,{quiet:true});};
  const timer=setInterval(refresh,30000);window.addEventListener('focus',refresh);window.addEventListener('pageshow',refresh);return()=>{version.current++;clearInterval(timer);window.removeEventListener('focus',refresh);window.removeEventListener('pageshow',refresh);};},[]);
 async function mutate(payload){if(pending.current)return;pending.current=true;version.current++;setBusy(true);setError('');setNotice('');
  try{const result=await notificationRequest(api,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...scope,...payload})});if(payload.action==='open'){window.location.assign(result.href);return;}
   if(payload.action==='preferences')setSettings(old=>({...old,[payload.category]:payload.enabled}));
   setNotice(payload.action==='preferences'?'Preference saved.':payload.action==='markAll'?'Notifications marked read.':'Notification updated.');await load(category,unread,data?.page||1);
  }catch(e){setError(e.message);if([401,403,404].includes(e.status)){setData(null);setSettings(null);}}finally{pending.current=false;setBusy(false);}}
 async function preferences(){setShowSettings(v=>!v);if(showSettings)return;const run=++version.current;setBusy(true);setError('');try{const next=await notificationRequest(api+'?settings=true');if(run!==version.current)return;if(!sameScope(next)){setData(null);setSettings(null);setError('Your account or workspace changed. Reload this page.');return;}setSettings(next);}catch(e){if(run===version.current){setError(e.message);if([401,403,404].includes(e.status)){setData(null);setSettings(null);}}}finally{if(run===version.current)setBusy(false);}}
 const disabled=busy||!ready;
 return <article className={styles.page}>
  <header className={styles.header}><span className={styles.headingIcon}><Icon name="bell" size={23}/></span><div><h1>Notifications</h1><p>{data?`${data.unread} unread${category==='all'?'':` in ${labels[category].toLowerCase()}`}`:'Inbox unavailable'}</p></div><Button variant="ghost" icon="settings" onClick={preferences} disabled={disabled} aria-expanded={showSettings}>Preferences</Button></header>
  {showSettings&&<section className={styles.preferences} aria-label="Notification preferences"><h2>Notify me about</h2><p>Applies to new notifications in this workspace.</p>{settings?Object.entries(labels).map(([key,label])=><label key={key}><Icon name={icons[key]} size={18}/><span>{label}</span><input type="checkbox" checked={settings[key]} disabled={disabled} onChange={e=>mutate({action:'preferences',category:key,enabled:e.target.checked})}/></label>):<p role="status">{error?'Preferences unavailable.':'Loading preferences…'}</p>}</section>}
  ...
  <div className={styles.feedback} role="status">{busy?'Updating…':notice}</div>{error&&<p className={styles.error} role="alert">{error}</p>}

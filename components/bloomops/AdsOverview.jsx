...
const pageHref = (filters, page) => '/ads?' + new URLSearchParams({ ...filters, page });

export function AdsOverview({ projection }) {
  const { filters, projects, deliverables, options } = projection;
  return <div className="bo-ads">
    <form className="bo-action-filters" action="/ads" aria-label="Ads filters">
      <Field id="ads-client" label="Client"><select className="bo-control" id="ads-client" name="clientId" defaultValue={filters.clientId}>
        <option value="">All clients</option>{options.clients.items.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}
      </select></Field>
      <Field id="ads-service" label="Service"><select className="bo-control" id="ads-service" name="serviceEngagementId" defaultValue={filters.serviceEngagementId}>
        <option value="">All Ads services</option>{options.services.items.map(item => <option key={item.id} value={item.id}>{item.clientName}: {item.name}</option>)}
      </select></Field>
      <Field id="ads-status" label="Project status"><select className="bo-control" id="ads-status" name="status" defaultValue={filters.status}>
        <option value="active">Open projects</option><option value="all">All statuses</option>
        {PROJECT_STATUSES.map(status => <option key={status} value={status}>{PROJECT_STATUS_LABELS[status]}</option>)}
      </select></Field>
      <div className="bo-action-filter-buttons"><Button type="submit">Apply filters</Button><Button href="/ads" variant="ghost">Reset filters</Button></div>
    </form>
    ...
    <Section id="ads-projects" title="Campaign work">
      ...
      {(filters.page > 1 || projects.hasMore) && <nav className="bo-action-pagination" aria-label="Ads project pages">

...
export default function ClientReportPreview({initial,scope}){
 ...
 return <article className="bo-report bo-report-preview"><p className="bo-report-private">Private preview — internal access only. Nothing has been published.</p>
 ...
 <h2>Manual observations</h2><p>All values are manually entered and unverified. Source notes provide context, not automatic verification.</p>
 <dl className="bo-report-observations">{template.metrics.map(m=>{const v=report.metrics[m.key];return <div key={m.key}><dt>{m.label}</dt><dd><strong>{v.state==='value'?v.value.toLocaleString('en-US'):{missing:'Not supplied',unavailable:'Unavailable',not_tracked:'Not tracked'}[v.state]}</strong><p>{m.definition}</p><p className="bo-report-prose">Source: {v.sourceNote||'Not supplied — unverified'}</p><p>Collected: {v.collectedAt||'Not supplied'}</p></dd></div>;})}</dl>
